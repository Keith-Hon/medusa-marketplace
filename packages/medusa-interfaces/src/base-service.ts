import { MedusaError } from "@medusa-marketplace/medusa-core-utils";
...
class BaseService {
   ...
   */
  buildQuery_(selector, config: Record<string, any> = {}) {
    const build = (obj) => {
          ...
          case value !== null && typeof value === "object":
            {
              ...
              if (subquery.length) {
                acc[key] = Raw(
                  (a) => subquery.map((s, index) => `${a} ${s.operator} :${index}`).join(" AND "),
   ...
   */
  validateId_(rawId, config: Record<string, any> = {}) {
    const { prefix, length } = config;
    if (!rawId) {
      throw new MedusaError(MedusaError.Types.INVALID_DATA, `Failed to validate id: ${rawId}`);
    }

    if (prefix || length) {
      const [pre, rand] = rawId.split("_");
      if (prefix && pre !== prefix) {
        throw new MedusaError(MedusaError.Types.INVALID_DATA, `The provided id: ${rawId} does not adhere to prefix constraint: ${prefix}`);
      }

      if (length && length !== rand.length) {
        throw new MedusaError(MedusaError.Types.INVALID_DATA, `The provided id: ${rawId} does not adhere to length constraint: ${length}`);
      }
    }
   ...
   */
  async atomicPhase_(work, isolationOrErrorHandler, maybeErrorHandlerOrDontFail) {
