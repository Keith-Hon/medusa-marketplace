...
export async function request(method, url, opts = {}) {
    ...
    if (opts.adminSession) {
        if (opts.adminSession.jwt) {
            opts.adminSession.jwt = jwt.sign(opts.adminSession.jwt, config.projectConfig.jwt_secret, {
                expiresIn: "30m"
            });
        }
    ...
    if (opts.clientSession) {
        if (opts.clientSession.jwt) {
            opts.clientSession.jwt = jwt.sign(opts.clientSession.jwt, config.projectConfig.jwt_secret, {
                expiresIn: "30d"
            });
        }
