...
import { IsArray, IsBoolean, IsEnum, IsOptional, IsString, Validate, ValidateNested } from "class-validator";
...
export class AdminUpsertConditionsReq {
    @Validate(ExactlyOne, ["product_collections", "product_types", "product_tags", "customer_groups"])
    @IsArray()
    @IsOptional()
    @IsString({ each: true })
    products?: string[];

    @Validate(ExactlyOne, ["products", "product_types", "product_tags", "customer_groups"])
    @IsArray()
    @IsOptional()
    @IsString({ each: true })
    product_collections?: string[];

    @Validate(ExactlyOne, ["product_collections", "products", "product_tags", "customer_groups"])
    @IsArray()
    @IsOptional()
    @IsString({ each: true })
    product_types?: string[];

    @Validate(ExactlyOne, ["product_collections", "product_types", "products", "customer_groups"])
    @IsArray()
    @IsOptional()
    @IsString({ each: true })
    product_tags?: string[];

    @Validate(ExactlyOne, ["product_collections", "product_types", "products", "product_tags"])
    @IsArray()
    @IsOptional()
    @IsString({ each: true })
