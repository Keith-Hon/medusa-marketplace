...
const migrate = async function ({ typeormConfig }): Promise<void> {
    ...
    await connection.transaction(async (manager) => {
        const discountRuleCount = await manager.createQueryBuilder().from(DiscountRule, "dr").getCount();
        ...
        while (offset < discountRuleCount) {
            ...
            const discountConditionRepo = manager.getCustomRepository(DiscountConditionRepository);
            await manager
                .createQueryBuilder()
                .insert()
        ...
        const discountRuleProductCount = await manager.query("SELECT COUNT(*) FROM discount_rule_products");
    ...
    // Validate results
    const noDanglingProductsValidation = await connection.manager.query(`SELECT drp.discount_rule_id, drp.product_id, dcp.product_id FROM "discount_rule_products" drp
  ...
  WHERE dcp.product_id IS NULL`);
    ...
    if (noDanglingProductsValidation && noDanglingProductsValidation?.length === 0) {
