...
const migrate = async function ({ typeormConfig }) {
    ...
    await connection.transaction(async (manager) => {
        const getDiscountableLineItems = (qb: SelectQueryBuilder<any>) => {
            return qb
                .from(LineItem, "li")
                .select(["li.order_id as order_id", "SUM(li.quantity * li.unit_price)::bigint AS discountable_subtotal"])
        ...
        while (offset < totalAdjustments) {
            const lineItemAdjustments = await manager
                .createQueryBuilder()
                .from(getLineItemAllocations, "lia")
                .select(["lia.id AS item_id", "lia.discount_id", "lia.discount_total AS amount", "'discount' AS description"])
                .limit(BATCH_SIZE)
                .offset(offset)
                .getRawMany();
