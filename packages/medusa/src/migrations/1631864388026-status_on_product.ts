...
export class statusOnProduct1631864388026 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "product_status_enum" AS ENUM('draft', 'proposed', 'published', 'rejected')`);
        await queryRunner.query(`ALTER TABLE "product" ADD "status" "product_status_enum" `);
        await queryRunner.query(`UPDATE "product" SET "status" = 'published' WHERE "status" IS NULL`);
