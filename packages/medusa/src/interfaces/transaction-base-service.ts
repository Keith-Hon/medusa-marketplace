...
export abstract class TransactionBaseService<TChild extends TransactionBaseService<TChild, TContainer>, TContainer = unknown> {
    protected abstract manager_: EntityManager;
    protected abstract transactionManager_: EntityManager | undefined;

    protected constructor(protected readonly container: TContainer, protected readonly configModule?: Record<string, unknown>) {}
    ...
    protected async atomicPhase_<TResult, TError>(
        ...
        let isolation: IsolationLevel | ((error: TError) => Promise<never | TResult | void>) | undefined | null = isolationOrErrorHandler;
        let dontFail = false;
            ...
            if (isolation && this.manager_) {
                ...
                try {
                    ...
                    if (this.shouldRetryTransaction_(error)) {
                        return this.manager_.transaction(isolation as IsolationLevel, (m): Promise<never | TResult> => doWork(m));
                    } else {
                        if (errorHandler) {
                            await errorHandler(error);
                        }
