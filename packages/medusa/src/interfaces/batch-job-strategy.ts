...
export abstract class AbstractBatchJobStrategy<T extends TransactionBaseService<never, TContainer>, TContainer = unknown>
    ...
    implements IBatchJobStrategy<T>
{
    ...
    protected async handleProcessingError<T>(batchJobId: string, err: unknown, result: T): Promise<void> {
        return await this.atomicPhase_(async (transactionManager) => {
            ...
            const errMessage = (err as { message: string }).message ?? `Something went wrong with the batchJob ${batchJob.id}`;
            const errCode = (err as { code: string | number }).code ?? "unknown";
            const resultError = { message: errMessage, code: errCode, err };

            if (shouldRetry) {
                const existingErrors = batchJob?.result?.errors ?? ([] as BatchJobResultError[]);
                ...
                await this.batchJobService_.withTransaction(transactionManager).setFailed(batchJob, resultError);
            }
        });
    }
}

export function isBatchJobStrategy(object: unknown): object is IBatchJobStrategy<never> {
