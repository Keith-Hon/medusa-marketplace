...
class ReturnService extends BaseService {
    ...
    async cancel(returnId) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (ret.status === "received") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't cancel a return which has been returned");
            }
    ...
    validateReturnStatuses_(order) {
        if (order.fulfillment_status === "not_fulfilled" || order.fulfillment_status === "returned") {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't return an unfulfilled or already returned order");
        }

        if (order.payment_status !== "captured") {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't return an order with payment unprocessed");
        }
    }
    ...
    validateReturnLineItem_(item, quantity, additional) {
        if (!item) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Return contains invalid line item");
        }

        const returnable = item.quantity - item.returned_quantity;
        if (quantity > returnable) {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot return more items than have been purchased");
        }
    ...
    async retrieve(id, config = {}) {
        const returnRepository = this.manager_.getCustomRepository(this.returnRepository_);
        ...
        if (!returnObj) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Return with id: ${id} was not found`);
        }
        return returnObj;
    }
    ...
    async retrieveBySwap(swapId, relations = []) {
        ...
        if (!returnObj) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Return with swa_id: ${swapId} was not found`);
        }
        return returnObj;
    }
    ...
    async update(returnId, update) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (ret.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot update a canceled return");
            }
    ...
    async create(data) {
        return this.atomicPhase_(async (manager) => {
            const returnRepository = manager.getCustomRepository(this.returnRepository_);
            ...
            for (const item of data.items) {
                ...
                if (line.order?.canceled_at || line.swap?.canceled_at || line.claim_order?.canceled_at) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Cannot create a return for a canceled item.`);
                }
            }

            const order = await this.orderService_.withTransaction(manager).retrieve(orderId, {
            ...
            const returnLines = await this.getFulfillmentItems_(order, data.items, this.validateReturnLineItem_);
            ...
            if (typeof toRefund !== "undefined") {
                ...
                if (toRefund > refundable) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, "Cannot refund more than the original payment");
                }
            } else {
            ...
            const returnReasons = await this.returnReasonService_.list({ id: [...returnLines.map((rl) => rl.reason_id)] }, { relations: ["return_reason_children"] });
            ...
            if (method) {
                const shippingMethod = await this.shippingOptionService_.withTransaction(manager).createShippingMethod(
                    method.option_id,
                    {},
                    {
                        price: method.price,
                ...
                const calculationContext = this.totalsService_.getCalculationContext(order);

                const taxLines = await this.taxProviderService_.withTransaction(manager).createShippingTaxLines(shippingMethod, calculationContext);

                const shippingTotal = shippingMethod.price + taxLines.reduce((acc, tl) => acc + Math.round(shippingMethod.price * (tl.rate / 100)), 0);
    ...
    fulfill(returnId) {
        return this.atomicPhase_(async (manager) => {
            const returnOrder = await this.retrieve(returnId, {
                relations: ["items", "shipping_method", "shipping_method.tax_lines", "shipping_method.shipping_option", "swap", "claim_order"]
            });

            if (returnOrder.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot fulfill a canceled return");
            }
            ...
            if (returnOrder.shipping_data) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Return has already been fulfilled");
            }
            ...
            const fulfillmentData = await this.fulfillmentProviderService_.createReturn(returnData);
    ...
    async receive(return_id, received_items, refund_amount, allow_mismatch = false) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (returnObj.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot receive a canceled return");
            }
            ...
            if (returnObj.status === "received") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `Return with id ${return_id} has already been received`);
            }

            const returnLines = await this.getFulfillmentItems_(order, received_items, this.validateReturnLineItem_);
            ...
            for (const i of returnObj.items) {
                const lineItem = await this.lineItemService_.withTransaction(manager).retrieve(i.item_id);
            ...
            for (const line of newLines) {
                ...
                if (orderItem) {
                    await this.inventoryService_.withTransaction(manager).adjustInventory(orderItem.variant_id, line.received_quantity);
                }
            }
