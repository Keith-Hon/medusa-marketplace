...
class StoreService extends TransactionBaseService<StoreService> {
    ...
    async create(): Promise<Store> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const storeRepository = transactionManager.getCustomRepository(this.storeRepository_);
    ...
    async update(data: UpdateStoreInput): Promise<Store> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (storeCurrencies) {
                ...
                if (!hasDefCurrency) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `You are not allowed to remove default currency from store currencies without replacing it as well`);
                }
                ...
                store.currencies = await Promise.all(
                    storeCurrencies.map(async (curr) => {
                        ...
                        if (!currency) {
                            throw new MedusaError(MedusaError.Types.INVALID_DATA, `Currency with code ${curr} does not exist`);
                        }
            ...
            if (default_currency_code) {
                const hasDefCurrency = store.currencies.find((c) => c.code.toLowerCase() === default_currency_code.toLowerCase());

                if (!hasDefCurrency) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Store does not have currency: ${default_currency_code}`);
                }
    ...
    async addCurrency(code: string): Promise<Store | never> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const storeRepo = transactionManager.getCustomRepository(this.storeRepository_);
            const currencyRepository = transactionManager.getCustomRepository(this.currencyRepository_);
            ...
            if (!curr) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `Currency ${code} not found`);
            }

            const store = await this.retrieve({ relations: ["currencies"] });

            const doesStoreInclCurrency = store.currencies.map((c) => c.code.toLowerCase()).includes(curr.code.toLowerCase());
            if (doesStoreInclCurrency) {
                throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, `Currency already added`);
            }
