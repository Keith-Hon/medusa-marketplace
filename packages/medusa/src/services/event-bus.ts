...
export default class EventBusService {
    ...
    constructor({ manager, logger, stagedJobRepository, redisClient, redisSubscriber }: InjectedDependencies, config: ConfigModule, singleton = true) {
        const opts = {
     ...
     */
    async emit<T>(eventName: string, data: T, options: { delay?: number } = {}): Promise<StagedJob | void> {
    ...
    async enqueuer_(): Promise<void> {
        while (this.shouldEnqueuerRun) {
            ...
            const stagedJobRepo = this.manager_.getCustomRepository(this.stagedJobRepository_);
            const jobs = await stagedJobRepo.find(listConfig);

            await Promise.all(
                jobs.map((job) => {
                    this.queue_.add({ eventName: job.event_name, data: job.data }, { removeOnComplete: true }).then(async () => {
     ...
     */
    worker_ = async <T>(job: { data: { eventName: string; data: T } }): Promise<unknown[]> => {
        ...
        this.logger_.info(`Processing ${eventName} which has ${eventObservers.length} subscribers`);
     ...
     * @return resolves to the results of the subscriber calls.
     */
    cronWorker_ = async <T>(job: { data: { eventName: string; data: T } }): Promise<unknown[]> => {
        const { eventName, data } = job.data;
        ...
        return await Promise.all(
            observers.map((subscriber) => {
                return subscriber(data, eventName).catch((err) => {
                    this.logger_.warn(`An error occured while processing ${eventName}: ${err}`);
                    return err;
                });
            })
        );
