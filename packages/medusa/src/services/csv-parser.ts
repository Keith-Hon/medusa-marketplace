...
class CsvParser<TSchema extends CsvSchema<TParserResult, TOutputResult> = CsvSchema, TParserResult = unknown, TOutputResult = unknown> extends AbstractParser<
    TSchema,
    TParserResult,
    ParseConfig,
    TOutputResult
> {
    ...
    constructor(protected readonly container: AwilixContainer, schema: TSchema, delimiter?: string) {
        super(schema);
        if (delimiter) {
    ...
    public async parse(readableStream: NodeJS.ReadableStream, options: ParseConfig = DEFAULT_PARSE_OPTIONS): Promise<TParserResult[]> {
    ...
    private async _buildLine(line: TParserResult, lineNumber: number): Promise<TOutputResult> {
        ...
        for (const tupleKey of tupleKeys) {
             ...
             */
            if (!column) {
                throw new Error(`Unable to treat column ${tupleKey} from the csv file. No target column found in the provided schema`);
            }
            ...
            if (!line[tupleKey] && column.required) {
                throw new Error(`No value found for target column "${column.name}" in line ${lineNumber} of the given csv file`);
            }
         ...
         */
        const missingColumns = difference(Object.keys(columnMap), Object.keys(processedColumns));

        if (missingColumns.length > 0) {
            throw new Error(`Missing column(s) ${formatMissingColumns(missingColumns)} from the given csv file`);
        }
    ...
    private buildColumnMap_(columns: TSchema["columns"]): Record<string, TSchema["columns"][number]> {
    ...
    private resolveColumn_(tupleKey: string, columnMap: Record<string, TSchema["columns"][number]>): TSchema["columns"][number] | undefined {
        if (columnMap[tupleKey]) {
            return columnMap[tupleKey];
        }

        const matchedColumn = this.$$schema.columns.find((column) =>
            "match" in column && typeof column.match === "object" && column.match instanceof RegExp ? column.match.test(tupleKey) : false
        );

        return matchedColumn;
    }

    private resolveTuple_(tuple: TOutputResult, column: TSchema["columns"][number], context: CsvParserContext<TParserResult> & { tupleKey: string }): TOutputResult {
        ...
        const resolvedValue = column.transform ? column.transform(line[tupleKey], csvContext) : line[tupleKey];
...
const formatMissingColumns = (list: string[]): string => list.reduce((text, curr, i, array) => text + (i < array.length - 1 ? `"${curr}", ` : `"${curr}"`), "");
