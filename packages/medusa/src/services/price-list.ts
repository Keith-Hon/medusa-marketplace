...
import { PriceListFindOptions, PriceListRepository } from "../repositories/price-list";
...
class PriceListService extends TransactionBaseService<PriceListService> {
     ...
     */
    async retrieve(priceListId: string, config: FindConfig<PriceList> = {}): Promise<PriceList> {
        ...
        if (!priceList) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Price list with id: ${priceListId} was not found`);
        }
     ...
     * @returns {Promise<PriceList>} updated Price List
     */
    async addPrices(id: string, prices: PriceListPriceCreateInput[], replace = false): Promise<PriceList> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const moneyAmountRepo = manager.getCustomRepository(this.moneyAmountRepo_);
     ...
     */
    async list(selector: FilterablePriceListProps = {}, config: FindConfig<FilterablePriceListProps> = { skip: 0, take: 20 }): Promise<PriceList[]> {
    ...
    async listAndCount(
        ...
        const priceListRepo = manager.getCustomRepository(this.priceListRepo_);
        const { q, ...priceListSelector } = selector;
        const { relations, ...query } = buildQuery<FilterablePriceListProps, FilterablePriceListProps>(priceListSelector, config);

        const groups = query.where.customer_groups as FindOperator<string[]>;
        delete query.where.customer_groups;

        if (q) {
            return await priceListRepo.getFreeTextSearchResultsAndCount(q, query as PriceListFindOptions, groups, relations);
        }
        return await priceListRepo.listAndCount({ ...query, relations }, groups);
    }

    protected async upsertCustomerGroups_(priceListId: string, customerGroups: { id: string }[]): Promise<void> {
        const priceListRepo = this.manager_.getCustomRepository(this.priceListRepo_);
    ...
    async listProducts(
        ...
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const productVariantRepo = manager.getCustomRepository(this.productVariantRepo_);
            const [products, count] = await this.productService_.withTransaction(manager).listAndCount(selector, config);
                    ...
                    if (p.variants?.length) {
                        p.variants = await Promise.all(
                            p.variants.map(async (v) => {
                                const [prices] = await moneyAmountRepo.findManyForVariantInPriceList(v.id, priceListId, requiresPriceList);
    ...
    async listVariants(
        ...
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const [variants, count] = await this.variantService_.withTransaction(manager).listAndCount(selector, config);

            const moneyAmountRepo = manager.getCustomRepository(this.moneyAmountRepo_);

            const variantsWithPrices = await Promise.all(
                variants.map(async (variant) => {
                    const [prices] = await moneyAmountRepo.findManyForVariantInPriceList(variant.id, priceListId, requiresPriceList);
    ...
    public async deleteProductPrices(priceListId: string, productIds: string[]): Promise<[string[], number]> {
        return await this.atomicPhase_(async () => {
            ...
            const priceIds = products.map(({ variants }) => variants.map((variant) => variant.prices.map((price) => price.id)).flat()).flat();
    ...
    public async deleteVariantPrices(priceListId: string, variantIds: string[]): Promise<[string[], number]> {
        return await this.atomicPhase_(async () => {
            ...
            const priceIds = variants.map((variant) => variant.prices.map((price) => price.id)).flat();
     ...
     */
    protected async addCurrencyFromRegion<T extends PriceListPriceUpdateInput | PriceListPriceCreateInput>(prices: T[]): Promise<T[]> {
        const prices_: typeof prices = [];

        for (const p of prices) {
            if (p.region_id) {
                const region = await this.regionService_.withTransaction(this.manager_).retrieve(p.region_id);
