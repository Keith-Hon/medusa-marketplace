...
import { ExtendedFindConfig, FindConfig, QuerySelector, Selector } from "../types/common";
...
class GiftCardService extends TransactionBaseService<GiftCardService> {
     ...
     */
    async listAndCount(selector: QuerySelector<GiftCard> = {}, config: FindConfig<GiftCard> = { relations: [], skip: 0, take: 10 }): Promise<[GiftCard[], number]> {
        ...
        const query: ExtendedFindConfig<GiftCard, QuerySelector<GiftCard>> = buildQuery<QuerySelector<GiftCard>, GiftCard>(selector, config);
     ...
     * @return the result of the find operation
     */
    async list(selector: QuerySelector<GiftCard> = {}, config: FindConfig<GiftCard> = { relations: [], skip: 0, take: 10 }): Promise<GiftCard[]> {
        const manager = this.manager_;
        const giftCardRepo = manager.getCustomRepository(this.giftCardRepository_);
    ...
    async createTransaction(data: CreateGiftCardTransactionInput): Promise<string> {
        const manager = this.manager_;
    ...
    async create(giftCard: CreateGiftCardInput): Promise<GiftCard> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const region = await this.regionService_.withTransaction(manager).retrieve(giftCard.region_id);
            ...
            await this.eventBus_.withTransaction(manager).emit(GiftCardService.Events.CREATED, {
                id: result.id
            });
    ...
    protected async retrieve_(selector: Selector<GiftCard>, config: FindConfig<GiftCard> = {}): Promise<GiftCard> {
        const manager = this.manager_;
        const giftCardRepo = manager.getCustomRepository(this.giftCardRepository_);

        const { relations, ...query } = buildQuery(selector, config);

        const giftCard = await giftCardRepo.findOneWithRelations(relations as (keyof GiftCard)[], query);
        ...
        if (!giftCard) {
            ...
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Gift card with ${selectorConstraints} was not found`);
        }
     ...
     */
    async retrieve(giftCardId: string, config: FindConfig<GiftCard> = {}): Promise<GiftCard> {
        return await this.retrieve_({ id: giftCardId }, config);
    }

    async retrieveByCode(code: string, config: FindConfig<GiftCard> = {}): Promise<GiftCard> {
     ...
     */
    async update(giftCardId: string, update: UpdateGiftCardInput): Promise<GiftCard> {
        return await this.atomicPhase_(async (manager) => {
            const giftCardRepo = manager.getCustomRepository(this.giftCardRepository_);
            ...
            if (region_id && region_id !== giftCard.region_id) {
                const region = await this.regionService_.withTransaction(manager).retrieve(region_id);
            ...
            if (typeof balance !== "undefined") {
                if (balance < 0 || giftCard.value < balance) {
                    throw new MedusaError(MedusaError.Types.INVALID_ARGUMENT, "new balance is invalid");
                }
