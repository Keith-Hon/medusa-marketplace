...
class ShippingOptionService extends TransactionBaseService<ShippingOptionService> {
     ...
     */
    async validateRequirement_(requirement: ShippingOptionRequirement, optionId: string | undefined = undefined): Promise<ShippingOptionRequirement> {
        return await this.atomicPhase_(async (manager) => {
            if (!requirement.type) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "A Shipping Requirement must have a type field");
            }

            if (requirement.type !== "min_subtotal" && requirement.type !== "max_subtotal") {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Requirement type must be one of min_subtotal, max_subtotal");
            }
     ...
     */
    async list(selector: Selector<ShippingMethod>, config: FindConfig<ShippingOption> = { skip: 0, take: 50 }): Promise<ShippingOption[]> {
     ...
     * @return {Promise} the result of the find operation
     */
    async listAndCount(selector: Selector<ShippingMethod>, config: FindConfig<ShippingOption> = { skip: 0, take: 50 }): Promise<[ShippingOption[], number]> {
        const manager = this.manager_;
        const optRepo = manager.getCustomRepository(this.optionRepository_);
     ...
     */
    async retrieve(optionId, options: { select?: (keyof ShippingOption)[]; relations?: string[] } = {}): Promise<ShippingOption> {
        const manager = this.manager_;
        const soRepo: ShippingOptionRepository = manager.getCustomRepository(this.optionRepository_);
        ...
        if (!option) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Shipping Option with ${optionId} was not found`);
        }
     ...
     */
    async updateShippingMethod(id: string, update: ShippingMethodUpdate): Promise<ShippingMethod | undefined> {
        return await this.atomicPhase_(async (manager) => {
            const methodRepo: ShippingMethodRepository = manager.getCustomRepository(this.methodRepository_);
            ...
            for (const key of Object.keys(update).filter((k) => typeof update[k] !== `undefined`)) {
     ...
     */
    async deleteShippingMethods(shippingMethods: ShippingMethod | ShippingMethod[]): Promise<ShippingMethod[]> {
        const removeEntities: ShippingMethod[] = Array.isArray(shippingMethods) ? shippingMethods : [shippingMethods];
     ...
     */
    async createShippingMethod(optionId: string, data: object, config: CreateShippingMethodDto): Promise<ShippingMethod> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const validatedData = await this.providerService_.validateFulfillmentData(option, data, config.cart || {});
    ...
    async validateCartOption(option: ShippingOption, cart: Cart): Promise<ShippingOption | null> {
        ...
        if (cart.region_id !== option.region_id) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "The shipping option is not available in the cart's region");
        }

        const subtotal = cart.subtotal as number;

        const requirementResults: boolean[] = option.requirements.map((requirement) => {
        ...
        if (!requirementResults.every(Boolean)) {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "The Cart does not satisfy the shipping option's requirements");
        }
    ...
    async create(data: CreateShippingOptionInput): Promise<ShippingOption> {
        return this.atomicPhase_(async (manager) => {
            ...
            const option = await optionRepo.create(data as DeepPartial<ShippingOption>);

            const region = await this.regionService_.withTransaction(manager).retrieve(option.region_id, {
                relations: ["fulfillment_providers"]
            });

            if (!region.fulfillment_providers.find(({ id }) => id === option.provider_id)) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "The fulfillment provider is not available in the provided region");
            }

            option.price_type = await this.validatePriceType_(data.price_type, option);
            option.amount = data.price_type === "calculated" ? null : data.amount ?? null;

            const isValid = await this.providerService_.validateOption(option);

            if (!isValid) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "The fulfillment provider cannot validate the shipping option");
            }
            ...
            if (typeof data.requirements !== "undefined") {
                ...
                for (const r of data.requirements) {
                    ...
                    if (acc.find((raw) => raw.type === validated.type)) {
                        throw new MedusaError(MedusaError.Types.INVALID_DATA, "Only one requirement of each type is allowed");
                    }

                    if (acc.find((raw) => (raw.type === "max_subtotal" && validated.amount > raw.amount) || (raw.type === "min_subtotal" && validated.amount < raw.amount))) {
                        throw new MedusaError(MedusaError.Types.INVALID_DATA, "Max. subtotal must be greater than Min. subtotal");
                    }
     ...
     */
    async validatePriceType_(priceType: ShippingOptionPriceType, option: ShippingOption): Promise<ShippingOptionPriceType> {
        if (!priceType || (priceType !== "flat_rate" && priceType !== "calculated")) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "The price must be of type flat_rate or calculated");
        }

        if (priceType === "calculated") {
            const canCalculate = await this.providerService_.canCalculate(option);
            if (!canCalculate) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "The fulfillment provider cannot calculate prices for this option");
            }
        }
     ...
     */
    async update(optionId: string, update: UpdateShippingOptionInput): Promise<ShippingOption> {
        return this.atomicPhase_(async (manager) => {
            const option = await this.retrieve(optionId, {
                relations: ["requirements"]
            });
            ...
            if (update.region_id || update.provider_id || update.data) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Region and Provider cannot be updated after creation");
            }

            if (typeof update.is_return !== "undefined") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "is_return cannot be changed after creation");
            }
            ...
            if (typeof update.requirements !== "undefined") {
                ...
                if (option.requirements) {
                    ...
                    const toRemove = option.requirements.filter((r) => !accReqs.includes(r.id));
                    await Promise.all(
            ...
            if (typeof update.price_type !== "undefined") {
                option.price_type = await this.validatePriceType_(update.price_type, option);
                if (update.price_type === "calculated") {
                    option.amount = null;
                }
            }

            if (typeof update.amount !== "undefined" && option.price_type !== "calculated") {
     ...
     */
    async addRequirement(optionId: string, requirement: ShippingOptionRequirement): Promise<ShippingOption> {
        return this.atomicPhase_(async (manager) => {
            const option = await this.retrieve(optionId, {
                relations: ["requirements"]
            });
            ...
            if (option.requirements.find((r) => r.type === validatedReq.type)) {
                throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, `A requirement with type: ${validatedReq.type} already exists`);
            }
     ...
     * @return {Promise} the result of update
     */
    async removeRequirement(requirementId): Promise<ShippingOptionRequirement | void> {
        return await this.atomicPhase_(async (manager) => {
            const reqRepo: ShippingOptionRequirementRepository = manager.getCustomRepository(this.requirementRepository_);
     ...
     */
    async getPrice_(option: ShippingOption, data: object, cart: Cart | Order | undefined): Promise<number> {
