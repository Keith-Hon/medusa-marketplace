...
class SwapService extends BaseService {
    ...
    async retrieve(id, config = {}) {
        ...
        const { cartSelects, cartRelations, ...newConfig } = this.transformQueryForCart_(config);
        ...
        if (cartRelations || cartSelects) {
            const cart = await this.cartService_.withTransaction(this.manager_).retrieve(swap.cart_id, {
                select: cartSelects,
     ...
     */
    list(selector, config = { skip: 0, take: 50, order: { created_at: "DESC" } }) {
        const swapRepo = this.manager_.getCustomRepository(this.swapRepository_);
    ...
    validateReturnItems_(order, returnItems) {
        return returnItems.map(({ item_id, quantity }) => {
            ...
            if (!item) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Item does not exist on order");
            }

            // Item's cannot be returned multiple times
            if (item.quantity < item.returned_quantity + quantity) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Cannot return more items than have been ordered");
            }
    ...
    async create(
        ...
        return this.atomicPhase_(async (manager) => {
            if (order.fulfillment_status === "not_fulfilled" || order.payment_status !== "captured") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Order cannot be swapped");
            }
            ...
            for (const item of returnItems) {
                ...
                if (line.order?.canceled_at || line.swap?.canceled_at || line.claim_order?.canceled_at) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Cannot create a swap on a canceled item.`);
                }
            }

            const newItems = await Promise.all(
                additionalItems.map(({ variant_id, quantity }) => {
                    return this.lineItemService_.generate(variant_id, order.region_id, quantity);
                })
            );
            ...
            await this.eventBus_.withTransaction(manager).emit(SwapService.Events.CREATED, {
                id: result.id,
                no_notification: evaluatedNoNotification
            });
    ...
    async processDifference(swapId) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (swap.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Canceled swap cannot be processed");
            }

            if (!swap.confirmed_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot process a swap that hasn't been confirmed by the customer");
            }
            ...
            if (swap.difference_due < 0) {
                ...
                try {
                    await this.paymentProviderService_.withTransaction(manager).refundPayment(swap.order.payments, -1 * swap.difference_due, "swap");
                } catch (err) {
                    swap.payment_status = "requires_action";
                    const result = await swapRepo.save(swap);

                    await this.eventBus_.withTransaction(manager).emit(SwapService.Events.PROCESS_REFUND_FAILED, {
                        id: result.id,
                ...
                await this.eventBus_.withTransaction(manager).emit(SwapService.Events.REFUND_PROCESSED, {
                    id: result.id,
                    no_notification: swap.no_notification
                });
            ...
            try {
                ...
                await this.paymentProviderService_.withTransaction(manager).capturePayment(swap.payment);
            } catch (err) {
                swap.payment_status = "requires_action";
                const result = await swapRepo.save(swap);

                await this.eventBus_.withTransaction(manager).emit(SwapService.Events.PAYMENT_CAPTURE_FAILED, {
                    id: swap.id,
                    no_notification: swap.no_notification
                });
            ...
            await this.eventBus_.withTransaction(manager).emit(SwapService.Events.PAYMENT_CAPTURED, {
                id: result.id,
                no_notification: swap.no_notification
            });
    ...
    async createCart(swapId, customShippingOptions = []) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (swap.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Canceled swap cannot be used to create a cart");
            }

            if (swap.cart_id) {
                throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, "A cart has already been created for the swap");
            }

            const order = swap.order;

            // filter out free shipping discounts
            const discounts = order?.discounts?.filter(({ rule }) => rule.type !== "free_shipping") || undefined;
            ...
            for (const customShippingOption of customShippingOptions) {
                await this.customShippingOptionService_.withTransaction(manager).create({
                    cart_id: cart.id,
            ...
            for (const item of swap.additional_items) {
                ...
                await this.lineItemAdjustmentService_.withTransaction(manager).createAdjustmentForLineItem(cart, item);
            }
            ...
            if (swap.return_order && swap.return_order.shipping_method) {
                const shippingTaxLineRepo = this.manager_.getCustomRepository(this.shippingTaxLineRepo_);
            ...
            await this.lineItemService_.withTransaction(manager).createReturnLines(swap.return_order.id, cart.id);
    ...
    async registerCartCompletion(swapId) {
        return await this.atomicPhase_(async (manager) => {
            const swap = await this.retrieve(swapId, {
                select: ["id", "order_id", "no_notification", "allow_backorder", "canceled_at", "confirmed_at", "cart_id"]
            });
            ...
            if (swap.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cart related to canceled swap cannot be completed");
            }
            ...
            if (!swap.allow_backorder) {
                for (const item of items) {
                    try {
                        await this.inventoryService_.withTransaction(manager).confirmInventory(item.variant_id, item.quantity);
                    } catch (err) {
                        if (payment) {
                            await this.paymentProviderService_.withTransaction(manager).cancelPayment(payment);
                        }
                        await this.cartService_.withTransaction(manager).update(cart.id, { payment_authorized_at: null });
                        throw err;
                    }
                }
            }
            ...
            if (total > 0) {
                if (!payment) {
                    throw new MedusaError(MedusaError.Types.INVALID_ARGUMENT, "Cart does not contain a payment");
                }
            ...
            for (const method of cart.shipping_methods) {
                await this.shippingOptionService_.withTransaction(manager).updateShippingMethod(method.id, {
                    swap_id: result.id
                });
            }

            this.eventBus_.withTransaction(manager).emit(SwapService.Events.PAYMENT_COMPLETED, {
                id: swap.id,
                no_notification: swap.no_notification
            });

            await this.cartService_.withTransaction(manager).update(cart.id, { completed_at: new Date() });
    ...
    async cancel(swapId) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (swap.payment_status === "difference_refunded" || swap.payment_status === "partially_refunded" || swap.payment_status === "refunded") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Swap with a refund cannot be canceled");
            }

            if (swap.fulfillments) {
                for (const f of swap.fulfillments) {
                    if (!f.canceled_at) {
                        throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "All fulfillments must be canceled before the swap can be canceled");
                    }
                }
            }

            if (swap.return_order && swap.return_order.status !== "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Return must be canceled before the swap can be canceled");
            }
            ...
            if (swap.payment) {
                await this.paymentProviderService_.withTransaction(manager).cancelPayment(swap.payment);
            }
    ...
    async createFulfillment(
        ...
        return this.atomicPhase_(async (manager) => {
            ...
            if (swap.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Canceled swap cannot be fulfilled");
            }

            if (swap.fulfillment_status !== "not_fulfilled" && swap.fulfillment_status !== "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "The swap was already fulfilled");
            }

            if (!swap.shipping_methods?.length) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot fulfill an swap that doesn't have shipping methods");
            }

            const evaluatedNoNotification = no_notification !== undefined ? no_notification : swap.no_notification;

            swap.fulfillments = await this.fulfillmentService_.withTransaction(manager).createFulfillment(
                {
                    ...swap,
    ...
    async cancelFulfillment(fulfillmentId) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (!canceled.swap_id) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `Fufillment not related to a swap`);
            }
    ...
    async createShipment(
        ...
        return this.atomicPhase_(async (manager) => {
            ...
            if (swap.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Canceled swap cannot be fulfilled as shipped");
            }
            const evaluatedNoNotification = no_notification !== undefined ? no_notification : swap.no_notification;

            // Update the fulfillment to register
            const shipment = await this.fulfillmentService_.withTransaction(manager).createShipment(fulfillmentId, trackingLinks, {
                metadata,
                no_notification: evaluatedNoNotification
            });
            ...
            const swapRepo = manager.getCustomRepository(this.swapRepository_);
            const result = await swapRepo.save(swap);
            await this.eventBus_.withTransaction(manager).emit(SwapService.Events.SHIPMENT_CREATED, {
                id: swapId,
    ...
    async deleteMetadata(swapId, key) {
        ...
        return this.swapModel_.updateOne({ _id: validatedId }, { $unset: { [keyPath]: "" } }).catch((err) => {
    ...
    async registerReceived(id) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (swap.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Canceled swap cannot be registered as received");
            }

            if (swap.return_order.status !== "received") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Swap is not received");
            }

            const result = await this.retrieve(id);

            await this.eventBus_.withTransaction(manager).emit(SwapService.Events.RECEIVED, {
                id: id,
