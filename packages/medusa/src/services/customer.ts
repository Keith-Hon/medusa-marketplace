...
class CustomerService extends TransactionBaseService<CustomerService> {
    ...
    async generateResetPasswordToken(customerId: string): Promise<string> {
        return await this.atomicPhase_(async (manager) => {
            const customer = await this.retrieve(customerId, {
                select: ["id", "has_account", "password_hash", "email", "first_name", "last_name"]
            });

            if (!customer.has_account) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "You must have an account to reset the password. Create an account first");
            }
            ...
            // Notify subscribers
            this.eventBusService_.withTransaction(manager).emit(CustomerService.Events.PASSWORD_RESET, {
                id: customerId,
     ...
     */
    async list(selector: Selector<Customer> & { q?: string } = {}, config: FindConfig<Customer> = { relations: [], skip: 0, take: 50 }): Promise<Customer[]> {
    ...
    private async retrieve_(selector: Selector<Customer>, config: FindConfig<Customer> = {}): Promise<Customer | never> {
        ...
        if (!customer) {
            const selectorConstraints = Object.entries(selector)
                ...
                .join(", ");
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Customer with ${selectorConstraints} was not found`);
        }
     ...
     */
    async retrieveByEmail(email: string, config: FindConfig<Customer> = {}): Promise<Customer | never> {
     ...
     * @return {Promise<Customer>} the customer document.
     */
    async retrieveByPhone(phone: string, config: FindConfig<Customer> = {}): Promise<Customer | never> {
    ...
    async create(customer: CreateCustomerInput): Promise<Customer> {
        return await this.atomicPhase_(async (manager) => {
            const customerRepository = manager.getCustomRepository(this.customerRepository_);
            ...
            if (existing && password && !existing.has_account) {
                ...
                await this.eventBusService_.withTransaction(manager).emit(CustomerService.Events.UPDATED, updated);
                return updated;
            } else {
                if (password) {
                    const hashedPassword = await this.hashPassword_(password);
    ...
    async update(customerId: string, update: UpdateCustomerInput): Promise<Customer> {
        return await this.atomicPhase_(
            async (manager) => {
                ...
                const { password, metadata, billing_address, billing_address_id, groups, ...rest } = update;
     ...
     */
    async updateBillingAddress_(customer: Customer, addressOrId: string | DeepPartial<Address> | undefined): Promise<void> {
        return await this.atomicPhase_(async (manager) => {
            const addrRepo: AddressRepository = manager.getCustomRepository(this.addressRepository_);
            ...
            if (typeof addressOrId === `string`) {
                ...
                if (!fetchedAddress) {
                    throw new MedusaError(MedusaError.Types.NOT_FOUND, `Address with id ${addressOrId} was not found`);
                }
    ...
    async updateAddress(customerId: string, addressId: string, address: StorePostCustomersCustomerAddressesAddressReq): Promise<Address> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (!toUpdate) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Could not find address for customer");
            }
    ...
    async addAddress(customerId: string, address: AddressCreatePayload): Promise<Customer | Address> {
        return await this.atomicPhase_(async (manager) => {
            const addressRepository = manager.getCustomRepository(this.addressRepository_);
