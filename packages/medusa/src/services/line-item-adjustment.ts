...
class LineItemAdjustmentService extends BaseService {
     ...
     */
    async retrieve(id: string, config: FindConfig<LineItemAdjustment> = {}): Promise<LineItemAdjustment> {
        const lineItemAdjustmentRepo: LineItemAdjustmentRepository = this.manager_.getCustomRepository(this.lineItemAdjustmentRepo_);
        ...
        if (!lineItemAdjustment) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Line item adjustment with id: ${id} was not found`);
        }
    ...
    async create(data: Partial<LineItemAdjustment>): Promise<LineItemAdjustment> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const lineItemAdjustmentRepo: LineItemAdjustmentRepository = manager.getCustomRepository(this.lineItemAdjustmentRepo_);
     ...
     */
    async list(selector: FilterableLineItemAdjustmentProps = {}, config: FindConfig<LineItemAdjustment> = { skip: 0, take: 20 }): Promise<LineItemAdjustment[]> {
     ...
     */
    async generateAdjustments(cart: Cart, generatedLineItem: LineItem, context: AdjustmentContext): Promise<GeneratedAdjustment[]> {
        return this.atomicPhase_(async (manager) => {
            // if lineItem should not be discounted
            // or lineItem is a return line item
            // or the cart does not have any discounts
            // then do nothing
            if (!generatedLineItem.allow_discounts || generatedLineItem.is_return || !cart?.discounts?.length) {
                return [];
            }
            ...
            const isValid = await this.discountService.withTransaction(manager).validateDiscountForProduct(discount.rule_id, lineItemProduct);
            ...
            const amount = await this.discountService.calculateDiscountForLineItem(discount.id, generatedLineItem, cart);
     ...
     */
    async createAdjustments(cart: Cart, lineItem?: LineItem): Promise<LineItemAdjustment[] | LineItemAdjustment[][]> {
        if (lineItem) {
