...
class NotificationService extends TransactionBaseService<NotificationService> {
    ...
    constructor(container: InjectedDependencies) {
        ...
        const { manager, notificationProviderRepository, notificationRepository, logger } = container;
    ...
    async list(
        ...
        const notiRepo = this.manager_.getCustomRepository(this.notificationRepository_);
     ...
     */
    async retrieve(id: string, config: FindConfig<Notification> = {}): Promise<Notification | never> {
        const notiRepository = this.manager_.getCustomRepository(this.notificationRepository_);
        ...
        if (!notification) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Notification with id: ${id} was not found.`);
        }
    ...
    subscribe(eventName: string, providerId: string): void {
        if (typeof providerId !== "string") {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "providerId must be a string");
        }
    ...
    protected retrieveProvider_(id: string): AbstractNotificationService<never> {
        try {
            ...
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Could not find a notification provider with id: ${id}.`);
        }
    }
        ...
        return Promise.all(
            subs.map(async (providerId) => {
                return this.send(eventName, data, providerId).catch((err) => {
                    console.log(err);
                    this.logger_.warn(`An error occured while ${providerId} was processing a notification for ${eventName}: ${err.message}`);
                });
            })
        );
    }
     ...
     */
    async send(event: string, eventData: Record<string, unknown>, providerId: string): Promise<Notification | undefined> {
        return await this.atomicPhase_(async (transactionManager) => {
            const provider = this.retrieveProvider_(providerId);
            const result = await provider.sendNotification(event, eventData, this.attachmentGenerator_);
            ...
            const notiRepo = transactionManager.getCustomRepository(this.notificationRepository_);
     ...
     */
    async resend(id: string, config: FindConfig<Notification> = {}): Promise<Notification> {
        return await this.atomicPhase_(async (transactionManager) => {
            const notification = await this.retrieve(id);

            const provider = this.retrieveProvider_(notification.provider_id);
            const { to, data } = await provider.resendNotification(notification, config, this.attachmentGenerator_);
