...
class RegionService extends BaseService {
    ...
    async create(regionObject) {
        return this.atomicPhase_(async (manager) => {
            const regionRepository = manager.getCustomRepository(this.regionRepository_);
            const currencyRepository = manager.getCustomRepository(this.currencyRepository_);
            ...
            if (currency_code) {
                ...
                if (!currency) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Could not find currency with code ${currency_code}`);
                }
            ...
            await this.eventBus_.withTransaction(manager).emit(RegionService.Events.CREATED, {
                id: result.id
            });
    ...
    async update(regionId, update) {
        return this.atomicPhase_(async (manager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(RegionService.Events.UPDATED, {
                id: result.id,
    ...
    async validateFields_(region, id = undefined) {
        const ppRepository = this.manager_.getCustomRepository(this.paymentProviderRepository_);
        const fpRepository = this.manager_.getCustomRepository(this.fulfillmentProviderRepository_);
        const tpRepository = this.manager_.getCustomRepository(this.taxProviderRepository_);
        ...
        if (region.countries) {
            region.countries = await Promise.all(region.countries.map((countryCode) => this.validateCountry_(countryCode, id))).catch((err) => {
                throw err;
            });
        }
        ...
        if (region.tax_provider_id) {
            ...
            if (!tp) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Tax provider not found");
            }
        }
        ...
        if (region.payment_providers) {
            region.payment_providers = await Promise.all(
                region.payment_providers.map(async (pId) => {
                    ...
                    if (!pp) {
                        throw new MedusaError(MedusaError.Types.INVALID_DATA, "Payment provider not found");
                    }
        ...
        if (region.fulfillment_providers) {
            region.fulfillment_providers = await Promise.all(
                region.fulfillment_providers.map(async (fId) => {
                    ...
                    if (!fp) {
                        throw new MedusaError(MedusaError.Types.INVALID_DATA, "Fulfillment provider not found");
                    }
    ...
    validateTaxRate_(taxRate) {
        if (taxRate > 100 || taxRate < 0) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "The tax_rate must be between 0 and 1");
        }
    }
    ...
    async validateCurrency_(currencyCode) {
        const store = await this.storeService_.withTransaction(this.transactionManager_).retrieve({ relations: ["currencies"] });

        const storeCurrencies = store.currencies.map((curr) => curr.code);

        if (!storeCurrencies.includes(currencyCode.toLowerCase())) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Invalid currency code");
        }
    }
    ...
    async validateCountry_(code, regionId) {
        const countryRepository = this.manager_.getCustomRepository(this.countryRepository_);

        const countryCode = code.toUpperCase();
        const isCountryExists = countries.some((country) => country.alpha2 === countryCode);
        if (!isCountryExists) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Invalid country code");
        }
        ...
        if (!country) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, `Country with code ${code} not found`);
        }

        if (country.region_id && country.region_id !== regionId) {
            throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, `${country.display_name} already exists in region ${country.region_id}`);
        }
    ...
    async retrieveByCountryCode(code, config = {}) {
        ...
        if (!country.region_id) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, `Country does not belong to a region`);
        }
    ...
    async retrieve(regionId, config = {}) {
        const regionRepository = this.manager_.getCustomRepository(this.regionRepository_);
        ...
        if (!region) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Region with ${regionId} was not found`);
        }
        return region;
    }
    ...
    async delete(regionId) {
        return this.atomicPhase_(async (manager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(RegionService.Events.DELETED, {
                id: regionId
            });
    ...
    async addCountry(regionId, code) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (region.countries && region.countries.map((c) => c.iso_2).includes(country.iso_2)) {
                return region;
            }
    ...
    async removeCountry(regionId, code) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (region.countries && !region.countries.map((c) => c.iso_2).includes(code)) {
                return region;
            }

            region.countries = region.countries.filter((country) => country.iso_2 !== code);
    ...
    async addPaymentProvider(regionId, providerId) {
        return this.atomicPhase_(async (manager) => {
            const regionRepo = manager.getCustomRepository(this.regionRepository_);
            const ppRepo = manager.getCustomRepository(this.paymentProviderRepository_);
            ...
            if (!pp) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `Payment provider ${providerId} was not found`);
            }
    ...
    async addFulfillmentProvider(regionId, providerId) {
        return this.atomicPhase_(async (manager) => {
            const regionRepo = manager.getCustomRepository(this.regionRepository_);
            const fpRepo = manager.getCustomRepository(this.fulfillmentProviderRepository_);
            ...
            if (!fp) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `Fulfillment provider ${providerId} was not found`);
            }
    ...
    async removePaymentProvider(regionId, providerId) {
        return this.atomicPhase_(async (manager) => {
            ...
            region.payment_providers = region.payment_providers.filter(({ id }) => id !== providerId);
    ...
    async removeFulfillmentProvider(regionId, providerId) {
        return this.atomicPhase_(async (manager) => {
            ...
            region.fulfillment_providers = region.fulfillment_providers.filter(({ id }) => id !== providerId);
