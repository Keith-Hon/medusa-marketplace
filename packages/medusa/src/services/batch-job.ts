...
class BatchJobService extends TransactionBaseService<BatchJobService> {
    ...
    protected batchJobStatusMapToProps = new Map<BatchJobStatus, { entityColumnName: string; eventType: string }>([
        [
    ...
    async retrieve(batchJobId: string, config: FindConfig<BatchJob> = {}): Promise<BatchJob | never> {
        ...
        if (!batchJob) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Batch job with id ${batchJobId} was not found`);
        }

        return batchJob;
    }

    async listAndCount(selector: FilterableBatchJobProps = {}, config: FindConfig<BatchJob> = { skip: 0, take: 20 }): Promise<[BatchJob[], number]> {
        const manager = this.manager_;
        const batchJobRepo = manager.getCustomRepository(this.batchJobRepository_);
    ...
    async create(data: BatchJobCreateProps): Promise<BatchJob> {
        return await this.atomicPhase_(async (manager) => {
            const batchJobRepo: BatchJobRepository = manager.getCustomRepository(this.batchJobRepository_);

            const batchJob = batchJobRepo.create(data);
            const result = await batchJobRepo.save(batchJob);

            await this.eventBus_.withTransaction(manager).emit(BatchJobService.Events.CREATED, {
                id: result.id
            });
    ...
    async update(batchJobOrId: BatchJob | string, data: BatchJobUpdateProps): Promise<BatchJob> {
        return await this.atomicPhase_(async (manager) => {
            const batchJobRepo: BatchJobRepository = manager.getCustomRepository(this.batchJobRepository_);
            ...
            await this.eventBus_.withTransaction(manager).emit(BatchJobService.Events.UPDATED, {
                id: batchJob.id
            });
    ...
    protected async updateStatus(batchJobOrId: BatchJob | string, status: BatchJobStatus): Promise<BatchJob | never> {
        ...
        const { entityColumnName, eventType } = this.batchJobStatusMapToProps.get(status) || {};

        if (!entityColumnName || !eventType) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, `Unable to update the batch job status from ${batchJob.status} to ${status}. The status doesn't exist`);
        }

        batchJob[entityColumnName] = new Date();

        const batchJobRepo = transactionManager.getCustomRepository(this.batchJobRepository_);
        batchJob = await batchJobRepo.save(batchJob);
        batchJob.loadStatus();
    ...
    async confirm(batchJobOrId: string | BatchJob): Promise<BatchJob | never> {
        return await this.atomicPhase_(async () => {
            ...
            if (batchJob.status !== BatchJobStatus.PRE_PROCESSED) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot confirm processing for a batch job that is not pre processed");
            }
    ...
    async complete(batchJobOrId: string | BatchJob): Promise<BatchJob | never> {
        return await this.atomicPhase_(async () => {
            ...
            if (batchJob.status !== BatchJobStatus.PROCESSING) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `Cannot complete a batch job with status "${batchJob.status}". The batch job must be processing`);
            }
    ...
    async cancel(batchJobOrId: string | BatchJob): Promise<BatchJob | never> {
        return await this.atomicPhase_(async () => {
            ...
            if (batchJob.status === BatchJobStatus.COMPLETED) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot cancel completed batch job");
            }
    ...
    async setPreProcessingDone(batchJobOrId: string | BatchJob): Promise<BatchJob | never> {
        return await this.atomicPhase_(async () => {
            let batchJob: BatchJob = batchJobOrId as BatchJob;
            if (typeof batchJobOrId === "string") {
                batchJob = await this.retrieve(batchJobOrId);
            ...
            if (batchJob.status !== BatchJobStatus.CREATED) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot mark a batch job as pre processed if it is not in created status");
            }

            batchJob = await this.updateStatus(batchJobOrId, BatchJobStatus.PRE_PROCESSED);
            if (batchJob.dry_run) {
                return batchJob;
            }
    ...
    async setProcessing(batchJobOrId: string | BatchJob): Promise<BatchJob | never> {
        return await this.atomicPhase_(async () => {
            let batchJob: BatchJob = batchJobOrId as BatchJob;
            if (typeof batchJobOrId === "string") {
                batchJob = await this.retrieve(batchJobOrId);
            ...
            if (batchJob.status !== BatchJobStatus.CONFIRMED) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot mark a batch job as processing if the status is different that confirmed");
            }
    ...
    async setFailed(batchJobOrId: string | BatchJob, error?: BatchJobResultError): Promise<BatchJob | never> {
        return await this.atomicPhase_(async () => {
            let batchJob = batchJobOrId as BatchJob;
    ...
    async prepareBatchJobForProcessing(data: CreateBatchJobInput, req: Request): Promise<CreateBatchJobInput | never> {
        return await this.atomicPhase_(async () => {
            const batchStrategy = this.strategyResolver_.resolveBatchJobByType(data.type);
