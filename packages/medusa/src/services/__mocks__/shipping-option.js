...
export const ShippingOptionServiceMock = {
    ...
    listAndCount: jest.fn().mockImplementation((data) => {
        ...
        if (data.region_id === IdMap.getId("testRegion")) {
            return Promise.resolve([[shippingOptions.freeShipping, shippingOptions.expensiveShipping], 2]);
        }
    ...
    list: jest.fn().mockImplementation((data) => {
        ...
        if (data.region_id === IdMap.getId("testRegion")) {
            return Promise.resolve([shippingOptions.freeShipping, shippingOptions.expensiveShipping]);
        }
    }),
    create: jest.fn().mockImplementation((data) => {
        return Promise.resolve(data);
    }),
    validateFulfillmentData: jest.fn().mockImplementation((methodId, data, cart) => {
        return Promise.resolve(data);
    }),
