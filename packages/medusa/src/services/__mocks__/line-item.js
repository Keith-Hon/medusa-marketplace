...
export const LineItemServiceMock = {
    ...
    isEqual: jest.fn().mockImplementation((line, match) => {
        if (Array.isArray(line.content)) {
            if (Array.isArray(match.content) && match.content.length === line.content.length) {
                return line.content.every((c, index) => c.variant.id === match[index].variant.id && c.quantity === match[index].quantity);
            }
        } else if (!Array.isArray(match.content)) {
            return line.content.variant.id === match.content.variant.id && line.content.quantity === match.content.quantity;
        }

        return false;
    }),
    generate: jest.fn().mockImplementation((variantId, regionId, quantity, metadata = {}) => {
