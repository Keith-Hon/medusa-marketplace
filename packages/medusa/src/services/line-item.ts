...
class LineItemService extends BaseService {
    ...
    async list(selector, config = { skip: 0, take: 50, order: { created_at: "DESC" } }): Promise<LineItem[]> {
    ...
    async generate(
        ...
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (context.unit_price === undefined || context.unit_price === null) {
                shouldMerge = true;
                const variantPricing = await this.pricingService_.withTransaction(transactionManager).getProductVariantPricingById(variant.id, {
                    region_id: region.id,
                    quantity: quantity,
            ...
            if (context.cart) {
                const adjustments = await this.lineItemAdjustmentService_.withTransaction(transactionManager).generateAdjustments(context.cart, lineItem, { variant });
    ...
    async update(id: string, data: Partial<LineItem>): Promise<LineItem> {
        ...
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            const lineItem = await this.retrieve(id).then((lineItem) => {
                const lineItemMetadata = metadata ? this.setMetadata_(lineItem, metadata) : lineItem.metadata;
    ...
    async delete(id: string): Promise<LineItem | undefined> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            return await lineItemRepository.findOne({ where: { id } }).then((lineItem) => lineItem && lineItemRepository.remove(lineItem));
        });
    }
}
