...
class ProductTypeService extends BaseService {
     ...
     */
    async list(selector: FilterableProductTypeProps = {}, config: FindConfig<ProductType> = { skip: 0, take: 20 }): Promise<ProductType[]> {
        const typeRepo = this.manager_.getCustomRepository(this.typeRepository_);
     ...
     * @return {Promise} the result of the find operation
     */
    async listAndCount(selector: FilterableProductTypeProps = {}, config: FindConfig<ProductType> = { skip: 0, take: 20 }): Promise<[ProductType[], number]> {
        const typeRepo = this.manager_.getCustomRepository(this.typeRepository_);
