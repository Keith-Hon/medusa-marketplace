...
class ShippingProfileService extends BaseService {
    ...
    async list(selector = {}, config = { relations: [], skip: 0, take: 10 }) {
        const shippingProfileRepo = this.manager_.getCustomRepository(this.shippingProfileRepository_);
    ...
    async fetchOptionsByProductIds(productIds, filter) {
        const products = await this.productService_.list(
            ...
            {
                relations: ["profile", "profile.shipping_options", "profile.shipping_options.requirements"]
            }
        );

        const profiles = products.map((p) => p.profile);

        const optionIds = profiles.reduce((acc, next) => acc.concat(next.shipping_options), []);
    ...
    async retrieve(profileId, options = {}) {
        const profileRepository = this.manager_.getCustomRepository(this.shippingProfileRepository_);
        ...
        if (!profile) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Profile with id: ${profileId} was not found`);
        }
    ...
    async createDefault() {
        return this.atomicPhase_(async (manager) => {
            ...
            if (!profile) {
                const profileRepository = manager.getCustomRepository(this.shippingProfileRepository_);
    ...
    async create(profile) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (profile.products || profile.shipping_options) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Please add products and shipping_options after creating Shipping Profiles");
            }
    ...
    async update(profileId, update) {
        return this.atomicPhase_(async (manager) => {
            ...
            const profile = await this.retrieve(profileId, {
                relations: ["products", "products.profile", "shipping_options", "shipping_options.profile"]
            });
            ...
            if (shipping_options) {
                for (const oId of shipping_options) {
                    await this.shippingOptionService_.withTransaction(manager).update(oId, {
                        profile_id: profile.id
                    });
                }
            }
    ...
    async delete(profileId) {
        return this.atomicPhase_(async (manager) => {
            const profileRepo = manager.getCustomRepository(this.shippingProfileRepository_);
    ...
    async addProduct(profileId, productId) {
        return this.atomicPhase_(async (manager) => {
            await this.productService_.withTransaction(manager).update(productId, { profile_id: profileId });
    ...
    async addShippingOption(profileId, optionId) {
        return this.atomicPhase_(async (manager) => {
            await this.shippingOptionService_.withTransaction(manager).update(optionId, { profile_id: profileId });
    ...
    async decorate(profile, fields, expandFields = []) {
        ...
        if (expandFields.includes("products") && profile.products) {
            decorated.products = await Promise.all(profile.products.map((pId) => this.productService_.retrieve(pId)));
        }

        if (expandFields.includes("shipping_options") && profile.shipping_options) {
            decorated.shipping_options = await Promise.all(profile.shipping_options.map((oId) => this.shippingOptionService_.retrieve(oId)));
        }
    ...
    async fetchCartOptions(cart) {
        ...
        const customShippingOptions = await this.customShippingOptionService_.withTransaction(this.manager_).list(
            {
                cart_id: cart.id
            },
        ...
        const rawOpts = await this.shippingOptionService_.withTransaction(this.manager_).list(selector, {
        ...
        if (hasCustomShippingOptions) {
            return rawOpts.map((so) => {
                const customOption = customShippingOptions.find((cso) => cso.shipping_option_id === so.id);
        ...
        const options = await Promise.all(
            rawOpts.map(async (so) => {
                try {
                    const option = await this.shippingOptionService_.withTransaction(this.manager_).validateCartOption(so, cart);
                    if (option) {
                        return option;
                    }
                    return null;
