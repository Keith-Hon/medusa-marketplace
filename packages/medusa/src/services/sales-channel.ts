...
class SalesChannelService extends TransactionBaseService<SalesChannelService> {
     ...
     */
    async retrieve(salesChannelId: string, config: FindConfig<SalesChannel> = {}): Promise<SalesChannel | never> {
        const manager = this.manager_;
        const salesChannelRepo = manager.getCustomRepository(this.salesChannelRepository_);
        ...
        if (!salesChannel) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Sales channel with id ${salesChannelId} was not found`);
        }
    ...
    async create(data: CreateSalesChannelInput): Promise<SalesChannel | never> {
        return await this.atomicPhase_(async (manager) => {
            const salesChannelRepo: SalesChannelRepository = manager.getCustomRepository(this.salesChannelRepository_);

            const salesChannel = salesChannelRepo.create(data);

            await this.eventBusService_.withTransaction(manager).emit(SalesChannelService.Events.CREATED, {
                id: salesChannel.id
            });
    ...
    async update(salesChannelId: string, data: UpdateSalesChannelInput): Promise<SalesChannel | never> {
        return await this.atomicPhase_(async (transactionManager) => {
            const salesChannelRepo: SalesChannelRepository = transactionManager.getCustomRepository(this.salesChannelRepository_);
            ...
            await this.eventBusService_.withTransaction(transactionManager).emit(SalesChannelService.Events.UPDATED, {
                id: result.id
            });
    ...
    async delete(salesChannelId: string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager) => {
            ...
            if (salesChannel.id === store?.default_sales_channel_id) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "You cannot delete the default sales channel");
            }

            await salesChannelRepo.softRemove(salesChannel);

            await this.eventBusService_.withTransaction(transactionManager).emit(SalesChannelService.Events.DELETED, {
                id: salesChannelId
            });
        });
    }
    ...
    async createDefault(): Promise<SalesChannel> {
        return this.atomicPhase_(async (transactionManager) => {
            const store = await this.storeService_.withTransaction(transactionManager).retrieve({
     ...
     */
    async removeProducts(salesChannelId: string, productIds: string[]): Promise<SalesChannel | never> {
        return await this.atomicPhase_(async (transactionManager) => {
            const salesChannelRepo = transactionManager.getCustomRepository(this.salesChannelRepository_);
     ...
     */
    async addProducts(salesChannelId: string, productIds: string[]): Promise<SalesChannel | never> {
        return await this.atomicPhase_(async (transactionManager) => {
            const salesChannelRepo = transactionManager.getCustomRepository(this.salesChannelRepository_);
