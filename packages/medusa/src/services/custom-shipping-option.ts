...
class CustomShippingOptionService extends TransactionBaseService<CustomShippingOptionService> {
     ...
     */
    async retrieve(id: string, config: FindConfig<CustomShippingOption> = {}): Promise<CustomShippingOption> {
        const manager = this.manager_;
        const customShippingOptionRepo = manager.getCustomRepository(this.customShippingOptionRepository_);
        ...
        if (!customShippingOption) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Custom shipping option with id: ${id} was not found.`);
        }
     ...
     */
    async create(data: CreateCustomShippingOptionInput): Promise<CustomShippingOption> {
