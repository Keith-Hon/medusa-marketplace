...
describe("ProductVariantService", () => {
    describe("retrieve", () => {
        ...
        it("successfully retrieves a product variant", async () => {
            const result = await productVariantService.retrieve(IdMap.getId("ironman"));
        ...
        it("fails on non-existing product variant id", async () => {
            try {
                ...
                expect(error.message).toBe(`Variant with id: ${IdMap.getId("batman")} was not found`);
            }
        });
    });
    ...
    describe("create", () => {
        ...
        it("fails if product options and variant options differ in length", async () => {
            try {
                ...
                expect(error.message).toBe("Product options length does not match variant options length. Product has 2 and variant has 1.");
            }
        });
        ...
        it("fails if variants options is missing a product option", async () => {
            try {
                ...
                expect(error.message).toBe("Variant options do not contain value for size");
            }
        });
        ...
        it("fails if new option already exists", async () => {
            try {
                ...
                expect(error.message).toBe("Variant with title V1 with provided options already exists");
            }
        });
    });
    ...
    describe("update", () => {
        ...
        const productOptionValueRepository = MockRepository({
            findOne: () => Promise.resolve({ id: IdMap.getId("some-value"), value: "blue" })
        });
        ...
        productVariantService.updateOptionValue = jest.fn().mockReturnValue(() => Promise.resolve());

        productVariantService.updateVariantPrices = jest.fn().mockReturnValue(() => Promise.resolve());
        ...
        it("successfully updates variant by id", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledTimes(1);
            expect(eventBusService.emit).toHaveBeenCalledWith("product-variant.updated", {
                id: IdMap.getId("ironman"),
                fields: ["title"]
            });
        ...
        it("successfully updates variant prices", async () => {
            ...
            expect(productVariantService.updateVariantPrices).toHaveBeenCalledWith(IdMap.getId("ironman"), [
                {
                    currency_code: "dkk",
                    amount: 1000
                }
        ...
        it("successfully updates variant options", async () => {
            ...
            expect(productVariantService.updateOptionValue).toHaveBeenCalledWith(IdMap.getId("ironman"), IdMap.getId("color"), "red");
    ...
    describe("updateVariantPrices", () => {
        ...
        moneyAmountRepository.findVariantPricesNotIn = jest.fn().mockImplementation(() => Promise.resolve(oldPrices));

        moneyAmountRepository.deleteVariantPrices = jest.fn().mockImplementation((variant_id, price_ids) => Promise.resolve({}));
    ...
    describe("getRegionPrice", () => {
        ...
        it("successfully retrieves region price", async () => {
            const result = await productVariantService.getRegionPrice(IdMap.getId("ironman"), IdMap.getId("california"));

            expect(result).toBe(1000);
        });

        it("successfully retrieves region sale price", async () => {
            const result = await productVariantService.getRegionPrice(IdMap.getId("ironman-sale"), IdMap.getId("california"));
        ...
        it("fails if no price is found", async () => {
            try {
                await productVariantService.getRegionPrice(IdMap.getId("ironmanv2"), IdMap.getId("california"));
            } catch (error) {
                expect(error.message).toBe("A price for region: California could not be found");
            }
        });
    });
    ...
    describe("updateVariantPrices", () => {
        ...
        moneyAmountRepository.upsertVariantCurrencyPrice = jest.fn().mockImplementation(() => Promise.resolve());
        ...
        const regionService = {
            list: jest.fn().mockImplementation((config) => {
                ...
                if (Array.isArray(idOrIds)) {
                    return Promise.resolve(idOrIds.map((id) => ({ id, currency_code: "usd" })));
                } else {
        ...
        it("successfully removes obsolete prices and calls save on new/existing prices", async () => {
            ...
            expect(moneyAmountRepository.findVariantPricesNotIn).toHaveBeenCalledTimes(1);

            expect(moneyAmountRepository.upsertVariantCurrencyPrice).toHaveBeenCalledTimes(1);
            expect(moneyAmountRepository.upsertVariantCurrencyPrice).toHaveBeenCalledWith("ironman", {
                currency_code: "usd",
                amount: 4000
            });
        ...
        it("successfully creates a currency price", async () => {
            ...
            expect(moneyAmountRepository.upsertVariantCurrencyPrice).toHaveBeenCalledTimes(1);
            expect(moneyAmountRepository.upsertVariantCurrencyPrice).toHaveBeenCalledWith(IdMap.getId("ironman"), {
                id: IdMap.getId("dkk"),
                currency_code: "dkk",
                amount: 750
            });
    ...
    describe("updateOptionValue", () => {
        ...
        it("successfully updates", async () => {
            await productVariantService.updateOptionValue(IdMap.getId("ironman"), IdMap.getId("color"), "more red");
        ...
        it("fails if product option value does not exist", async () => {
            try {
                await productVariantService.updateOptionValue(IdMap.getId("jibberish"), IdMap.getId("color"), "red");
            } catch (error) {
    ...
    describe("addOptionValue", () => {
        ...
        it("successfully add an option value", async () => {
            await productVariantService.addOptionValue(IdMap.getId("ironman"), IdMap.getId("color"), "black");
    ...
    describe("deleteOptionValue", () => {
        ...
        it("successfully deletes option value from variant", async () => {
            await productVariantService.deleteOptionValue(IdMap.getId("ironman"), IdMap.getId("color"));
        ...
        it("successfully resolves if product option value does not exist", async () => {
            const result = await productVariantService.deleteOptionValue(IdMap.getId("ironman"), IdMap.getId("size"));
    ...
    describe("delete", () => {
        ...
        it("successfully deletes variant", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledTimes(1);
            expect(eventBusService.emit).toHaveBeenCalledWith("product-variant.deleted", {
                id: IdMap.getId("ironman"),
                product_id: IdMap.getId("product-test")
            });
        });
        ...
        it("successfully resolves if variant does not exist", async () => {
            const result = await productVariantService.delete(IdMap.getId("ironmanv2"));
