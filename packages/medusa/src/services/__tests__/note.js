...
describe("NoteService", () => {
    describe("list", () => {
        const noteRepo = MockRepository({
            find: (q) => {
                return Promise.resolve([{ id: IdMap.getId("note"), value: "some note" }]);
            }
        });
    ...
    describe("retrieve", () => {
        ...
        it("fails when note is not found", async () => {
            await expect(noteService.retrieve(IdMap.getId("not-existing"))).rejects.toThrow(`Note with id: ${IdMap.getId("not-existing")} was not found.`);
        });
    });
    ...
    describe("create", () => {
        ...
        it("calls note model functions", async () => {
            ...
            expect(EventBusServiceMock.emit).toHaveBeenCalledWith(NoteService.Events.CREATED, { id: IdMap.getId("note") });
        });
    });
    ...
    describe("update", () => {
        ...
        it("calls note model functions", async () => {
            ...
            expect(EventBusServiceMock.emit).toHaveBeenCalledTimes(1);
            expect(EventBusServiceMock.emit).toHaveBeenCalledWith(NoteService.Events.UPDATED, { id: IdMap.getId("note") });
        });
    });
    ...
    describe("delete", () => {
        ...
        it("calls note model functions", async () => {
            ...
            expect(EventBusServiceMock.emit).toHaveBeenCalledTimes(1);
            expect(EventBusServiceMock.emit).toHaveBeenCalledWith(NoteService.Events.DELETED, { id: IdMap.getId("note") });
        });
    });
});
