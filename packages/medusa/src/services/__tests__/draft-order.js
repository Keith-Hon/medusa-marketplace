...
describe("DraftOrderService", () => {
    ...
    describe("create", () => {
        ...
        const regionService = {
            retrieve: () => Promise.resolve({ id: "test-region", countries: [{ iso_2: "dk" }] }),
            withTransaction: function () {
                return this;
            }
        };
        ...
        it("creates a draft order", async () => {
            ...
            expect(cartService.addShippingMethod).toHaveBeenCalledWith("test-cart", "test-option", {});

            expect(lineItemService.generate).toHaveBeenCalledTimes(1);
            expect(lineItemService.generate).toHaveBeenCalledWith("test-variant", "test-region", 2, {
                metadata: {},
        ...
        it("fails on missing region", async () => {
            try {
                ...
                expect(error.message).toEqual(`region_id is required to create a draft order`);
            }
        });
        ...
        it("fails on missing items", async () => {
            try {
                ...
                expect(error.message).toEqual(`Items are required to create a draft order`);
            }
        });
    });
    ...
    describe("update", () => {
        ...
        it("fails to update draftOrder when already complete", async () => {
            await expect(draftOrderService.update("completed", {})).rejects.toThrow("Can't update a draft order which is complete");
        });
    });
});
