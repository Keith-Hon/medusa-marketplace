...
const calculateAdjustment = (cart, lineItem, discount) => {
    ...
    if (discount.rule.type === "fixed" && discount.rule.allocation === "total") {
        let subtotal = cart.items.reduce((total, item) => total + item.unit_price * item.quantity, 0);
...
describe("TotalsService", () => {
    ...
    describe("getTaxTotal", () => {
        ...
        it("uses order tax lines", async () => {
            ...
            expect(calculateMock).toHaveBeenCalledWith(order.items, [{ id: "orderline1" }], {
        ...
        it("calculates tax", async () => {
            ...
            expect(getTaxLinesMock).toHaveBeenCalledWith([{ quantity: 2, unit_price: 20 }], {
                shipping_address: order.shipping_address,
                shipping_methods: order.shipping_methods,
                is_return: false,
                customer: order.customer,
