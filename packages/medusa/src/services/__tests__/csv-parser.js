    ...
    describe("buildData", () => {
        describe("schema validation", () => {
            ...
            it("given a line containing a column which is not defined in the schema, then validation should fail", async () => {
                try {
                    ...
                    expect(err.message).toEqual("Unable to treat column first_name from the csv file. No target column found in the provided schema");
                }
            });

            it("given a line containing a column which does not pass a validation constraint, then validation should fail", async () => {
                try {
                    await csvParser.buildData([{ title: "contains a number 1", size: "M", height: "100" }]);
                } catch (err) {
            ...
            it("given a line which passes all validation constraints, then should returned validated content", async () => {
                const content = await csvParser.buildData([{ title: "great product", size: "M", height: "100" }]);
            ...
            it("given a line which does not provide a value for a required column, then should throw an error", async () => {
                try {
                    ...
                    expect(err.message).toEqual(`Missing column(s) "title" from the given csv file`);
                }
            });
            ...
            it("given a line which does not provide a value for multiple required columns, then should throw an error", async () => {
                try {
                    ...
                    expect(err.message).toEqual(`Missing column(s) "title", "height" from the given csv file`);
                }
            });

            it("given a line which does not provide a value for a required column, then should throw an error", async () => {
                try {
                    await csvParser.buildData([{ title: "t-shirt", height: "100", size: "" }]);
                } catch (err) {
                    expect(err.message).toEqual(`No value found for target column "size" in line 1 of the given csv file`);
                }
            });
        });
        ...
        describe("transformer", () => {
            ...
            it("given a transformer function for a column, when building data, should transform that column's value according to the transformation function", async () => {
                const content = await csvParser.buildData([{ title: "medusa t-shirt", "price usd": "19.99" }]);
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            reducer: (builtLine, key, value) => {
                                const [, currency_code] = key.match(/.*Variant Price ([a-z]+).*/i);
                            ...
                            validator: {
                                validate: (builtLine) => {
                                    const unexistingCurrency = builtLine.prices?.find((price) => !currencies[price.currency_code.toUpperCase()]);
                                    if (unexistingCurrency) {
                                        throw new Error(`wrong currency: ${unexistingCurrency.currency_code}`);
                                    }
                                    return true;
                                }
                            }
