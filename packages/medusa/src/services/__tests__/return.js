...
describe("ReturnService", () => {
    ...
    describe("receive", () => {
        ...
        it("successfully receives a return", async () => {
            await returnService.receive(IdMap.getId("test-return"), [{ item_id: IdMap.getId("test-line"), quantity: 10 }], 1000);
            ...
            expect(lineItemService.update).toHaveBeenCalledWith(IdMap.getId("test-line"), {
                returned_quantity: 10
            });

            expect(inventoryService.adjustInventory).toHaveBeenCalledTimes(1);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("test-variant", 10);
        });
        ...
        it("successfully receives a return with requires_action status", async () => {
            ...
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("test-variant", 10);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("test-variant-2", 10);
    ...
    describe("canceled", () => {
        ...
        it("fails to cancel return when already received", async () => {
            await expect(returnService.cancel(IdMap.getId("test-return-2"))).rejects.toThrow("Can't cancel a return which has been returned");
        });
    });
    ...
    describe("fulfilled", () => {
        ...
        it("fails to fulfill when return is canceled", async () => {
            await expect(returnService.fulfill(IdMap.getId("test-return"))).rejects.toThrow("Cannot fulfill a canceled return");
        });
    });
    ...
    describe("update", () => {
        ...
        it("fails to update when return is canceled", async () => {
            await expect(returnService.update(IdMap.getId("test-return"), {})).rejects.toThrow("Cannot update a canceled return");
        });
    });
});
