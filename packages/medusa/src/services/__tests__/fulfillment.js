...
describe("FulfillmentService", () => {
    ...
    describe("cancelFulfillment", () => {
        ...
        const lineItemService = {
            retrieve: jest.fn().mockImplementation(() => Promise.resolve({ id: 1, fulfilled_quantity: 2 })),
            update: jest.fn(),
    ...
    describe("createShipment", () => {
        ...
        it("calls order model functions", async () => {
            await fulfillmentService.createShipment(IdMap.getId("fulfillment"), [{ tracking_number: "1234" }, { tracking_number: "2345" }], {});

            expect(fulfillmentRepository.save).toHaveBeenCalledTimes(1);
            expect(fulfillmentRepository.save).toHaveBeenCalledWith({
                id: IdMap.getId("fulfillment"),
                tracking_links: [{ tracking_number: "1234" }, { tracking_number: "2345" }],
                metadata: {},
                shipped_at: now
            });
        });
        ...
        it("fails when status is canceled", async () => {
            await expect(fulfillmentService.createShipment(IdMap.getId("canceled"), [({ tracking_number: "1234" }, { tracking_number: "2345" })], {})).rejects.toThrow(
