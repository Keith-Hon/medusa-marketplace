...
describe("UserService", () => {
    ...
    describe("create", () => {
        ...
        it("successfully create a user", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledWith(UserService.Events.CREATED, {
    ...
    describe("update", () => {
        ...
        it("successfully updates user", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledWith(UserService.Events.UPDATED, {
                id: IdMap.getId("ironman")
            });
        });
        ...
        it("fails on password update", async () => {
            try {
                ...
                expect(error.message).toBe("Use dedicated methods, `setPassword`, `generateResetPasswordToken` for password operations");
            }
        });
    });
    ...
    describe("generateResetPasswordToken", () => {
        const userRepository = MockRepository({
            findOne: () => Promise.resolve({ id: IdMap.getId("ironman"), password_hash: "lol" })
        });
        ...
        it("generates a token successfully", async () => {
            const token = await userService.generateResetPasswordToken(IdMap.getId("ironman"));
