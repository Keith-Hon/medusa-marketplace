...
describe("OrderService", () => {
    ...
    describe("createFromCart", () => {
        ...
        it("calls order model functions", async () => {
            ...
            expect(paymentProviderService.updatePayment).toHaveBeenCalledWith("testpayment", {
                order_id: "id"
            });

            expect(inventoryService.adjustInventory).toHaveBeenCalledTimes(2);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("variant-2", -1);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("variant-1", -1);
        ...
        it("fails because an item does not have the required inventory", async () => {
            ...
            await expect(res).rejects.toThrow("Variant with id: variant-1 does not have the required inventory");
            // check to see if payment is cancelled
            expect(orderService.paymentProviderService_.cancelPayment).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("update", () => {
        ...
        it("throws if payment method update is attempted after fulfillment", async () => {
            ...
            ).rejects.toThrow("Can't update shipping, billing, items and payment method when order is processed");
        });
        ...
        it("throws if order is canceled", async () => {
            await expect(orderService.update(IdMap.getId("canceled-order"), {})).rejects.toThrow("A canceled order cannot be updated");
        });
    });
    ...
    describe("cancel", () => {
        ...
        it("calls order model functions", async () => {
            ...
            expect(inventoryService.adjustInventory).toHaveBeenCalledTimes(2);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("variant-1", 12);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("variant-2", 1);
    ...
    describe("capturePayment", () => {
        ...
        const paymentProviderService = {
            capturePayment: jest.fn().mockImplementation((p) => (p.id === "payment_fail" ? Promise.reject() : Promise.resolve({ ...p, captured_at: "notnull" }))),
            withTransaction: function () {
                return this;
            }
        };
        ...
        it("fails if order is canceled", async () => {
            await expect(orderService.capturePayment(IdMap.getId("canceled"))).rejects.toThrow("A canceled order cannot capture payment");
        });
    });
    ...
    describe("cancelFulfillment", () => {
        ...
        it("successfully cancels fulfillment and corrects order status", async () => {
            ...
            expect(fulfillmentService.cancelFulfillment).toHaveBeenCalledWith(IdMap.getId("order"));
        ...
        it("fails to cancel fulfillment when not related to an order", async () => {
            await expect(orderService.cancelFulfillment(IdMap.getId("no-order"))).rejects.toThrow(`Fufillment not related to an order`);
        });
    });
    ...
    describe("registerReturnReceived", () => {
        ...
        const paymentProviderService = {
            refundPayment: jest.fn().mockImplementation((p) => (p.id === "payment_fail" ? Promise.reject() : Promise.resolve())),
            withTransaction: function () {
                return this;
            }
        };
        ...
        it("return with custom refund", async () => {
            ...
            expect(paymentProviderService.refundPayment).toHaveBeenCalledTimes(1);
            expect(paymentProviderService.refundPayment).toHaveBeenCalledWith(order.payments, 95, "return");
        });

        it("fails when order is canceled", async () => {
            await expect(orderService.registerReturnReceived(IdMap.getId("canceled"), {})).rejects.toThrow("A canceled order cannot be registered as received");
        });
    });
    ...
    describe("completeOrder", () => {
        ...
        it("fails when order is canceled", async () => {
            await expect(orderService.completeOrder(IdMap.getId("canceled"))).rejects.toThrow("A canceled order cannot be completed");
        });
    });
    ...
    describe("addShippingMethod", () => {
            ...
            findOneWithRelations: (rel, q) => {
                switch (q.where.id) {
                    ...
                    default:
                        return Promise.resolve({
                            id: IdMap.getId("order"),
                            shipping_methods: [{ shipping_option: { profile_id: IdMap.getId("method1") } }]
                        });
                }
            },
            save: jest.fn().mockImplementation((f) => f)
        ...
        const optionService = {
            createShippingMethod: jest.fn().mockImplementation((optionId, data, config) => Promise.resolve({ shipping_option: { profile_id: optionId } })),
            deleteShippingMethods: jest.fn().mockImplementation(() => Promise.resolve({})),
        ...
        it("successfully adds shipping method", async () => {
            await orderService.addShippingMethod(IdMap.getId("order"), IdMap.getId("option"), { some: "data" }, {});
        ...
        it("successfully removes shipping method if same option profile", async () => {
            await orderService.addShippingMethod(IdMap.getId("order"), IdMap.getId("method1"), { some: "data" });
        ...
        it("fails if order is canceled", async () => {
            await expect(orderService.addShippingMethod(IdMap.getId("canceled"), IdMap.getId("option"), { some: "data" })).rejects.toThrow(
    ...
    describe("createShipment", () => {
        ...
        const fulfillmentService = {
            ...
            createShipment: jest.fn().mockImplementation((shipmentId, tracking, meta) => {
                return Promise.resolve({
                    items: [
                        {
                            item_id: "item_1",
        ...
        it("calls order model functions", async () => {
            await orderService.createShipment(IdMap.getId("test"), IdMap.getId("fulfillment"), [{ tracking_number: "1234" }, { tracking_number: "2345" }], {});

            expect(fulfillmentService.createShipment).toHaveBeenCalledTimes(1);
            expect(fulfillmentService.createShipment).toHaveBeenCalledWith(IdMap.getId("fulfillment"), [{ tracking_number: "1234" }, { tracking_number: "2345" }], {
                metadata: undefined,
                no_notification: true
            });
        ...
        it("fails when order is canceled", async () => {
            await expect(orderService.createShipment(IdMap.getId("canceled", IdMap.getId("fulfillment"), [{ tracking_number: "1234" }], {}))).rejects.toThrow(
    ...
    describe("createRefund", () => {
        ...
        const paymentProviderService = {
            refundPayment: jest.fn().mockImplementation((p) => Promise.resolve({ id: "ref" })),
            withTransaction: function () {
                return this;
            }
        };
        ...
        it("success", async () => {
            await orderService.createRefund(IdMap.getId("order_123"), 100, "discount", "note");
        ...
        it("fails when refund is off", async () => {
            await expect(orderService.createRefund(IdMap.getId("cannot"), 100, "discount", "note")).rejects.toThrow("Cannot refund more than the original order amount");
        });
        it("fails when order is canceled", async () => {
            await expect(orderService.createRefund(IdMap.getId("canceled"), 100, "discount", "note")).rejects.toThrow("A canceled order cannot be refunded");
        });
        ...
        it.each([
            ...
            await orderService.createRefund(IdMap.getId("order_123"), 100, "discount", "note", { no_notification: input });
