...
describe("InventoryService", () => {
    describe("confirmInventory", () => {
        ...
        it("returns false when inventory is managed, and no back orders are allowed and the quantity is larger than inventory", async () => {
            await expect(inventoryService.confirmInventory("no_bo", 10)).rejects.toThrow(`Variant with id: no_bo does not have the required inventory`);
        });
        ...
        it("returns false when managed variant inventory_quantity < requested quantity", async () => {
            await expect(inventoryService.confirmInventory("10_man", 50)).rejects.toThrow(`Variant with id: 10_man does not have the required inventory`);
        });
    });
