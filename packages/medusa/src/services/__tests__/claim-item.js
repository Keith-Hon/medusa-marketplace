...
describe("ClaimItemService", () => {
    describe("create", () => {
        ...
        it("successfully creates a claim item", async () => {
            lineItemService.retrieve = jest.fn(() => Promise.resolve({ fulfilled_quantity: 1 }));
        ...
        it("fails if fulfilled_quantity < quantity", async () => {
            lineItemService.retrieve = jest.fn(() => Promise.resolve({ fulfilled_quantity: 0 }));
            await expect(claimItemService.create(testItem)).rejects.toThrow("Cannot claim more of an item than has been fulfilled");
        });
        ...
        it("fails if reason is unknown", async () => {
            ...
            ).rejects.toThrow(`Claim Item reason must be one of "missing_item", "wrong_item", "production_failure" or "other".`);
        });
    });
});
