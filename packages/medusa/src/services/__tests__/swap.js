...
const generateOrder = (orderId, items, additional = {}) => {
    return {
        ...
        items: items.map(({ id, product_id, variant_id, fulfilled, returned, quantity, price }) => ({
            id: IdMap.getId(id),
...
describe("SwapService", () => {
    ...
    describe("createCart", () => {
        ...
        describe("success", () => {
            ...
            const cartService = {
                ...
                retrieve: jest.fn().mockReturnValue(Promise.resolve({ id: "cart", items: [{ id: "test-item" }] })),
            ...
            it("finds swap and calls return create cart", async () => {
                await swapService.createCart(IdMap.getId("swap-1"), [{ option_id: "test-option", price: 10 }]);
                ...
                expect(lineItemService.createReturnLines).toHaveBeenCalledWith(expect.any(String), "cart");
                ...
                expect(LineItemAdjustmentServiceMock.createAdjustmentForLineItem).toHaveBeenCalledTimes(1);
        ...
        describe("failure", () => {
            ...
            it("fails if cart already created", async () => {
                ...
                await expect(res).rejects.toThrow("A cart has already been created for the swap");
            });

            it("fails if swap is canceled", async () => {
                await expect(swapService.createCart(IdMap.getId("canceled"))).rejects.toThrow("Canceled swap cannot be used to create a cart");
            });
        });
    });
    ...
    describe("create", () => {
        ...
        describe("success", () => {
            ...
            it("generates lines items", async () => {
                await swapService.create(testOrder, [{ item_id: IdMap.getId("line"), quantity: 1 }], [{ variant_id: IdMap.getId("new-variant"), quantity: 1 }], {
                    id: IdMap.getId("return-shipping"),
                    price: 20
                });

                expect(lineItemService.generate).toHaveBeenCalledTimes(1);
                expect(lineItemService.generate).toHaveBeenCalledWith(IdMap.getId("new-variant"), IdMap.getId("region"), 1);
            });
            ...
            it.each([
                ...
                expect(eventBusService.emit).toHaveBeenCalledWith(expect.any(String), { id: undefined, no_notification: expected });
            });
        });
    });
    ...
    describe("createFulfillment", () => {
        ...
        describe("success", () => {
            const fulfillmentService = {
                createFulfillment: jest.fn().mockReturnValue(Promise.resolve([{ items: [{ item_id: "1234", quantity: 2 }], data: "new" }])),
                withTransaction: function () {
                    return this;
                }
            };
            ...
            it("creates a fulfillment", async () => {
                ...
                expect(swapRepo.save).toHaveBeenCalledWith({
                    ...existing,
                    fulfillment_status: "fulfilled",
                    fulfillments: [{ items: [{ item_id: "1234", quantity: 2 }], data: "new" }]
                });
        ...
        describe("failure", () => {
            ...
            it("fails when swap has been canceled", async () => {
                await expect(swapService.createFulfillment(IdMap.getId("swap"), {})).rejects.toThrow("Canceled swap cannot be fulfilled");
            });
        });
    });
    ...
    describe("cancelFulfillment", () => {
        ...
        it("successfully cancels fulfillment and corrects swap status", async () => {
            ...
            expect(fulfillmentService.cancelFulfillment).toHaveBeenCalledWith(IdMap.getId("swap"));
        ...
        it("fails to cancel fulfillment when not related to a swap", async () => {
            await expect(swapService.cancelFulfillment(IdMap.getId("no-swap"))).rejects.toThrow(`Fufillment not related to a swap`);
        });
    });
    ...
    describe("createShipment", () => {
        ...
        describe("success", () => {
            ...
            it("creates a shipment", async () => {
                await swapService.createShipment(IdMap.getId("swap"), IdMap.getId("f1"), ["1234"], {});

                expect(lineItemService.update).toHaveBeenCalledWith(IdMap.getId("1234-1"), {
                    shipped_quantity: 2
                });
                ...
                expect(fulfillmentService.createShipment).toHaveBeenCalledWith(IdMap.getId("f1"), ["1234"], {});
            });
        });

        describe("failure", () => {
            const swapRepo = MockRepository({
                findOneWithRelations: () => Promise.resolve({ canceled_at: new Date() })
            });
            ...
            it("fails when swap is canceled", async () => {
                await expect(swapService.createShipment(IdMap.getId("swap", IdMap.getId("fulfillment"), [], {}))).rejects.toThrow("Canceled swap cannot be fulfilled as shipped");
            });
        });
    });
    ...
    describe("registerCartCompletion", () => {
        ...
        describe("success", () => {
            ...
            it("creates a shipment", async () => {
                ...
                expect(inventoryService.confirmInventory).toHaveBeenCalledWith("variant", 2);
                expect(inventoryService.adjustInventory).toHaveBeenCalledWith("variant", -2);
        ...
        describe("failure", () => {
            ...
            it("fails to register cart completion when swap is canceled", async () => {
                await expect(swapService.registerCartCompletion(IdMap.getId("canceled"))).rejects.toThrow("Cart related to canceled swap cannot be completed");
            });
            it("throws an error because inventory is to low", async () => {
                try {
                    await swapService.registerCartCompletion(IdMap.getId("swap"));
                } catch (e) {
                    expect(e.message).toEqual(`Variant with id: variant does not have the required inventory`);
                }
            });
        });
    });
    ...
    describe("processDifference", () => {
        ...
        describe("success", () => {
            ...
            const paymentProviderService = {
                capturePayment: jest.fn((g) => (g.id === "good" ? Promise.resolve() : Promise.reject())),
                refundPayment: jest.fn((g) => (g[0].id === "good" ? Promise.resolve() : Promise.reject())),
                withTransaction: function () {
                    return this;
                }
            };
            ...
            it("fails as swap is canceled", async () => {
                await expect(swapService.processDifference("canceled")).rejects.toThrow("Canceled swap cannot be processed");
            });
            ...
            it("not confirmed", async () => {
                await expect(swapService.processDifference("not_conf")).rejects.toThrow("Cannot process a swap that hasn't been confirmed by the customer");
            });
        });
        ...
        describe("registerReceived", () => {
            ...
            it("fails to register as received when swap is canceled", async () => {
                await expect(swapService.registerReceived("canceled")).rejects.toThrow("Canceled swap cannot be registered as received");
            });
        });
    });
    ...
    describe("cancel", () => {
        ...
        it("fails to cancel swap when fulfillment not canceled", async () => {
            await expect(swapService.cancel(IdMap.getId("fail-fulfillment"))).rejects.toThrow("All fulfillments must be canceled before the swap can be canceled");
        });

        it("fails to cancel swap when return not canceled", async () => {
            await expect(swapService.cancel(IdMap.getId("fail-return"))).rejects.toThrow("Return must be canceled before the swap can be cancele");
        });

        it.each([["fail-refund-1"], ["fail-refund-2"], ["fail-refund-3"]])("fails to cancel swap when contains refund", async (input) => {
            await expect(swapService.cancel(IdMap.getId(input))).rejects.toThrow("Swap with a refund cannot be canceled");
        });
    });
});
