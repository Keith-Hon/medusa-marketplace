...
describe("ShippingOptionService", () => {
    ...
    describe("update", () => {
        ...
        const fulfillmentProviderService = {
            canCalculate: jest.fn().mockImplementation((option) => option.provider_id !== "no_calc")
        };
        ...
        it("fails on invalid req", async () => {
            ...
            await expect(optionService.update(IdMap.getId("option"), { requirements })).rejects.toThrow("Requirement type must be one of min_subtotal, max_subtotal");
        });
        ...
        it("fails on duplicate reqs", async () => {
            ...
            await expect(optionService.update(IdMap.getId("validId"), { requirements })).rejects.toThrow("Only one requirement of each type is allowed");
        });
        ...
        it("fails if provider cannot calculate", async () => {
            ...
            ).rejects.toThrow("The fulfillment provider cannot calculate prices for this option");
        });

        it("throws error when trying to update region_id", async () => {
            const id = IdMap.getId("validId");
            await expect(optionService.update(`${id}`, { region_id: "id" })).rejects.toThrow("Region and Provider cannot be updated after creation");
        });

        it("throws error when trying to update provider_id", async () => {
            const id = IdMap.getId("validId");
            await expect(optionService.update(`${id}`, { provider_id: "id" })).rejects.toThrow("Region and Provider cannot be updated after creation");
        });
    });

    describe("delete", () => {
        const shippingOptionRepository = MockRepository({
            findOne: (i) => (i.where.id === IdMap.getId("validId") ? { id: IdMap.getId("validId") } : null)
        });
        const optionService = new ShippingOptionService({
            manager: MockManager,
            shippingOptionRepository
    ...
    describe("removeRequirement", () => {
        const shippingOptionRequirementRepository = MockRepository({
            ...
            findOne: (i) => (i.where.id === IdMap.getId("requirement_id") ? { id: IdMap.getId("requirement_id") } : null)
        });
    ...
    describe("create", () => {
        ...
        it("creates a shipping option", async () => {
            ...
            expect(fulfillmentProviderService.validateOption).toHaveBeenCalledWith(option);
        ...
        it("fails if region doesn't have fulfillment provider", async () => {
            ...
            await expect(optionService.create(option)).rejects.toThrow("The fulfillment provider is not available in the provided region");
        });
        ...
        it("fails if fulfillment provider cannot validate", async () => {
            ...
            await expect(optionService.create(option)).rejects.toThrow("The fulfillment provider cannot validate the shipping option");
        });
        ...
        it("fails if requirement is not validated", async () => {
            ...
            await expect(optionService.create(option)).rejects.toThrow("Requirement type must be one of min_subtotal, max_subtotal");
        });
        ...
        it("fails if price is not validated", async () => {
            ...
            await expect(optionService.create(option)).rejects.toThrow("The price must be of type flat_rate or calculated");
        });
    });
    ...
    describe("createShippingMethod", () => {
        ...
        const providerService = {
            validateFulfillmentData: jest.fn().mockImplementation((r) => Promise.resolve(r.data)),
            getPrice: (d) => d.price
        };
        ...
        it("validates", async () => {
            ...
            await optionService.createShippingMethod(IdMap.getId("option"), { provider_data: "dat" }, { cart });

            expect(providerService.validateFulfillmentData).toHaveBeenCalledTimes(1);
            expect(providerService.validateFulfillmentData).toHaveBeenCalledWith(option(IdMap.getId("option")), { provider_data: "dat" }, cart);
        ...
        it("fails on invalid req", async () => {
            ...
            await expect(optionService.createShippingMethod(id, d, { cart: c })).rejects.toThrow("The shipping option is not available in the cart's region");
        });
        ...
        it("fails if reqs are not satisfied", async () => {
            ...
            ).rejects.toThrow("The Cart does not satisfy the shipping option's requirements");
        });
    });
});
