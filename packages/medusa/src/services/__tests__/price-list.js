...
describe("PriceListService", () => {
    ...
    describe("retrieve", () => {
        ...
        it("fails on non-existing product variant id", async () => {
            try {
                ...
                expect(error.message).toBe(`Price list with id: ${IdMap.getId("batman")} was not found`);
            }
        });
    });
    ...
    describe("create", () => {
        it("creates a new Price List", async () => {
            ...
            expect(customerGroupService.retrieve).toHaveBeenCalledWith(IdMap.getId("group"));
            expect(moneyAmountRepository.addPriceListPrices).toHaveBeenCalledTimes(1);
            expect(moneyAmountRepository.addPriceListPrices).toHaveBeenCalledWith(IdMap.getId("ironman"), [
                {
                    amount: 100,
                    currency_code: "usd",
                    min_quantity: 1,
