...
describe("SalesChannelService", () => {
        ...
        ...MockRepository({
            findOne: jest.fn().mockImplementation((queryOrId: string | FindOneOptions<SalesChannel>): any => {
                return Promise.resolve({
                    id: typeof queryOrId === "string" ? queryOrId : (queryOrId?.where as FindConditions<SalesChannel>)?.id ?? IdMap.getId("sc_adjhlukiaeswhfae"),
                    ...salesChannelData
                });
            }),
    ...
    describe("retrieve", () => {
        ...
        it("should retrieve a sales channel", async () => {
            const salesChannel = await salesChannelService.retrieve(IdMap.getId("sales_channel_1"));
    ...
    describe("list", () => {
        ...
        it("should retrieve a sales channel using free text search", async () => {
            ...
            expect(salesChannelRepositoryMock.getFreeTextSearchResultsAndCount).toHaveBeenLastCalledWith(q, {
                skip: 0,
                take: 20,
                where: {}
            });
    ...
    describe("delete", () => {
        ...
        it("should soft remove a sales channel", async () => {
            const res = await salesChannelService.delete(IdMap.getId("sales_channel_1"));
            ...
            expect(EventBusServiceMock.emit).toHaveBeenLastCalledWith(SalesChannelService.Events.DELETED, { id: IdMap.getId("sales_channel_1") });
        });
        ...
        it("should fail if delete of the default channel is attempted", async () => {
            try {
                ...
                expect(error.message).toEqual("You cannot delete the default sales channel");
            }
        });
    });
    ...
    describe("Remove products", () => {
        ...
        it("should remove a list of product to a sales channel", async () => {
            const salesChannel = await salesChannelService.removeProducts(IdMap.getId("sales_channel_1"), [IdMap.getId("sales_channel_1_product_1")]);

            expect(salesChannelRepositoryMock.removeProducts).toHaveBeenCalledTimes(1);
            expect(salesChannelRepositoryMock.removeProducts).toHaveBeenCalledWith(IdMap.getId("sales_channel_1"), [IdMap.getId("sales_channel_1_product_1")]);
            expect(salesChannel).toBeTruthy();
            expect(salesChannel).toEqual({
                id: IdMap.getId("sales_channel_1"),
                ...salesChannelData
    ...
    describe("Add products", () => {
        ...
        it("should add a list of product to a sales channel", async () => {
            const salesChannel = await salesChannelService.addProducts(IdMap.getId("sales_channel_1"), [IdMap.getId("sales_channel_1_product_1")]);

            expect(salesChannelRepositoryMock.addProducts).toHaveBeenCalledTimes(1);
            expect(salesChannelRepositoryMock.addProducts).toHaveBeenCalledWith(IdMap.getId("sales_channel_1"), [IdMap.getId("sales_channel_1_product_1")]);
            expect(salesChannel).toBeTruthy();
            expect(salesChannel).toEqual({
                id: IdMap.getId("sales_channel_1"),
                ...salesChannelData
