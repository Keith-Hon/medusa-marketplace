...
describe("ShippingProfileService", () => {
    ...
    describe("update", () => {
        ...
        it("calls updateOne with shipping options", async () => {
            ...
            expect(shippingOptionService.update).toBeCalledWith(IdMap.getId("validId"), { profile_id: id });
        });
    });
    ...
    describe("addProduct", () => {
        ...
        it("add product to profile successfully", async () => {
            await profileService.addProduct(IdMap.getId("validId"), IdMap.getId("product2"));
    ...
    describe("fetchCartOptions", () => {
        ...
        it("given a cart with no custom shipping options, should return normal shipping options", async () => {
            ...
            await expect(profileService.fetchCartOptions(cart)).resolves.toEqual([{ id: "ship_1" }, { id: "ship_2" }]);

            expect(shippingOptionService.validateCartOption).toBeCalledTimes(2);
            expect(shippingOptionService.validateCartOption).toBeCalledWith({ id: "ship_1" }, cart);
            expect(shippingOptionService.validateCartOption).toBeCalledWith({ id: "ship_2" }, cart);
        });
    });
    ...
    describe("addShippingOption", () => {
        ...
        it("add shipping option to profile successfully", async () => {
            await profileService.addShippingOption(IdMap.getId("validId"), IdMap.getId("freeShipping"));

            expect(shippingOptionService.update).toBeCalledTimes(1);
            expect(shippingOptionService.update).toBeCalledWith(IdMap.getId("freeShipping"), { profile_id: IdMap.getId("validId") });
        });
    });
    ...
    describe("create", () => {
        ...
        it("throws if trying to create with products", async () => {
            ...
            ).rejects.toThrow("Please add products and shipping_options after creating Shipping Profiles");
        });
    });
});
