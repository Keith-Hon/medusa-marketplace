...
describe("ProductService", () => {
    ...
    describe("create", () => {
        ...
        const productCollectionService = {
            ...
            retrieve: (id) => Promise.resolve({ id: IdMap.getId("cat"), title: "Suits" })
        };
        ...
        it("successfully create a product", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledWith("product.created", expect.any(Object));
    ...
    describe("update", () => {
        ...
        it("successfully updates product metadata", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledTimes(1);
            expect(eventBusService.emit).toHaveBeenCalledWith("product.updated", expect.any(Object));
        ...
        it("throws on wrong variant in update", async () => {
            try {
                await productService.update(IdMap.getId("ironman&co"), {
                    variants: [{ id: IdMap.getId("yellow") }, { id: IdMap.getId("green") }]
                });
            } catch (err) {
                expect(err.message).toEqual(`Variant with id: ${IdMap.getId("yellow")} is not associated with this product`);
            }
        });
    });
    ...
    describe("addOption", () => {
        ...
        const productOptionRepository = MockRepository({
            create: () => Promise.resolve({ id: IdMap.getId("Material"), title: "Material" })
        });
        ...
        it("creates product option", async () => {
            ...
            expect(productVariantService.addOptionValue).toHaveBeenCalledWith(IdMap.getId("green"), IdMap.getId("Material"), "Default Value");
        ...
        it("throws on duplicate option", async () => {
            try {
                await productService.addOption(IdMap.getId("productWithVariantsFail"), "Color");
            } catch (err) {
                expect(err.message).toBe("An option with the title: Color already exists");
            }
        });
    });
    ...
    describe("reorderVariants", () => {
        ...
        it("reorders variants", async () => {
            await productService.reorderVariants(IdMap.getId("ironman"), [IdMap.getId("blue"), IdMap.getId("green")]);
        ...
        it("throws if a variant id is not in the products variants", async () => {
            try {
                await productService.reorderVariants(IdMap.getId("ironman"), [IdMap.getId("yellow"), IdMap.getId("blue")]);
            } catch (err) {
                expect(err.message).toEqual(`Product has no variant with id: ${IdMap.getId("yellow")}`);
            }
        });

        it("throws if order length and product variant lengths differ", async () => {
            try {
                await productService.reorderVariants(IdMap.getId("ironman"), [IdMap.getId("blue")]);
            } catch (err) {
                expect(err.message).toEqual(`Product variants and new variant order differ in length.`);
            }
        });
    });
    ...
    describe("updateOption", () => {
        ...
        const productOptionRepository = MockRepository({
            findOne: () => Promise.resolve({ id: IdMap.getId("color"), title: "Color" })
        });
        ...
        it("updates option title", async () => {
            await productService.updateOption(IdMap.getId("ironman"), IdMap.getId("color"), {
                title: "Suit color"
            });
        ...
        it("throws if option doesn't exist", async () => {
            try {
                await productService.updateOption(IdMap.getId("ironman"), IdMap.getId("material"), {
                    title: "Size"
                });
            } catch (err) {
                expect(err.message).toEqual(`Product has no option with id: ${IdMap.getId("material")}`);
            }
        });
    });
    ...
    describe("deleteOption", () => {
        ...
        it("deletes an option from a product", async () => {
            await productService.deleteOption(IdMap.getId("ironman"), IdMap.getId("color"));
        ...
        it("resolve if product option does not exist", async () => {
            await productService.deleteOption(IdMap.getId("ironman"), IdMap.getId("material"));
        ...
        it("throw if variant option values are not equal", async () => {
            try {
                ...
                expect(error.message).toBe("To delete an option, first delete all variants, such that when option is deleted, no duplicate variants will exist.");
            }
        });
    });
});
