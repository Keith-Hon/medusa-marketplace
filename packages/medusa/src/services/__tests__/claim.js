...
describe("ClaimService", () => {
    describe("create", () => {
        ...
        it("successfully creates a claim", async () => {
            ...
            expect(lineItemService.generate).toHaveBeenCalledWith("var_123", "order_region", 1);

            expect(inventoryService.confirmInventory).toHaveBeenCalledTimes(1);
            expect(inventoryService.confirmInventory).toHaveBeenCalledWith("var_123", 1);
            expect(withTransactionMock).toHaveBeenCalledWith("inventory");
            expect(inventoryService.adjustInventory).toHaveBeenCalledTimes(1);
            expect(inventoryService.adjustInventory).toHaveBeenCalledWith("var_123", -1);
        ...
        it("fails if replace and no additional items", async () => {
            ...
            ).rejects.toThrow(`Claims with type "replace" must have at least one additional item.`);
        });
        ...
        it("fails if replace and refund amount", async () => {
            ...
            ).rejects.toThrow(`Claim has type "replace" but must be type "refund" to have a refund_amount.`);
        });
        ...
        it("fails if additional items are not in stock", async () => {
            try {
                ...
                expect(e.message).toEqual(`Variant with id: var_123 does not have the required inventory`);
            }
        });
        it.each(
            [
    ...
    describe("createFulfillment", () => {
        const fulfillmentService = {
            createFulfillment: jest.fn((_, items) => Promise.resolve([{ id: "ful", items }])),
            withTransaction: function () {
        ...
        it("fails if no shipping_methods", async () => {
            claimRepo.setFindOne(() => Promise.resolve({ ...claim, shipping_methods: [] }));

            await expect(claimService.createFulfillment("claim_id", { meta: "data" })).rejects.toThrow(`Cannot fulfill a claim without a shipping method.`);
        });

        it("fails if claim is canceled", async () => {
            claimRepo.setFindOne(() => Promise.resolve({ ...claim, canceled_at: new Date() }));

            await expect(claimService.createFulfillment("claim_id", {})).rejects.toThrow("Canceled claim cannot be fulfilled");
        });

        it("fails if already fulfilled", async () => {
            claimRepo.setFindOne(() => Promise.resolve({ ...claim, fulfillment_status: "fulfilled" }));

            await expect(claimService.createFulfillment("claim_id", { meta: "data" })).rejects.toThrow(`The claim has already been fulfilled.`);
        });

        it("fails if type is refund", async () => {
            claimRepo.setFindOne(() => Promise.resolve({ ...claim, type: "refund" }));

            await expect(claimService.createFulfillment("claim_id", { meta: "data" })).rejects.toThrow(`Claims with the type "refund" can not be fulfilled`);
        });
    });
    ...
    describe("cancelFulfillment", () => {
        ...
        it("successfully cancels fulfillment and corrects claim status", async () => {
            ...
            expect(fulfillmentService.cancelFulfillment).toHaveBeenCalledWith(IdMap.getId("claim"));
        ...
        it("fails to cancel fulfillment when not related to a claim", async () => {
            await expect(claimService.cancelFulfillment(IdMap.getId("no-claim"))).rejects.toThrow(`Fufillment not related to a claim`);
        });
    });
    ...
    describe("processRefund", () => {
        ...
        it("fails when claim is canceled", async () => {
            await expect(claimService.processRefund(IdMap.getId("claim-id"))).rejects.toThrow("Canceled claim cannot be processed");
        });
    });
    ...
    describe("createShipment", () => {
        ...
        it("calls fulfillment service", async () => {
            ...
            expect(fulfillmentService.createShipment).toHaveBeenCalledWith("ful_123", ["track1234"], {
                metadata: {
                    meta: "data"
                },
                no_notification: false
        ...
        it("fails if claim is canceled", async () => {
            ...
            await expect(claimService.createShipment("claim", "ful_123", ["track1234"], {})).rejects.toThrow("Canceled claim cannot be fulfilled as shipped");
        });
    });
    ...
    describe("update", () => {
        ...
        it("fails when claim is canceled", async () => {
            await expect(claimService.update(IdMap.getId("claim-id"))).rejects.toThrow("Canceled claim cannot be updated");
        });
    });
    ...
    describe("cancel", () => {
        ...
        it("fails if fulfillment isn't canceled", async () => {
            await expect(claimService.cancel(IdMap.getId("fail-fulfillment"))).rejects.toThrow("All fulfillments must be canceled before the claim can be canceled");
        });

        it("fails if return isn't canceled", async () => {
            await expect(claimService.cancel(IdMap.getId("fail-return"))).rejects.toThrow("Return must be canceled before the claim can be canceled");
        });

        it("fails if associated with a refund", async () => {
            await expect(claimService.cancel(IdMap.getId("fail-refund"))).rejects.toThrow("Claim with a refund cannot be canceled");
        });
    });
});
