...
describe("RegionService", () => {
    describe("create", () => {
        ...
        it("throws if country already is in region", async () => {
            try {
                ...
                expect(error.message).toBe(`Denmark already exists in region ${IdMap.getId("dk-reg")}`);
            }
        });
    ...
    describe("validateFields_", () => {
        ...
        it("throws on invalid country code", async () => {
            await expect(regionService.validateFields_({ countries: ["ddd"] })).rejects.toThrow("Invalid country code");
        });

        it("throws on in use country code", async () => {
            await expect(regionService.validateFields_({ countries: ["DK"] })).rejects.toThrow(`Denmark already exists in region ${IdMap.getId("dk-reg")}`);
        });

        it("throws on unknown payment providers", async () => {
            await expect(regionService.validateFields_({ payment_providers: ["should_fail"] })).rejects.toThrow("Payment provider not found");
        });
    ...
    describe("addPaymentProvider", () => {
        ...
        it("successfully adds payment provider", async () => {
            await regionService.addPaymentProvider(IdMap.getId("region"), "default_provider");
            ...
            expect(regionRepository.save).toHaveBeenCalledWith({
                id: IdMap.getId("region"),
                payment_providers: [{ id: "sweden_provider" }, { id: "default_provider" }]
            });
        });

        it("resolves if exists", async () => {
            await regionService.addPaymentProvider(IdMap.getId("region"), "sweden_provider");
    ...
    describe("addFulfillmentProvider", () => {
        ...
        it("successfully adds payment provider", async () => {
            await regionService.addFulfillmentProvider(IdMap.getId("region"), "default_provider");
            ...
            expect(regionRepository.save).toHaveBeenCalledWith({
                id: IdMap.getId("region"),
                fulfillment_providers: [{ id: "sweden_provider" }, { id: "default_provider" }]
            });
        });

        it("resolves if exists", async () => {
            await regionService.addFulfillmentProvider(IdMap.getId("region"), "sweden_provider");
    ...
    describe("removePaymentProvider", () => {
        ...
        it("removes payment provider", async () => {
            await regionService.removePaymentProvider(IdMap.getId("region"), "sweden_provider");
    ...
    describe("removeFulfillmentProvider", () => {
        ...
        it("removes payment provider", async () => {
            await regionService.removeFulfillmentProvider(IdMap.getId("region"), "sweden_provider");
