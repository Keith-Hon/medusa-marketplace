...
describe("CartService", () => {
    ...
    describe("retrieve", () => {
        ...
        const cartRepository = MockRepository({
            findOneWithRelations: () => Promise.resolve({ id: IdMap.getId("emptyCart") })
        });
        beforeAll(async () => {
            jest.clearAllMocks();
        ...
        it("calls cart model functions", () => {
            ...
            expect(cartRepository.findOneWithRelations).toHaveBeenCalledWith(undefined, {
    ...
    describe("setMetadata", () => {
        ...
        it("calls updateOne with correct params", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledWith("cart.updated", expect.any(Object));
        ...
        it("throw error on invalid key type", async () => {
            ...
            try {
                ...
                expect(err.message).toEqual("Key type is invalid. Metadata keys must be strings");
            }
        });
    });
    ...
    describe("create", () => {
        ...
        it("successfully creates a cart", async () => {
            ...
            expect(eventBusService.emit).toHaveBeenCalledTimes(1);
            expect(eventBusService.emit).toHaveBeenCalledWith("cart.created", expect.any(Object));
    ...
    describe("addLineItem", () => {
        ...
        const inventoryService = {
            ...
            confirmInventory: jest.fn().mockImplementation((variantId, _quantity) => {
                if (variantId !== IdMap.getId("cannot-cover")) {
                    ...
                    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `Variant with id: ${variantId} does not have the required inventory`);
                }
            })
        };
        ...
        it("creates a new line item and emits a created event", async () => {
            ...
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledWith(expect.objectContaining({ id: IdMap.getId("emptyCart") }));
        });
        ...
        it("successfully merges existing line item", async () => {
            ...
            expect(lineItemService.update).toHaveBeenCalledWith(IdMap.getId("merger"), {
                quantity: 2
            });
            ...
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledTimes(1);
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledWith(expect.objectContaining({ id: IdMap.getId("cartWithLine") }));
        });
        ...
        it("throws if inventory isn't covered", async () => {
            ...
            await expect(cartService.addLineItem(IdMap.getId("cartWithLine"), lineItem)).rejects.toThrow(
                `Variant with id: ${IdMap.getId("cannot-cover")} does not have the required inventory`
            );
        });
    ...
    describe("addLineItem w. SalesChannel", () => {
        ...
        it("validates if cart and variant's product belong to the same sales channel if flag is passed", async () => {
            const validateSpy = jest.spyOn(cartService, "validateLineItem").mockImplementation(() => Promise.resolve(true));
    ...
    describe("removeLineItem", () => {
        ...
        it("successfully removes a line item", async () => {
            await cartService.removeLineItem(IdMap.getId("cartWithLine"), IdMap.getId("itemToRemove"));

            expect(lineItemService.delete).toHaveBeenCalledTimes(1);
            expect(lineItemService.delete).toHaveBeenCalledWith(IdMap.getId("itemToRemove"));
            ...
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledTimes(1);
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledWith(
        ...
        it("removes shipping method if not necessary", async () => {
            await cartService.removeLineItem(IdMap.getId("withShipping"), IdMap.getId("itemToRemove"));

            expect(shippingOptionService.deleteShippingMethods).toHaveBeenCalledTimes(1);
        ...
        it("resolves if line item is not in cart", async () => {
            await cartService.removeLineItem(IdMap.getId("cartWithLine"), IdMap.getId("nonExisting"));
    ...
    describe("updateLineItem", () => {
        ...
        const inventoryService = {
            ...InventoryServiceMock,
            confirmInventory: jest.fn().mockImplementation((id) => id !== IdMap.getId("cannot-cover"))
        };
        ...
        it("successfully updates existing line item", async () => {
            await cartService.updateLineItem(IdMap.getId("cartWithLine"), IdMap.getId("existing"), { quantity: 2 });

            expect(eventBusService.emit).toHaveBeenCalledTimes(1);
            expect(eventBusService.emit).toHaveBeenCalledWith("cart.updated", expect.any(Object));

            expect(lineItemService.update).toHaveBeenCalledTimes(1);
            expect(lineItemService.update).toHaveBeenCalledWith(IdMap.getId("existing"), { quantity: 2 });
        ...
        it("throws if inventory isn't covered", async () => {
            await expect(cartService.updateLineItem(IdMap.getId("cannot"), IdMap.getId("existing"), { quantity: 2 })).rejects.toThrow(
    ...
    describe("updateEmail", () => {
        ...
        it("throws on invalid email", async () => {
            await expect(cartService.update(IdMap.getId("emptyCart"), { email: "test@test" })).rejects.toThrow("The email is not valid");
        });
    });
    ...
    describe("setRegion", () => {
        ...
        const priceSelectionStrat = {
            ...
            calculateVariantPrice: async (variantId, context) => {
                if (variantId === IdMap.getId("fail")) {
                    throw new MedusaError(MedusaError.Types.NOT_FOUND, `Money amount for variant with id ${variantId} in region ${context.region_id} does not exist`);
                } else {
        ...
        it("successfully set new region", async () => {
            ...
            expect(lineItemService.update).toHaveBeenCalledTimes(1);
            expect(lineItemService.update).toHaveBeenCalledWith(IdMap.getId("testitem"), {
                unit_price: 100,
                has_shipping: false
            });
    ...
    describe("setPaymentSession", () => {
        ...
        it("successfully sets a payment method", async () => {
            await cartService.setPaymentSession(IdMap.getId("cartWithLine"), "test-provider");
        ...
        it("fails if the region does not contain the provider_id", async () => {
            await expect(cartService.setPaymentSession(IdMap.getId("cartWithLine"), "unknown")).rejects.toThrow(`The payment method is not available in this region`);
        });
    });
    ...
    describe("setPaymentSessions", () => {
        ...
        const cart3 = {
            total: 100,
            payment_sessions: [{ provider_id: "provider_1" }, { provider_id: "not_in_region" }],
            region: {
                payment_providers: [{ id: "provider_1" }, { id: "provider_2" }]
            }
        };
        ...
        const cart4 = {
            total: 0,
            payment_sessions: [{ provider_id: "provider_1" }, { provider_id: "provider_2" }],
            region: {
                payment_providers: [{ id: "provider_1" }, { id: "provider_2" }]
            }
        };
        ...
        it("initializes payment sessions for each of the providers", async () => {
            ...
            expect(paymentProviderService.createSession).toHaveBeenCalledWith("provider_1", cart1);
            expect(paymentProviderService.createSession).toHaveBeenCalledWith("provider_2", cart1);
        });
    ...
    describe("findCustomShippingOption", () => {
        ...
        it("given a cart with custom shipping options and a shipping option id corresponding to a custom shipping option, then it should return a custom shipping option", async () => {
            const cartCSO = [{ id: "cso-test", shipping_option_id: "test-so", price: 20 }];
        ...
        it("given a cart with custom shipping options and a shipping option id that does not belong to the cart, then it should throw an invalid error", async () => {
            const cartCSO = [{ id: "cso-test", shipping_option_id: "test-so", price: 500 }];
    ...
    describe("addShippingMethod", () => {
        ...
        it("successfully adds the shipping method", async () => {
            ...
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledWith(IdMap.getId("option"), data, { cart: cart1 });
        });
        ...
        it("successfully overrides existing profile shipping method", async () => {
            ...
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledWith(IdMap.getId("profile1"), data, { cart: cart2 });
            expect(shippingOptionService.deleteShippingMethods).toHaveBeenCalledWith({
        ...
        it("successfully adds additional shipping method", async () => {
            ...
            expect(shippingOptionService.deleteShippingMethods).toHaveBeenCalledTimes(0);
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledTimes(1);
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledWith(IdMap.getId("additional"), data, { cart: cart2 });
        });
        ...
        it("updates item shipping", async () => {
            ...
            expect(shippingOptionService.deleteShippingMethods).toHaveBeenCalledTimes(0);
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledTimes(1);
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledWith(IdMap.getId("profile1"), data, { cart: cart3 });
        ...
        it("successfully adds a shipping method from a custom shipping option and custom price", async () => {
            ...
            cartService.findCustomShippingOption = jest.fn().mockImplementation((cartCustomShippingOptions) => {
                return {
                    price: 0
                };
            });
            ...
            expect(shippingOptionService.createShippingMethod).toHaveBeenCalledWith(IdMap.getId("test-so"), data, {
                cart_id: IdMap.getId("cart-with-custom-so"),
                price: 0
            });
        });
    ...
    describe("applyDiscount", () => {
        ...
        const discountService = {
            ...
            canApplyForCustomer: jest.fn().mockImplementation((ruleId, customerId) => {
                if (ruleId === "test-rule") {
            ...
            validateDiscountForCartOrThrow: jest.fn().mockImplementation((cart, discount) => {
        ...
        it("successfully applies discount to cart", async () => {
            ...
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledTimes(1);
            expect(LineItemAdjustmentServiceMock.createAdjustments).toHaveBeenCalledWith(expect.objectContaining({ id: IdMap.getId("cart") }));
        });
