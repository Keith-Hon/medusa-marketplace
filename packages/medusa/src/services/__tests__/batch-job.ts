    ...
    describe("update status", () => {
        describe("confirm", () => {
            it("should be able to confirm_processing a batch job to emit the processing event", async () => {
                ...
                expect(eventBusServiceMock.emit).toHaveBeenCalledWith(BatchJobService.Events.CONFIRMED, { id: batchJobId_1 });
            });
        ...
        describe("complete", () => {
            it("should be able to complete a batch job", async () => {
                ...
                expect(eventBusServiceMock.emit).toHaveBeenCalledWith(BatchJobService.Events.COMPLETED, { id: batchJobId_1 });
        ...
        describe("pre processed", () => {
            it("should be able to mark as pre processed a batch job in dry_run", async () => {
                ...
                expect(eventBusServiceMock.emit).toHaveBeenCalledWith(BatchJobService.Events.PRE_PROCESSED, { id: batchJobId_1 });
            });
            ...
            it("should be able to mark as completed a batch job that has been pre processed but not in dry_run", async () => {
                ...
                expect(eventBusServiceMock.emit).toHaveBeenCalledWith(BatchJobService.Events.PRE_PROCESSED, { id: batchJobId_1 });
                expect(eventBusServiceMock.emit).toHaveBeenLastCalledWith(BatchJobService.Events.CONFIRMED, { id: batchJobId_1 });
            });
        });
        ...
        describe("cancel", () => {
            it("should be able to cancel a batch job", async () => {
                ...
                expect(eventBusServiceMock.emit).toHaveBeenCalledWith(BatchJobService.Events.CANCELED, { id: batchJobId_1 });
            });
        ...
        describe("processing", () => {
            it("should be able to mark as processing a batch job", async () => {
                ...
                expect(eventBusServiceMock.emit).toHaveBeenCalledWith(BatchJobService.Events.PROCESSING, { id: batchJobId_1 });
            });
            ...
            it("should not be able to mark as processing a batch job with the wrong status", async () => {
                ...
                const err = await batchJobService.setProcessing(batchJob).catch((e) => e);
                expect(err).toBeTruthy();
