...
describe("DiscountService", () => {
    ...
    describe("retrieve", () => {
        ...
        it("throws on invalid discount id", async () => {
            try {
                ...
                expect(error.message).toBe(`Discount with ${IdMap.getId("invalid")} was not found`);
            }
        });
    });
    ...
    describe("addRegion", () => {
        ...
        it("successfully adds a region", async () => {
            await discountService.addRegion(IdMap.getId("total10"), IdMap.getId("test-region-2"));

            expect(discountRepository.save).toHaveBeenCalledTimes(1);
            expect(discountRepository.save).toHaveBeenCalledWith({
                id: IdMap.getId("total10"),
                regions: [{ id: IdMap.getId("test-region") }, { id: IdMap.getId("test-region-2") }],
                rule: {
                    type: "percentage"
                }
            });
        ...
        it("successfully resolves if region already exists", async () => {
            await discountService.addRegion(IdMap.getId("total10"), IdMap.getId("test-region"));
    ...
    describe("removeRegion", () => {
        ...
        it("successfully removes a region", async () => {
            await discountService.removeRegion(IdMap.getId("total10"), IdMap.getId("test-region"));
        ...
        it("successfully resolve if region does not exist", async () => {
            await discountService.removeRegion(IdMap.getId("total10"), IdMap.getId("test-region-2"));
    ...
    describe("listAndCount", () => {
        ...
        it("calls repository function specified query", async () => {
            await discountService.listAndCount({}, { skip: 50, take: 50, order: { created_at: "ASC" } });
    ...
    describe("calculateDiscountForLineItem", () => {
        ...
        it("correctly calculates fixed + item discount", async () => {
            const adjustment = await discountService.calculateDiscountForLineItem("disc_fixed", {
                unit_price: 300,
                quantity: 2,
        ...
        it("correctly calculates fixed + total discount", async () => {
            const adjustment1 = await discountService.calculateDiscountForLineItem("disc_fixed_total", {
                unit_price: 400,
                quantity: 2,
                allow_discounts: true
            });

            const adjustment2 = await discountService.calculateDiscountForLineItem("disc_fixed_total", {
                unit_price: 300,
                quantity: 1,
                allow_discounts: true
            });
        ...
        it("correctly calculates percentage discount", async () => {
            const adjustment = await discountService.calculateDiscountForLineItem("disc_percentage", {
                unit_price: 400,
                quantity: 2,
                allow_discounts: true
            });
    ...
    describe("validateDiscountForCartOrThrow", () => {
        ...
        beforeEach(async () => {
            ...
            const isValidForRegionMock = jest.fn().mockImplementation(() => Promise.resolve(true));
        ...
        it("calls all validation methods when cart includes a customer_id and doesn't throw", async () => {
            ...
            const result = await discountService.validateDiscountForCartOrThrow(cart, discount);
            ...
            expect(discountService.isValidForRegion).toHaveBeenCalledWith(discount, cart.region_id);

            expect(discountService.canApplyForCustomer).toHaveBeenCalledTimes(1);
            expect(discountService.canApplyForCustomer).toHaveBeenCalledWith(discount.rule.id, cart.customer_id);
        });
        ...
        it("throws when hasReachedLimit returns true", async () => {
            ...
            expect(discountService.validateDiscountForCartOrThrow(cart, discount)).rejects.toThrow({
        ...
        it("throws when isValidForRegion returns false", async () => {
            const discount = {};
            const cart = getCart("with-d-and-customer");
            discountService.isValidForRegion = jest.fn().mockImplementation(() => Promise.resolve(false));
        ...
        it("throws when canApplyForCustomer returns false", async () => {
            ...
            const cart = getCart("with-d-and-customer");
            discountService.canApplyForCustomer = jest.fn().mockImplementation(() => Promise.resolve(false));
    ...
    describe("isValidForRegion", () => {
        ...
        it("returns false if discount is not available in a given region", async () => {
            ...
            const isValidForRegion = await discountService.isValidForRegion(discount, "dk");
        ...
        it("returns true if discount is available in a given region", async () => {
            ...
            const isValidForRegion = await discountService.isValidForRegion(discount, "us");
    ...
    describe("canApplyForCustomer", () => {
        const discountConditionRepository = {
            canApplyForCustomer: jest.fn().mockImplementation(() => Promise.resolve(true))
        };
        ...
        it("returns false on undefined customer id", async () => {
            ...
            expect(discountConditionRepository.canApplyForCustomer).toHaveBeenCalledTimes(0);
        });

        it("returns true on customer with groups", async () => {
            const res = await discountService.canApplyForCustomer("rule-1", "customer-with-groups");

            expect(res).toBe(true);

            expect(discountConditionRepository.canApplyForCustomer).toHaveBeenCalledTimes(1);
            expect(discountConditionRepository.canApplyForCustomer).toHaveBeenCalledWith("rule-1", "customer-with-groups");
        });
    });
});
