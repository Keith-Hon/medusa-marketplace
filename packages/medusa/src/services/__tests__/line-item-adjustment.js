...
describe("LineItemAdjustmentService", () => {
    ...
    describe("retrieve", () => {
        ...
        it("fails when lineItemAdjustment is not found", async () => {
            await expect(lineItemAdjustmentService.retrieve("not-existing")).rejects.toThrow(`Line item adjustment with id: not-existing was not found`);
        });
    });
    ...
    describe("delete", () => {
        ...
        describe("delete by line item adjustment id", () => {
            ...
            it("calls lineItemAdjustment delete method with the right params", async () => {
                ...
                expect(lineItemAdjustmentRepo.remove).toHaveBeenCalledWith(lineItemAdjustment);
            });
        });
    ...
    describe("createAdjustments", () => {
        ...
        lineItemAdjustmentService.createAdjustmentForLineItem = jest.fn().mockImplementation(() => {
            return Promise.resolve({
                item_id: "li-1",
                amount: 1000,
                discount_id: "disc-1",
        ...
        it("calls createAdjustmentForLineItem once when given a line item", () => {
            ...
            expect(lineItemAdjustmentService.createAdjustmentForLineItem).toHaveBeenCalledTimes(1);
            expect(lineItemAdjustmentService.createAdjustmentForLineItem).toHaveBeenCalledWith(cart, lineItem);
        });
        ...
        it("calls createAdjustmentForLineItem 3 times when given a cart containing 3 line items", () => {
            ...
            expect(lineItemAdjustmentService.createAdjustmentForLineItem).toHaveBeenCalledTimes(3);
            expect(lineItemAdjustmentService.createAdjustmentForLineItem).toHaveBeenNthCalledWith(1, cart, { id: "li-2" });
        });
    });
});
