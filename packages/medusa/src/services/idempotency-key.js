...
class IdempotencyKeyService extends TransactionBaseService {
    ...
    async create(payload) {
        return this.atomicPhase_(async (manager) => {
            const idempotencyKeyRepo = manager.getCustomRepository(this.idempotencyKeyRepository_);
    ...
    async retrieve(idempotencyKey) {
        const idempotencyKeyRepo = this.manager_.getCustomRepository(this.idempotencyKeyRepository_);
    ...
    async workStage(idempotencyKey, func) {
        try {
            return await this.atomicPhase_(async (manager) => {
                ...
                const { recovery_point, response_code, response_body } = await func(manager);
