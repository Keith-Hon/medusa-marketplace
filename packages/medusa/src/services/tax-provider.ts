...
import { ITaxService, ItemTaxCalculationLine, TaxCalculationContext, TransactionBaseService } from "../interfaces";
...
class TaxProviderService extends TransactionBaseService<TaxProviderService> {
    ...
    retrieveProvider(region: Region): ITaxService {
        ...
        if (!provider) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Could not find a tax provider with id: ${region.tax_provider_id}`);
        }
    ...
    async clearTaxLines(cartId: string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager) => {
            const taxLineRepo = transactionManager.getCustomRepository(this.taxLineRepo_);
            const shippingTaxRepo = transactionManager.getCustomRepository(this.smTaxLineRepo_);

            await Promise.all([taxLineRepo.deleteForCart(cartId), shippingTaxRepo.deleteForCart(cartId)]);
        });
    }
     ...
     */
    async createTaxLines(cartOrLineItems: Cart | LineItem[], calculationContext: TaxCalculationContext): Promise<(ShippingMethodTaxLine | LineItemTaxLine)[]> {
        return await this.atomicPhase_(async (transactionManager) => {
            let taxLines: (ShippingMethodTaxLine | LineItemTaxLine)[] = [];
            if (isCart(cartOrLineItems)) {
                taxLines = await this.getTaxLines(cartOrLineItems.items, calculationContext);
            } else {
                taxLines = await this.getTaxLines(cartOrLineItems, calculationContext);
            }

            const itemTaxLineRepo = transactionManager.getCustomRepository(this.taxLineRepo_);
            const shippingTaxLineRepo = transactionManager.getCustomRepository(this.smTaxLineRepo_);
            ...
            return (await Promise.all([itemTaxLineRepo.upsertLines(lineItems), shippingTaxLineRepo.upsertLines(shipping)])).flat();
        });
    }
     ...
     * @return the newly created tax lines
     */
    async createShippingTaxLines(shippingMethod: ShippingMethod, calculationContext: TaxCalculationContext): Promise<(ShippingMethodTaxLine | LineItemTaxLine)[]> {
        return await this.atomicPhase_(async (transactionManager) => {
            const taxLines = await this.getShippingTaxLines(shippingMethod, calculationContext);
     ...
     */
    async getShippingTaxLines(shippingMethod: ShippingMethod, calculationContext: TaxCalculationContext): Promise<ShippingMethodTaxLine[]> {
        const calculationLines = [
            {
                shipping_method: shippingMethod,
                rates: await this.getRegionRatesForShipping(shippingMethod.shipping_option_id, calculationContext.region)
            }
        ];

        const taxProvider = this.retrieveProvider(calculationContext.region);
        const providerLines = await taxProvider.getTaxLines([], calculationLines, calculationContext);
        ...
        return providerLines.map((pl) => {
            if (!("shipping_method_id" in pl)) {
                throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Expected only shipping method tax lines");
            }
     ...
     * @return the computed tax lines
     */
    async getTaxLines(lineItems: LineItem[], calculationContext: TaxCalculationContext): Promise<(ShippingMethodTaxLine | LineItemTaxLine)[]> {
        const calculationLines = await Promise.all(
            lineItems.map(async (l) => {
                ...
                if (l.variant && l.variant.product_id) {
                    return {
                        item: l,
                        rates: await this.getRegionRatesForProduct(l.variant.product_id, calculationContext.region)
                    };
                }
        ...
        const shippingCalculationLines = await Promise.all(
            calculationContext.shipping_methods.map(async (sm) => {
                return {
                    shipping_method: sm,
                    rates: await this.getRegionRatesForShipping(sm.shipping_option_id, calculationContext.region)
                };
            })
        );

        const taxProvider = this.retrieveProvider(calculationContext.region);
        const providerLines = await taxProvider.getTaxLines(calculationLines.filter((v) => v !== null) as ItemTaxCalculationLine[], shippingCalculationLines, calculationContext);
        ...
        return providerLines.map((pl) => {
            ...
            if (!("item_id" in pl)) {
                throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Tax Provider returned invalid tax lines");
            }
     ...
     */
    async getRegionRatesForShipping(optionId: string, regionDetails: RegionDetails): Promise<TaxServiceRate[]> {
        ...
        const optionRates = await this.taxRateService_.withTransaction(this.manager_).listByShippingOption(optionId);
     ...
     * @return the tax rates configured for the shipping option.
     */
    async getRegionRatesForProduct(productId: string, region: RegionDetails): Promise<TaxServiceRate[]> {
        ...
        let toReturn: TaxServiceRate[] = [];
        const productRates = await this.taxRateService_.withTransaction(this.manager_).listByProduct(productId, {
            region_id: region.id
        });
     ...
     */
    private async setCache(productId: string, regionId: string, value: TaxServiceRate[]): Promise<void> {
        const cacheKey = this.getCacheKey(productId, regionId);
        return await this.redis_.set(cacheKey, JSON.stringify(value), "EX", CACHE_TIME);
    }
     ...
     */
    private async getCacheEntry(productId: string, regionId: string): Promise<TaxServiceRate[] | null> {
        const cacheKey = this.getCacheKey(productId, regionId);
