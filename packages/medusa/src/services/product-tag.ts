...
class ProductTagService extends BaseService {
    ...
    async retrieve(tagId: string, config: FindConfig<ProductTag> = {}): Promise<ProductTag> {
        ...
        if (!tag) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Product tag with id: ${tagId} was not found`);
        }
     ...
     */
    async list(selector: FilterableProductTagProps = {}, config: FindConfig<ProductTag> = { skip: 0, take: 20 }): Promise<ProductTag[]> {
        const tagRepo = this.manager_.getCustomRepository(this.tagRepo_);
     ...
     * @return {Promise} the result of the find operation
     */
    async listAndCount(selector: FilterableProductTagProps = {}, config: FindConfig<ProductTag> = { skip: 0, take: 20 }): Promise<[ProductTag[], number]> {
        const tagRepo = this.manager_.getCustomRepository(this.tagRepo_);
