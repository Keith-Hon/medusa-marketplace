...
import { Product, ProductTag, ProductType, ProductVariant, SalesChannel } from "../models";
...
class ProductService extends TransactionBaseService<ProductService, InjectedDependencies> {
    ...
    async list(
        ...
        if (q) {
            const [products] = await productRepo.getFreeTextSearchResultsAndCount(q, query, relations);
            return products;
        }
    ...
    async listAndCount(
        ...
        if (q) {
            return await productRepo.getFreeTextSearchResultsAndCount(q, query, relations);
        }
     ...
     * @return the result of the find one operation.
     */
    async retrieveByHandle(productHandle: string, config: FindProductConfig = {}): Promise<Product> {
     ...
     * @return the result of the find one operation.
     */
    async retrieveByExternalId(externalId: string, config: FindProductConfig = {}): Promise<Product> {
    ...
    async retrieve_(
        ...
        const product = await productRepo.findOneWithRelations(relations, query as FindWithoutRelationsOptions);
        ...
        if (!product) {
            ...
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Product with ${selectorConstraints} was not found`);
        }
    ...
    async filterProductsBySalesChannel(
        ...
        const productSalesChannelsMap = new Map<string, SalesChannel[]>(products.map((product) => [product.id, product.sales_channels]));
        return products.filter((product) => {
            return productSalesChannelsMap.get(product.id)?.some((sc) => sc.id === salesChannelId);
        });
    }

    async listTypes(): Promise<ProductType[]> {
        const manager = this.manager_;
        const productTypeRepository = manager.getCustomRepository(this.productTypeRepository_);
    ...
    async create(productObject: CreateProductInput): Promise<Product> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const { options, tags, type, images, sales_channels: salesChannels, ...rest } = productObject;
            ...
            try {
                ...
                if (this.featureFlagRouter_.isFeatureEnabled(SalesChannelFeatureFlag.key)) {
                    if (typeof salesChannels !== "undefined") {
                        ...
                        if (salesChannels?.length) {
                            ...
                            product.sales_channels = salesChannelIds?.map((id) => ({ id } as SalesChannel));
                        }
                    }
                }
                ...
                await this.eventBus_.withTransaction(manager).emit(ProductService.Events.CREATED, {
                    id: result.id
                });
                return result;
            } catch (error) {
    ...
    async update(productId: string, update: UpdateProductInput): Promise<Product> {
        return await this.atomicPhase_(async (manager) => {
            const productRepo = manager.getCustomRepository(this.productRepository_);
            const productVariantRepo = manager.getCustomRepository(this.productVariantRepository_);
            const productTagRepo = manager.getCustomRepository(this.productTagRepository_);
            const productTypeRepo = manager.getCustomRepository(this.productTypeRepository_);
            const imageRepo = manager.getCustomRepository(this.imageRepository_);
            ...
            if (this.featureFlagRouter_.isFeatureEnabled(SalesChannelFeatureFlag.key)) {
                ...
                if (typeof update.sales_channels !== "undefined") {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, "the property sales_channels should no appears as part of the payload");
                }
            }
            ...
            const { variants, metadata, images, tags, type, sales_channels: salesChannels, ...rest } = update;
            ...
            if (variants) {
                ...
                for (const [i, newVariant] of variants.entries()) {
                    ...
                    if (newVariant.id) {
                        ...
                        if (!variant) {
                            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Variant with id: ${newVariant.id} is not associated with this product`);
                        }

                        const saved = await this.productVariantService_.withTransaction(manager).update(variant, {
                            ...newVariant,
                            variant_rank,
                        ...
                        // should be created
                        const created = await this.productVariantService_.withTransaction(manager).create(product.id, {
                            ...newVariant,
                            variant_rank,
            ...
            await this.eventBus_.withTransaction(manager).emit(ProductService.Events.UPDATED, {
                id: result.id,
    ...
    async delete(productId: string): Promise<void> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const product = await productRepo.findOne({ id: productId }, { relations: ["variants", "variants.prices", "variants.options"] });
            ...
            await this.eventBus_.withTransaction(manager).emit(ProductService.Events.DELETED, {
                id: productId
            });
    ...
    async addOption(productId: string, optionTitle: string): Promise<Product> {
        return await this.atomicPhase_(async (manager) => {
            const productOptionRepo = manager.getCustomRepository(this.productOptionRepository_);
            ...
            if (product.options.find((o) => o.title === optionTitle)) {
                throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, `An option with the title: ${optionTitle} already exists`);
            }
            ...
            for (const variant of product.variants) {
                this.productVariantService_.withTransaction(manager).addOptionValue(variant.id, option.id, "Default Value");
            }

            const result = await this.retrieve(productId);

            await this.eventBus_.withTransaction(manager).emit(ProductService.Events.UPDATED, result);
            return result;
        });
    }

    async reorderVariants(productId: string, variantOrder: string[]): Promise<Product> {
        return await this.atomicPhase_(async (manager) => {
            const productRepo = manager.getCustomRepository(this.productRepository_);
            ...
            if (product.variants.length !== variantOrder.length) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `Product variants and new variant order differ in length.`);
            }

            product.variants = variantOrder.map((vId) => {
                const variant = product.variants.find((v) => v.id === vId);
                if (!variant) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Product has no variant with id: ${vId}`);
                }
     ...
     */
    async updateOption(productId: string, optionId: string, data: ProductOptionInput): Promise<Product> {
        return await this.atomicPhase_(async (manager) => {
            const productOptionRepo = manager.getCustomRepository(this.productOptionRepository_);
            ...
            const optionExists = product.options.some((o) => o.title.toUpperCase() === title.toUpperCase() && o.id !== optionId);
            if (optionExists) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `An option with title ${title} already exists`);
            }
            ...
            if (!productOption) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `Option with id: ${optionId} does not exist`);
            }
            ...
            await this.eventBus_.withTransaction(manager).emit(ProductService.Events.UPDATED, product);
            return product;
        });
    }
     ...
     * @return the updated product
     */
    async deleteOption(productId: string, optionId: string): Promise<Product | void> {
        return await this.atomicPhase_(async (manager) => {
            const productOptionRepo = manager.getCustomRepository(this.productOptionRepository_);
            ...
            const valueToMatch = firstVariant.options.find((o) => o.option_id === optionId)?.value;
