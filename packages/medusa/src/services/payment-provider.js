...
class PaymentProviderService extends BaseService {
    ...
    async retrievePayment(id, relations = []) {
        const paymentRepo = this.manager_.getCustomRepository(this.paymentRepository_);
        ...
        if (!payment) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Payment with ${id} was not found`);
        }

        return payment;
    }

    listPayments(selector, config = { skip: 0, take: 50, order: { created_at: "DESC" } }) {
    ...
    async retrieveSession(id, relations = []) {
        const sessionRepo = this.manager_.getCustomRepository(this.paymentSessionRepository_);
        const validatedId = this.validateId_(id);
        ...
        if (!session) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Payment Session with ${id} was not found`);
        }
    ...
    async createSession(providerId, cart) {
        return this.atomicPhase_(async (manager) => {
            ...
            const sessionRepo = manager.getCustomRepository(this.paymentSessionRepository_);
    ...
    deleteSession(paymentSession) {
        return this.atomicPhase_(async (manager) => {
            const session = await this.retrieveSession(paymentSession.id).catch((_) => undefined);
    ...
    retrieveProvider(providerId) {
        try {
            ...
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Could not find a payment provider with id: ${providerId}`);
        }
    }
    ...
    async authorizePayment(paymentSession, context) {
        return this.atomicPhase_(async (manager) => {
            ...
            const provider = this.retrieveProvider(paymentSession.provider_id);
            const { status, data } = await provider.withTransaction(manager).authorizePayment(session, context);
    ...
    async refundPayment(payObjs, amount, reason, note) {
        return this.atomicPhase_(async (manager) => {
            ...
            if (refundable < amount) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Refund amount is too high");
            }
            ...
            while (toRefund) {
                ...
                if (balance > 0) {
                    toRefund = payments.find((p) => p.amount - p.amount_refunded > 0 && !used.includes(p.id));
                } else {
                    toRefund = null;
                }
            }
    ...
    async retrieveRefund(id, config = {}) {
        ...
        if (!refund) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `A refund with ${id} was not found`);
        }
