...
class DiscountConditionService extends TransactionBaseService<DiscountConditionService> {
    ...
    async retrieve(conditionId: string, config?: FindConfig<DiscountCondition>): Promise<DiscountCondition | never> {
        const manager = this.manager_;
        const conditionRepo = manager.getCustomRepository(this.discountConditionRepository_);
        ...
        if (!condition) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `DiscountCondition with id ${conditionId} was not found`);
        }
        ...
        return await this.atomicPhase_(
            async (manager: EntityManager) => {
                ...
                if (!resolvedConditionType) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Missing one of products, collections, tags, types or customer groups in data`);
                }
                ...
                if (data.id) {
                    ...
                    return await discountConditionRepo.addConditionResources(data.id, resolvedConditionType.resource_ids, resolvedConditionType.type, true);
                }
                ...
                return await discountConditionRepo.addConditionResources(discountCondition.id, resolvedConditionType.resource_ids, resolvedConditionType.type);
            },
