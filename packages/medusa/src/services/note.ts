...
class NoteService extends TransactionBaseService<NoteService> {
     ...
     */
    async retrieve(id: string, config: FindConfig<Note> = {}): Promise<Note | never> {
        ...
        if (!note) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Note with id: ${id} was not found.`);
        }
     ...
     */
    async create(data: CreateNoteInput, config: { metadata: Record<string, unknown> } = { metadata: {} }): Promise<Note> {
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(NoteService.Events.CREATED, { id: result.id });
    ...
    async update(noteId: string, value: string): Promise<Note> {
        return await this.atomicPhase_(async (manager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(NoteService.Events.UPDATED, { id: result.id });
    ...
    async delete(noteId: string): Promise<void> {
        return await this.atomicPhase_(async (manager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(NoteService.Events.DELETED, { id: noteId });
        });
    }
}
