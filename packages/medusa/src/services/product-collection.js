...
class ProductCollectionService extends BaseService {
    ...
    async retrieve(collectionId, config = {}) {
        const collectionRepo = this.manager_.getCustomRepository(this.productCollectionRepository_);
        ...
        if (!collection) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Product collection with id: ${collectionId} was not found`);
        }
    ...
    async retrieveByHandle(collectionHandle, config = {}) {
        ...
        if (!collection) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Product collection with handle: ${collectionHandle} was not found`);
        }
    ...
    async create(collection) {
        return this.atomicPhase_(async (manager) => {
            const collectionRepo = manager.getCustomRepository(this.productCollectionRepository_);
    ...
    async delete(collectionId) {
        return this.atomicPhase_(async (manager) => {
            const productCollectionRepo = manager.getCustomRepository(this.productCollectionRepository_);
    ...
    async list(selector = {}, config = { skip: 0, take: 20 }) {
        const productCollectionRepo = this.manager_.getCustomRepository(this.productCollectionRepository_);
