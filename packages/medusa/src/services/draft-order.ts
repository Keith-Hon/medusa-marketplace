...
class DraftOrderService extends TransactionBaseService<DraftOrderService> {
     ...
     */
    async retrieve(id: string, config: FindConfig<DraftOrder> = {}): Promise<DraftOrder | never> {
        const manager = this.manager_;
        const draftOrderRepo = manager.getCustomRepository(this.draftOrderRepository_);

        const query = buildQuery({ id }, config);
        const draftOrder = await draftOrderRepo.findOne(query);
        if (!draftOrder) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Draft order with ${id} was not found`);
        }
     ...
     * @return the draft order
     */
    async retrieveByCartId(cartId: string, config: FindConfig<DraftOrder> = {}): Promise<DraftOrder | never> {
        const manager = this.manager_;
        const draftOrderRepo = manager.getCustomRepository(this.draftOrderRepository_);
        ...
        const draftOrder = await draftOrderRepo.findOne(query);
        if (!draftOrder) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Draft order was not found`);
        }
    ...
    async listAndCount(
        ...
        const manager = this.manager_;
        const draftOrderRepository = manager.getCustomRepository(this.draftOrderRepository_);

        const { q, ...restSelector } = selector;
        const query = buildQuery(restSelector, config) as FindManyOptions<DraftOrder> & ExtendedFindConfig<DraftOrder>;
    ...
    async create(data: AdminPostDraftOrdersReq): Promise<DraftOrder> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (!data.region_id) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `region_id is required to create a draft order`);
            }

            if (!data.items || !data.items.length) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `Items are required to create a draft order`);
            }

            const { shipping_methods, no_notification_order, items, ...rawCart } = data;
            ...
            if (rawCart.discounts) {
                ...
                for (const { code } of discounts) {
                    await this.cartService_.withTransaction(transactionManager).applyDiscount(rawCart as Cart, code);
                }
            }

            const createdCart = await this.cartService_.withTransaction(transactionManager).create({ type: CartType.DRAFT_ORDER, ...rawCart });
            ...
            await this.eventBus_.withTransaction(transactionManager).emit(DraftOrderService.Events.CREATED, {
                id: result.id
            });

            for (const item of items) {
                if (item.variant_id) {
                    const line = await this.lineItemService_.withTransaction(transactionManager).generate(item.variant_id, data.region_id, item.quantity, {
            ...
            for (const method of shipping_methods) {
                await this.cartService_.withTransaction(transactionManager).addShippingMethod(createdCart.id, method.option_id, method.data);
            }
     ...
     */
    async registerCartCompletion(draftOrderId: string, orderId: string): Promise<UpdateResult> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const draftOrderRepo = transactionManager.getCustomRepository(this.draftOrderRepository_);
    ...
    async update(id: string, data: { no_notification_order: boolean }): Promise<DraftOrder> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (draftOrder.status === DraftOrderStatus.COMPLETED) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't update a draft order which is complete");
            }
            ...
            if (touched) {
                ...
                await this.eventBus_.withTransaction(transactionManager).emit(DraftOrderService.Events.UPDATED, {
                    id: draftOrder.id
                });
            }
