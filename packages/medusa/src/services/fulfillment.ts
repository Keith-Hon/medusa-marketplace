...
class FulfillmentService extends TransactionBaseService<FulfillmentService> {
     ...
     */
    async getFulfillmentItems_(order: CreateFulfillmentOrder, items: FulFillmentItemType[]): Promise<(LineItem | null)[]> {
     ...
     */
    async retrieve(id: string, config: FindConfig<Fulfillment> = {}): Promise<Fulfillment> {
        const manager = this.manager_;
        const fulfillmentRepository = manager.getCustomRepository(this.fulfillmentRepository_);
        ...
        if (!fulfillment) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Fulfillment with id: ${id} was not found`);
        }
        return fulfillment;
    }
     ...
     */
    async createFulfillment(order: CreateFulfillmentOrder, itemsToFulfill: FulFillmentItemType[], custom: Partial<Fulfillment> = {}): Promise<Fulfillment[]> {
        return await this.atomicPhase_(async (manager) => {
            ...
            // partition order items to their dedicated shipping method
            const fulfillments = this.partitionItems_(shipping_methods, lineItems as LineItem[]);
            ...
            const created = await Promise.all(
                fulfillments.map(async ({ shipping_method, items }) => {
                    ...
                    result.data = await this.fulfillmentProviderService_.createFulfillment(shipping_method, items, { ...order }, { ...result });
     ...
     *
     */
    async cancelFulfillment(fulfillmentOrId: Fulfillment | string): Promise<Fulfillment> {
        return await this.atomicPhase_(async (manager) => {
            const id = typeof fulfillmentOrId === "string" ? fulfillmentOrId : fulfillmentOrId.id;
            ...
            if (fulfillment.shipped_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `The fulfillment has already been shipped. Shipped fulfillments cannot be canceled`);
            }
            ...
            const fulfillmentRepo = manager.getCustomRepository(this.fulfillmentRepository_);
    ...
    async createShipment(
        ...
        return await this.atomicPhase_(async (manager) => {
            const fulfillmentRepository = manager.getCustomRepository(this.fulfillmentRepository_);
            const trackingLinkRepo = manager.getCustomRepository(this.trackingLinkRepository_);
            ...
            if (fulfillment.canceled_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Fulfillment has been canceled");
            }

            const now = new Date();
            fulfillment.shipped_at = now;

            fulfillment.tracking_links = (trackingLinks || []).map((tl) => trackingLinkRepo.create(tl));
