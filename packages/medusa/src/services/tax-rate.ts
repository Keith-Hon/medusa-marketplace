...
class TaxRateService extends BaseService {
    ...
    async list(selector: FilterableTaxRateProps, config: FindConfig<TaxRate> = {}): Promise<TaxRate[]> {
    ...
    async listAndCount(selector: FilterableTaxRateProps, config: FindConfig<TaxRate> = {}): Promise<[TaxRate[], number]> {
        const taxRateRepo = this.manager_.getCustomRepository(this.taxRateRepository_);
        const query = this.buildQuery_(selector, config);
    ...
    async addToProduct(id: string, productIds: string | string[], replace = false): Promise<ProductTaxRate | ProductTaxRate[]> {
        let ids: string[];
        if (typeof productIds === "string") {
            ids = [productIds];
        } else {
        ...
        const result = await this.atomicPhase_(
            ...
            async (err: any) => {
                if (err.code === "23503") {
                    ...
                    await Promise.all([this.retrieve(id, { select: ["id"] }), ...ids.map((pId) => this.productService_.retrieve(pId, { select: ["id"] }))]);
                }
            }
        );
        return result;
    ...
    async addToProductType(id: string, productTypeIds: string | string[], replace = false): Promise<ProductTypeTaxRate[]> {
        let ids: string[];
    ...
    async addToShippingOption(id: string, optionIds: string | string[], replace = false): Promise<ShippingTaxRate> {
        let ids: string[];
        if (typeof optionIds === "string") {
            ids = [optionIds];
        } else {
        ...
        return await this.atomicPhase_(
            async (manager: EntityManager) => {
                ...
                const options = await this.shippingOptionService_.list({ id: ids }, { select: ["id", "region_id"] });
            ...
            async (err: any) => {
                if (err.code === "23503") {
                    ...
                    // will try to retrieve all of the resources and will fail when
                    // something is not found.
                    await Promise.all([this.retrieve(id, { select: ["id"] }), ...ids.map((sId) => this.shippingOptionService_.retrieve(sId, { select: ["id"] }))]);
                }
            }
        );
    }
