...
class ReturnReasonService extends TransactionBaseService<ReturnReasonService> {
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (data.parent_return_reason_id && data.parent_return_reason_id !== "") {
                ...
                if (parentReason.parent_return_reason_id) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, "Doubly nested return reasons is not supported");
                }
            }
    ...
    async update(id: string, data: UpdateReturnReason): Promise<ReturnReason> {
        return await this.atomicPhase_(async (manager) => {
            ...
            for (const key of Object.keys(data).filter((k) => typeof data[k] !== `undefined`)) {
                reason[key] = data[key];
            }
     ...
     */
    async retrieve(id: string, config: FindConfig<ReturnReason> = {}): Promise<ReturnReason | never> {
        const rrRepo = this.manager_.getCustomRepository(this.retReasonRepo_);
        ...
        if (!item) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Return Reason with id: ${id} was not found.`);
        }
