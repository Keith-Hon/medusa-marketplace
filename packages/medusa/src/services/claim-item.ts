...
class ClaimItemService extends BaseService<ClaimItemService> {
    ...
    async create(data: CreateClaimItemInput): Promise<ClaimItem> {
        return await this.atomicPhase_(async (manager) => {
            const ciRepo: ClaimItemRepository = manager.getCustomRepository(this.claimItemRepository_);

            const { item_id, reason, quantity, tags, images, ...rest } = data;

            if (reason !== "missing_item" && reason !== "wrong_item" && reason !== "production_failure" && reason !== "other") {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `Claim Item reason must be one of "missing_item", "wrong_item", "production_failure" or "other".`);
            }

            const lineItem = await this.lineItemService_.withTransaction(manager).retrieve(item_id);

            if (lineItem.fulfilled_quantity < quantity) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot claim more of an item than has been fulfilled.");
            }
            ...
            if (images && images.length) {
                const claimImgRepo = manager.getCustomRepository(this.claimImageRepository_);
            ...
            await this.eventBus_.withTransaction(manager).emit(ClaimItemService.Events.CREATED, {
                id: result.id
            });
    ...
    async update(id, data): Promise<ClaimItem> {
        return this.atomicPhase_(async (manager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(ClaimItemService.Events.UPDATED, {
                id: item.id
            });
     ...
     */
    async retrieve(id: string, config: FindConfig<ClaimItem> = {}): Promise<ClaimItem> {
        const claimItemRepo = this.manager_.getCustomRepository(this.claimItemRepository_);
        const query = buildQuery({ id }, config);
        const item = await claimItemRepo.findOne(query);

        if (!item) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Claim item with id: ${id} was not found.`);
        }
