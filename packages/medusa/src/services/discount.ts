...
import { Brackets, DeepPartial, EntityManager, ILike, SelectQueryBuilder } from "typeorm";
import { EventBusService, ProductService, RegionService, TotalsService } from ".";
import { Cart, Discount, LineItem, Region } from "../models";
import { AllocationType as DiscountAllocation, DiscountRule, DiscountRuleType } from "../models/discount-rule";
...
import { CreateDiscountInput, CreateDiscountRuleInput, CreateDynamicDiscountInput, FilterableDiscountProps, UpdateDiscountInput, UpdateDiscountRuleInput } from "../types/discount";
...
class DiscountService extends TransactionBaseService<DiscountService> {
     ...
     */
    validateDiscountRule_<T extends { type: DiscountRuleType; value: number }>(discountRule: T): T {
        if (discountRule.type === "percentage" && discountRule.value > 100) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Discount value above 100 is not allowed when type is percentage");
        }
     ...
     */
    async list(selector: FilterableDiscountProps = {}, config: FindConfig<Discount> = { relations: [], skip: 0, take: 10 }): Promise<Discount[]> {
    ...
    async create(discount: CreateDiscountInput): Promise<Discount> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            ...
            const validatedRule: Omit<CreateDiscountRuleInput, "conditions"> = this.validateDiscountRule_(ruleToCreate);

            if (discount?.regions && discount?.regions.length > 1 && discount?.rule?.type === "fixed") {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Fixed discounts can have one region");
            }
            try {
                if (discount.regions) {
                    discount.regions = (await Promise.all(discount.regions.map((regionId) => this.regionService_.withTransaction(manager).retrieve(regionId)))) as Region[];
                }
                ...
                const created: Discount = discountRepo.create(discount as DeepPartial<Discount>);
                ...
                if (conditions?.length) {
                    await Promise.all(
                        conditions.map(async (cond) => {
                            await this.discountConditionService_.withTransaction(manager).upsertCondition({ rule_id: result.rule_id, ...cond });
                        })
                    );
                }
    ...
    async retrieve(discountId: string, config: FindConfig<Discount> = {}): Promise<Discount> {
        ...
        if (!discount) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Discount with ${discountId} was not found`);
        }
     ...
     */
    async retrieveByCode(discountCode: string, config: FindConfig<Discount> = {}): Promise<Discount> {
        const manager = this.manager_;
        const discountRepo = manager.getCustomRepository(this.discountRepository_);
        ...
        if (!discount) {
            ...
            if (!discount) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `Discount with code ${discountCode} was not found`);
            }
        }
     ...
     */
    async update(discountId: string, update: UpdateDiscountInput): Promise<Discount> {
        return await this.atomicPhase_(async (manager) => {
            const discountRepo: DiscountRepository = manager.getCustomRepository(this.discountRepository_);
            const ruleRepo: DiscountRuleRepository = manager.getCustomRepository(this.discountRuleRepository_);
            ...
            if (rest.ends_at) {
                if (discount.starts_at >= new Date(rest.ends_at)) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `"ends_at" must be greater than "starts_at"`);
                }
            }
            ...
            if (conditions?.length) {
                await Promise.all(
                    conditions.map(async (cond) => {
                        await this.discountConditionService_.withTransaction(manager).upsertCondition({ rule_id: discount.rule_id, ...cond });
                    })
                );
            }

            if (regions) {
                discount.regions = await Promise.all(regions.map((regionId) => this.regionService_.retrieve(regionId)));
            }
            ...
            for (const key of Object.keys(rest).filter((k) => typeof rest[k] !== `undefined`)) {
     ...
     */
    async createDynamicCode(discountId: string, data: CreateDynamicDiscountInput): Promise<Discount> {
        return await this.atomicPhase_(async (manager) => {
            const discountRepo = manager.getCustomRepository(this.discountRepository_);
            ...
            if (!discount.is_dynamic) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Discount must be set to dynamic");
            }

            if (!data.code) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Discount must have a code");
            }
            ...
            if (discount.valid_duration) {
                ...
                lastValidDate.setSeconds(lastValidDate.getSeconds() + toSeconds(parse(discount.valid_duration)));
    ...
    async validateDiscountForProduct(discountRuleId: string, productId: string | undefined): Promise<boolean> {
        return await this.atomicPhase_(async (manager) => {
            const discountConditionRepo: DiscountConditionRepository = manager.getCustomRepository(this.discountConditionRepository_);
            ...
            return await discountConditionRepo.isValidForProduct(discountRuleId, product.id);
        });
    }

    async calculateDiscountForLineItem(discountId: string, lineItem: LineItem, cart: Cart): Promise<number> {
    ...
    async validateDiscountForCartOrThrow(cart: Cart, discount: Discount): Promise<void> {
        return await this.atomicPhase_(async () => {
            if (this.hasReachedLimit(discount)) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Discount has been used maximum allowed times");
            }

            if (this.hasNotStarted(discount)) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Discount is not valid yet");
            }

            if (this.hasExpired(discount)) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Discount is expired");
            }

            if (this.isDisabled(discount)) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "The discount code is disabled");
            }

            const isValidForRegion = await this.isValidForRegion(discount, cart.region_id);
            if (!isValidForRegion) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "The discount is not available in current region");
            }

            if (cart.customer_id) {
                const canApplyForCustomer = await this.canApplyForCustomer(discount.rule.id, cart.customer_id);

                if (!canApplyForCustomer) {
                    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Discount is not valid for customer");
                }
            }
        });
    }
    ...
    async isValidForRegion(discount: Discount, region_id: string): Promise<boolean> {
        return await this.atomicPhase_(async () => {
    ...
    async canApplyForCustomer(discountRuleId: string, customerId: string | undefined): Promise<boolean> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            const discountConditionRepo: DiscountConditionRepository = manager.getCustomRepository(this.discountConditionRepository_);
            ...
            const customer = await this.customerService_.withTransaction(manager).retrieve(customerId, {
                relations: ["groups"]
            });

            return await discountConditionRepo.canApplyForCustomer(discountRuleId, customer.id);
        });
    }
}
