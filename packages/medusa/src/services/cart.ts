...
import { DiscountRuleType, Address, Cart, CustomShippingOption, Customer, Discount, LineItem, ShippingMethod, SalesChannel } from "../models";
...
class CartService extends TransactionBaseService<CartService> {
    ...
    protected transformQueryForTotals_(config: FindConfig<Cart>): FindConfig<Cart> & { totalsToSelect: TotalField[] } {
        ...
        const totalFields = ["subtotal", "tax_total", "shipping_total", "discount_total", "gift_card_total", "total"];

        const totalsToSelect = select.filter((v) => totalFields.includes(v)) as TotalField[];
    ...
    protected async decorateTotals_(cart: Cart, totalsToSelect: TotalField[], options: TotalsConfig = { force_taxes: false }): Promise<Cart> {
        ...
        for (const key of totalsToSelect) {
            switch (key) {
                ...
                case "discount_total":
                    ...
                    break;
                case "tax_total":
                    totals.tax_total = await this.totalsService_.getTaxTotal(cart, options.force_taxes);
                    break;
     ...
     */
    async list(selector: FilterableCartProps, config: FindConfig<Cart> = {}): Promise<Cart[]> {
     ...
     */
    async retrieve(cartId: string, options: FindConfig<Cart> = {}, totalsConfig: TotalsConfig = {}): Promise<Cart> {
        const manager = this.manager_;
        const cartRepo = manager.getCustomRepository(this.cartRepository_);
        const validatedId = validateId(cartId);

        const { select, relations, totalsToSelect } = this.transformQueryForTotals_(options);

        const query = buildQuery({ id: validatedId }, { ...options, select, relations });
        ...
        if (!raw) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Cart with ${cartId} was not found`);
        }
    ...
    async create(data: CartCreateProps): Promise<Cart | never> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cartRepo = transactionManager.getCustomRepository(this.cartRepository_);
            const addressRepo = transactionManager.getCustomRepository(this.addressRepository_);

            const rawCart: DeepPartial<Cart> = {
                context: data.context ?? {}
            };

            if (this.featureFlagRouter_.isFeatureEnabled(SalesChannelFeatureFlag.key)) {
                rawCart.sales_channel_id = (await this.getValidatedSalesChannel(data.sales_channel_id)).id;
            }
            ...
            if (!data.region_id) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, `A region_id must be provided when creating a cart`);
            }

            rawCart.region_id = data.region_id;
            const region = await this.regionService_.withTransaction(transactionManager).retrieve(data.region_id, {
                relations: ["countries"]
            });
            ...
            if (!data.shipping_address && !data.shipping_address_id) {
                ...
                if (data.shipping_address) {
                    if (!regCountries.includes(data.shipping_address.country_code)) {
                        throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Shipping country not in region");
                    }
            ...
            const remainingFields: (keyof Cart)[] = ["billing_address_id", "context", "type", "metadata", "discounts", "gift_cards"];

            for (const remainingField of remainingFields) {
                if (typeof data[remainingField] !== "undefined" && remainingField !== "object") {
            ...
            await this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.CREATED, {
                id: cart.id
            });
            return cart;
        });
    ...
    protected async getValidatedSalesChannel(salesChannelId?: string): Promise<SalesChannel | never> {
        let salesChannel: SalesChannel;
        if (typeof salesChannelId !== "undefined") {
            salesChannel = await this.salesChannelService_.withTransaction(this.manager_).retrieve(salesChannelId);
        } else {
            salesChannel = (
        ...
        if (salesChannel.is_disabled) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, `Unable to assign the cart to a disabled Sales Channel "${salesChannel.name}"`);
        }
    ...
    async removeLineItem(cartId: string, lineItemId: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
                relations: ["items", "items.variant", "items.variant.product", "payment_sessions"]
            });
            ...
            if (cart.shipping_methods?.length) {
                await this.shippingOptionService_.withTransaction(transactionManager).deleteShippingMethods(cart.shipping_methods);
            }
            ...
            await this.lineItemService_.withTransaction(transactionManager).delete(lineItem.id);
            ...
            // Notify subscribers
            await this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.UPDATED, {
                id: cart.id
            });
    ...
    protected validateLineItemShipping_(shippingMethods: ShippingMethod[], lineItem: LineItem): boolean {
        ...
        if (shippingMethods && shippingMethods.length && lineItem.variant && lineItem.variant.product) {
            const productProfile = lineItem.variant.product.profile_id;
            const selectedProfiles = shippingMethods.map(({ shipping_option }) => shipping_option.profile_id);
     ...
     */
    protected async validateLineItem(cart: Cart, lineItem: LineItem): Promise<boolean> {
        if (!cart.sales_channel_id) {
            return true;
        }

        const lineItemVariant = await this.productVariantService_.withTransaction(this.manager_).retrieve(lineItem.variant_id);

        return !!(await this.productService_.withTransaction(this.manager_).filterProductsBySalesChannel([lineItemVariant.product_id], cart.sales_channel_id)).length;
    }
     ...
     * @return the result of the update operation
     */
    async addLineItem(cartId: string, lineItem: LineItem, config = { validateSalesChannels: true }): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
                relations: ["shipping_methods", "items", "items.adjustments", "payment_sessions", "items.variant", "items.variant.product", "discounts", "discounts.rule"]
            });
            ...
            const quantity = currentItem ? (currentItem.quantity += lineItem.quantity) : lineItem.quantity;

            // Confirm inventory or throw error
            await this.inventoryService_.withTransaction(transactionManager).confirmInventory(lineItem.variant_id, quantity);

            if (currentItem) {
                await this.lineItemService_.withTransaction(transactionManager).update(currentItem.id, {
            ...
            await this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.UPDATED, result);
     ...
     * @return the result of the update operation
     */
    async updateLineItem(cartId: string, lineItemId: string, lineItemUpdate: LineItemUpdate): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
            ...
            if (!lineItemExists) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "A line item with the provided id doesn't exist in the cart");
            }

            if (lineItemUpdate.quantity) {
                const hasInventory = await this.inventoryService_.withTransaction(transactionManager).confirmInventory(lineItemExists.variant_id, lineItemUpdate.quantity);

                if (!hasInventory) {
                    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Inventory doesn't cover the desired quantity");
                }
            }

            await this.lineItemService_.withTransaction(transactionManager).update(lineItemId, lineItemUpdate);
            ...
            await this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.UPDATED, updatedCart);
     ...
     * @return void
     */
    protected async adjustFreeShipping_(cart: Cart, shouldAdd: boolean): Promise<void> {
        const transactionManager = this.transactionManager_ ?? this.manager_;

        if (cart.shipping_methods?.length) {
            const shippingMethodRepository = transactionManager.getCustomRepository(this.shippingMethodRepository_);

            // if any free shipping discounts, we ensure to update shipping method amount
            if (shouldAdd) {
                await shippingMethodRepository.update(
                    {
                        id: In(cart.shipping_methods.map((shippingMethod) => shippingMethod.id))
                    },
                    {
                        price: 0
                    }
                ...
                await Promise.all(
                    cart.shipping_methods.map(async (shippingMethod) => {
                        ...
                        shippingMethod.price =
                            shippingMethod.shipping_option.amount ?? (await this.shippingOptionService_.getPrice_(shippingMethod.shipping_option, shippingMethod.data, cart));
    ...
    async update(cartId: string, data: CartUpdateProps): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (this.featureFlagRouter_.isFeatureEnabled(SalesChannelFeatureFlag.key) && data.sales_channel_id) {
            ...
            if (typeof data.customer_id !== "undefined" || typeof data.region_id !== "undefined") {
                await this.updateUnitPrices_(cart, data.region_id, data.customer_id);
            }

            if (typeof data.region_id !== "undefined") {
                const shippingAddress = typeof data.shipping_address !== "string" ? data.shipping_address : {};
                const countryCode = (data.country_code || shippingAddress?.country_code) ?? null;
                await this.setRegion_(cart, data.region_id, countryCode);
            }

            const addrRepo = transactionManager.getCustomRepository(this.addressRepository_);
            ...
            const shippingAddress = data.shipping_address_id ?? data.shipping_address;
            if (shippingAddress !== undefined) {
                await this.updateShippingAddress_(cart, shippingAddress, addrRepo);
            }

            if (this.featureFlagRouter_.isFeatureEnabled(SalesChannelFeatureFlag.key)) {
                if (typeof data.sales_channel_id !== "undefined" && data.sales_channel_id != cart.sales_channel_id) {
            ...
            if (typeof data.discounts !== "undefined") {
                ...
                const hasFreeShipping = cart.discounts.some(({ rule }) => rule?.type === "free_shipping");

                // if we previously had a free shipping discount and then removed it,
                // we need to update shipping methods to original price
                if (previousDiscounts.some(({ rule }) => rule.type === "free_shipping") && !hasFreeShipping) {
            ...
            if ("email" in data || "customer_id" in data) {
                await this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.CUSTOMER_UPDATED, updatedCart.id);
            }
     ...
     * @protected
     */
    protected async onSalesChannelChange(cart: Cart, newSalesChannelId: string): Promise<void> {
        await this.getValidatedSalesChannel(newSalesChannelId);

        const productIds = cart.items.map((item) => item.variant.product_id);
        const productsToKeep = await this.productService_.withTransaction(this.manager_).filterProductsBySalesChannel(productIds, newSalesChannelId, {
            select: ["id", "sales_channels"],
            take: productIds.length
        });
        const productIdsToKeep = new Set<string>(productsToKeep.map((product) => product.id));
     ...
     * @return the result of the update operation
     */
    protected async updateCustomerId_(cart: Cart, customerId: string): Promise<void> {
        const customer = await this.customerService_.withTransaction(this.transactionManager_).retrieve(customerId);
    ...
    protected async createOrFetchUserFromEmail_(email: string): Promise<Customer> {
        ...
        if (error) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "The email is not valid");
        }
        ...
        if (!customer) {
            customer = await this.customerService_.withTransaction(this.transactionManager_).create({ email: value });
        }
     ...
     * @return the result of the update operation
     */
    protected async updateBillingAddress_(cart: Cart, addressOrId: AddressPayload | Partial<Address> | string, addrRepo: AddressRepository): Promise<void> {
        let address: Address;
     ...
     * @return the result of the update operation
     */
    protected async updateShippingAddress_(cart: Cart, addressOrId: AddressPayload | Partial<Address> | string, addrRepo: AddressRepository): Promise<void> {
        let address: Address;
        ...
        if (address.country_code && !cart.region.countries.find(({ iso_2 }) => address.country_code === iso_2)) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Shipping country must be in the cart region");
        }
    ...
    protected async applyGiftCard_(cart: Cart, code: string): Promise<void> {
        const giftCard = await this.giftCardService_.withTransaction(this.transactionManager_).retrieveByCode(code);

        if (giftCard.is_disabled) {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "The gift card is disabled");
        }

        if (giftCard.region_id !== cart.region_id) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "The gift card cannot be used in the current region");
        }
    ...
    async applyDiscount(cart: Cart, discountCode: string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const discount = await this.discountService_.withTransaction(transactionManager).retrieveByCode(discountCode, { relations: ["rule", "regions"] });

            await this.discountService_.withTransaction(transactionManager).validateDiscountForCartOrThrow(cart, discount);
            ...
            cart.discounts = newDiscounts.filter((newDiscount): newDiscount is Discount => {
                return !!newDiscount;
            });
    ...
    async removeDiscount(cartId: string, discountCode: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
                relations: ["discounts", "discounts.rule", "payment_sessions", "shipping_methods"]
            });
    ...
    async updatePaymentSession(cartId: string, update: object): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (cart.payment_session) {
                await this.paymentProviderService_.withTransaction(transactionManager).updateSessionData(cart.payment_session, update);
            }
     ...
     * @return the resulting cart
     */
    async authorizePayment(cartId: string, context: Record<string, unknown> = {}): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cartRepository = transactionManager.getCustomRepository(this.cartRepository_);

            const cart = await this.retrieve(cartId, {
                select: ["total"],
                relations: ["items", "items.adjustments", "region", "payment_sessions"]
            });

            if (typeof cart.total === "undefined") {
                throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "cart.total should be defined");
            }
            ...
            if (!cart.payment_session) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "You cannot complete a cart without a payment session.");
            }

            const session = await this.paymentProviderService_.withTransaction(transactionManager).authorizePayment(cart.payment_session, context);
            ...
            if (session.status === "authorized") {
                freshCart.payment = await this.paymentProviderService_.withTransaction(transactionManager).createPayment(freshCart);
    ...
    async setPaymentSession(cartId: string, providerId: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const psRepo = transactionManager.getCustomRepository(this.paymentSessionRepository_);

            const cart = await this.retrieve(cartId, {
                select: ["total", "subtotal", "tax_total", "discount_total", "gift_card_total"],
                relations: ["region", "region.payment_providers", "payment_sessions"]
            });

            // The region must have the provider id in its providers array
            if (providerId !== "system" && !(cart.region.payment_providers.length && cart.region.payment_providers.find(({ id }) => providerId === id))) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `The payment method is not available in this region`);
            }
            ...
            const paymentSession = cart.payment_sessions.find((ps) => ps.provider_id === providerId);

            if (!paymentSession) {
                throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Could not find payment session");
            }
    ...
    async setPaymentSessions(cartOrCartId: Cart | string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            const cartId = typeof cartOrCartId === `string` ? cartOrCartId : cartOrCartId.id;

            const cart = await this.retrieve(
                cartId,
                {
                    select: ["total", "subtotal", "tax_total", "discount_total", "shipping_total", "gift_card_total"],
                    relations: [
                        "items",
                        "items.adjustments",
                        "discounts",
            ...
            if (typeof total === "undefined") {
                throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "cart.total must be defined");
            }
            ...
            if (cart.payment_sessions?.length) {
                await Promise.all(
                    cart.payment_sessions.map(async (paymentSession) => {
                        if (total <= 0 || !region.payment_providers.find(({ id }) => id === paymentSession.provider_id)) {
                            return this.paymentProviderService_.withTransaction(transactionManager).deleteSession(paymentSession);
                        } else {
                            seen.push(paymentSession.provider_id);
                            return this.paymentProviderService_.withTransaction(transactionManager).updateSession(paymentSession, cart);
                        }
                    })
                );
            }
            ...
            if (total > 0) {
                ...
                if (region.payment_providers.length === 1 && !cart.payment_session) {
                    ...
                    const paymentSession = await this.paymentProviderService_.withTransaction(transactionManager).createSession(paymentProvider.id, cart);
                    ...
                    await Promise.all(
                        region.payment_providers.map(async (paymentProvider) => {
                            if (!seen.includes(paymentProvider.id)) {
                                return this.paymentProviderService_.withTransaction(transactionManager).createSession(paymentProvider.id, cart);
                            }
                            return;
                        })
                    );
     ...
     * @return the resulting cart.
     */
    async deletePaymentSession(cartId: string, providerId: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
                relations: ["payment_sessions"]
            });
            ...
            if (cart.payment_sessions) {
                const paymentSession = cart.payment_sessions.find(({ provider_id }) => provider_id === providerId);

                cart.payment_sessions = cart.payment_sessions.filter(({ provider_id }) => provider_id !== providerId);

                if (paymentSession) {
                    // Delete the session with the provider
                    await this.paymentProviderService_.withTransaction(transactionManager).deleteSession(paymentSession);
                }
            }

            await cartRepo.save(cart);

            await this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.UPDATED, cart);
            return cart;
        });
    }
     ...
     */
    async refreshPaymentSession(cartId: string, providerId: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
                relations: ["payment_sessions"]
            });
            ...
            if (cart.payment_sessions) {
                ...
                if (paymentSession) {
                    // Delete the session with the provider
                    await this.paymentProviderService_.withTransaction(transactionManager).refreshSession(paymentSession, cart);
                }
            }
     ...
     * @return the result of the update operation
     */
    async addShippingMethod(cartId: string, optionId: string, data: Record<string, unknown> = {}): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cart = await this.retrieve(cartId, {
                select: ["subtotal"],
                relations: [
            ...
            const cartCustomShippingOptions = await this.customShippingOptionService_.withTransaction(transactionManager).list({ cart_id: cart.id });

            const customShippingOption = this.findCustomShippingOption(cartCustomShippingOptions, optionId);
             ...
             * just the id.
             */
            const shippingMethodConfig = customShippingOption ? { cart_id: cart.id, price: customShippingOption.price } : { cart };

            const newShippingMethod = await this.shippingOptionService_.withTransaction(transactionManager).createShippingMethod(optionId, data, shippingMethodConfig);

            const methods = [newShippingMethod];
            if (shipping_methods?.length) {
                for (const shippingMethod of shipping_methods) {
                    if (shippingMethod.shipping_option.profile_id === newShippingMethod.shipping_option.profile_id) {
                        await this.shippingOptionService_.withTransaction(transactionManager).deleteShippingMethods(shippingMethod);
                    } else {
            ...
            if (cart.items?.length) {
                await Promise.all(
                    cart.items.map(async (item) => {
                        return this.lineItemService_.withTransaction(transactionManager).update(item.id, {
            ...
            if (updatedCart.discounts.some(({ rule }) => rule.type === "free_shipping")) {
    ...
    findCustomShippingOption(cartCustomShippingOptions: CustomShippingOption[], optionId: string): CustomShippingOption | undefined {
        const customOption = cartCustomShippingOptions?.find((cso) => cso.shipping_option_id === optionId);
        const hasCustomOptions = cartCustomShippingOptions?.length;

        if (hasCustomOptions && !customOption) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Wrong shipping option");
        }

        return customOption;
    }

    protected async updateUnitPrices_(cart: Cart, regionId?: string, customer_id?: string): Promise<void> {
        const transactionManager = this.transactionManager_ ?? this.manager_;
        ...
        // value if it exists)
        if (cart.items?.length) {
            const region = await this.regionService_.withTransaction(this.transactionManager_).retrieve(regionId || cart.region_id, {
                relations: ["countries"]
            });
            ...
            cart.items = (
                await Promise.all(
                    cart.items.map(async (item) => {
                        ...
                        if (availablePrice !== undefined && availablePrice.calculatedPrice !== null) {
                            return this.lineItemService_.withTransaction(transactionManager).update(item.id, {
                                has_shipping: false,
                                unit_price: availablePrice.calculatedPrice
                            });
                        } else {
                            await this.lineItemService_.withTransaction(transactionManager).delete(item.id);
                            return;
                        }
                    })
                )
     ...
     * @return the result of the update operation
     */
    protected async setRegion_(cart: Cart, regionId: string, countryCode: string | null): Promise<void> {
        const transactionManager = this.transactionManager_ ?? this.manager_;

        if (cart.completed_at || cart.payment_authorized_at) {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot change the region of a completed cart");
        }

        const region = await this.regionService_.withTransaction(transactionManager).retrieve(regionId, {
            relations: ["countries"]
        });
        cart.region = region;
         ...
         */
        if (countryCode !== null) {
            if (!region.countries.find(({ iso_2 }) => iso_2 === countryCode.toLowerCase())) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `Country not available in region`);
            }
        ...
        if (cart.payment_sessions && cart.payment_sessions.length) {
            const paymentSessionRepo = transactionManager.getCustomRepository(this.paymentSessionRepository_);
            await paymentSessionRepo.delete({
                id: In(cart.payment_sessions.map((paymentSession) => paymentSession.id))
            });
    ...
    async delete(cartId: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            if (cart.completed_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Completed carts cannot be deleted");
            }

            if (cart.payment_authorized_at) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't delete a cart with an authorized payment");
            }
     ...
     */
    async setMetadata(cartId: string, key: string, value: string | number): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            const cartRepo = transactionManager.getCustomRepository(this.cartRepository_);
            ...
            if (!cart) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, "Unable to find the cart with the given id");
            }
            ...
            const updatedCart = await cartRepo.save(cart);
            this.eventBus_.withTransaction(transactionManager).emit(CartService.Events.UPDATED, updatedCart);
            return updatedCart;
        });
    }
    ...
    async createTaxLines(id: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            const calculationContext = this.totalsService_.withTransaction(transactionManager).getCalculationContext(cart);

            await this.taxProviderService_.withTransaction(transactionManager).createTaxLines(cart, calculationContext);
    ...
    protected async refreshAdjustments_(cart: Cart): Promise<void> {
        ...
        const nonReturnLineIDs = cart.items.filter((item) => !item.is_return).map((i) => i.id);

        // delete all old non return line item adjustments
        await this.lineItemAdjustmentService_.withTransaction(transactionManager).delete({
            item_id: nonReturnLineIDs
        });

        // potentially create/update line item adjustments
        await this.lineItemAdjustmentService_.withTransaction(transactionManager).createAdjustments(cart);
    }
    ...
    async deleteMetadata(cartId: string, key: string): Promise<Cart> {
        return await this.atomicPhase_(async (transactionManager: EntityManager) => {
            ...
            const cart = await cartRepo.findOne(validatedId);
            if (!cart) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `Cart with id: ${validatedId} was not found`);
            }
