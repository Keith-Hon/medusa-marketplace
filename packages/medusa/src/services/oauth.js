...
class Oauth extends OauthService {
    ...
    async generateToken(appName, code, state) {
        ...
        if (!service) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, `An OAuth handler for ${app.display_name} could not be found make sure the plugin is installed`);
        }

        if (!app.state === state) {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `${app.display_name} could not match state`);
        }
