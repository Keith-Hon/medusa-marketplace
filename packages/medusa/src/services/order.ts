...
import { Address, ClaimOrder, Fulfillment, FulfillmentItem, FulfillmentStatus, LineItem, Order, OrderStatus, Payment, PaymentStatus, Return, Swap, TrackingLink } from "../models";
import { UpdateOrderInput } from "../types/orders";
import { CreateShippingMethodDto } from "../types/shipping-options";
import { CreateFulfillmentOrder, FulFillmentItemType } from "../types/fulfillment";
...
class OrderService extends TransactionBaseService<OrderService> {
    ...
    async list(
        ...
        const { select, relations, totalsToSelect } = this.transformQueryForTotals(config);
        ...
        return await Promise.all(raw.map(async (r) => await this.decorateTotals(r, totalsToSelect)));
    }
    ...
    async listAndCount(
        ...
        const orders = await Promise.all(raw.map(async (r) => await this.decorateTotals(r, totalsToSelect)));
    ...
    async retrieve(orderId: string, config: FindConfig<Order> = {}): Promise<Order> {
        ...
        if (!raw) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Order with ${orderId} was not found`);
        }
     ...
     * @return the order document
     */
    async retrieveByCartId(cartId: string, config: FindConfig<Order> = {}): Promise<Order> {
        const orderRepo = this.manager_.getCustomRepository(this.orderRepository_);
        ...
        if (!raw) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Order with cart id: ${cartId} was not found`);
        }
     ...
     * @return the order document
     */
    async retrieveByExternalId(externalId: string, config: FindConfig<Order> = {}): Promise<Order> {
        const orderRepo = this.manager_.getCustomRepository(this.orderRepository_);
        ...
        const raw = await orderRepo.findOneWithRelations(rels, query);
        if (!raw) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Order with external id ${externalId} was not found`);
        }
    ...
    async completeOrder(orderId: string): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot be completed");
            }
    ...
    async createFromCart(cartId: string): Promise<Order | never> {
        return await this.atomicPhase_(async (manager) => {
            ...
            const cart = await cartService.retrieve(cartId, {
                ...
                relations: ["region", "payment", "items", "discounts", "discounts.rule", "gift_cards", "shipping_methods"]
            });

            if (cart.items.length === 0) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Cannot create order from empty cart");
            }
            ...
            for (const item of cart.items) {
                try {
                    await inventoryService.confirmInventory(item.variant_id, item.quantity);
                } catch (err) {
                    if (payment) {
                        await this.paymentProviderService_.withTransaction(manager).cancelPayment(payment);
                    }
            ...
            if (exists) {
                throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, "Order from cart already exists");
            }
            ...
            if (total !== 0) {
                ...
                if (!payment) {
                    throw new MedusaError(MedusaError.Types.INVALID_ARGUMENT, "Cart does not contain a payment method");
                }

                const paymentStatus = await this.paymentProviderService_.withTransaction(manager).getStatus(payment);

                // If payment status is not authorized, we throw
                if (paymentStatus !== "authorized") {
                    throw new MedusaError(MedusaError.Types.INVALID_ARGUMENT, "Payment method is not authorized");
                }
            }
            ...
            if (cart.type === "draft_order") {
                const draft = await this.draftOrderService_.withTransaction(manager).retrieveByCartId(cart.id);
            ...
            if (total !== 0 && payment) {
                await this.paymentProviderService_.withTransaction(manager).updatePayment(payment.id, {
                    order_id: result.id
                });
            }
            ...
            for (const g of cart.gift_cards) {
                ...
                await gcService.createTransaction({
                    ...
                    tax_rate: cart.region.gift_cards_taxable ? cart.region.tax_rate : null
                });
            ...
            for (const method of cart.shipping_methods) {
                await this.shippingOptionService_.withTransaction(manager).updateShippingMethod(method.id, { order_id: result.id });
            }
            ...
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.PLACED, {
                id: result.id,
    ...
    async createShipment(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const shipment = await this.fulfillmentService_.withTransaction(manager).retrieve(fulfillmentId);

            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot be fulfilled as shipped");
            }

            if (!shipment || shipment.order_id !== orderId) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, "Could not find fulfillment");
            }

            const evaluatedNoNotification = no_notification !== undefined ? no_notification : shipment.no_notification;

            const shipmentRes = await this.fulfillmentService_.withTransaction(manager).createShipment(fulfillmentId, trackingLinks, {
                metadata,
            ...
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.SHIPMENT_CREATED, {
                id: orderId,
     ...
     */
    protected async updateBillingAddress(order: Order, address: Address): Promise<void> {
        ...
        const region = await this.regionService_.withTransaction(this.manager_).retrieve(order.region_id, {
            relations: ["countries"]
        });

        if (!region.countries.find(({ iso_2 }) => address.country_code === iso_2)) {
            throw new MedusaError(MedusaError.Types.INVALID_DATA, "Shipping country must be in the order region");
        }
     ...
     * @return the result of the update operation
     */
    protected async updateShippingAddress(order: Order, address: Address): Promise<void> {
        const addrRepo = this.manager_.getCustomRepository(this.addressRepository_);
        address.country_code = address.country_code?.toLowerCase() ?? null;
    ...
    async addShippingMethod(orderId: string, optionId: string, data?: Record<string, unknown>, config: CreateShippingMethodDto = {}): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            const order = await this.retrieve(orderId, {
                select: ["subtotal"],
                relations: ["shipping_methods", "shipping_methods.shipping_option", "items", "items.variant", "items.variant.product"]
            });
            const { shipping_methods } = order;

            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A shipping method cannot be added to a canceled order");
            }

            const newMethod = await this.shippingOptionService_.withTransaction(manager).createShippingMethod(optionId, data ?? {}, { order, ...config });

            const methods = [newMethod];
            if (shipping_methods.length) {
                for (const sm of shipping_methods) {
                    if (sm.shipping_option.profile_id === newMethod.shipping_option.profile_id) {
                        await this.shippingOptionService_.withTransaction(manager).deleteShippingMethods(sm);
                    } else {
                        methods.push(sm);
                    }
                }
            ...
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.UPDATED, { id: result.id });
            return result;
        });
    }
    ...
    async update(orderId: string, update: UpdateOrderInput): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot be updated");
            }

            if ((update.payment || update.items) && (order.fulfillment_status !== "not_fulfilled" || order.payment_status !== "awaiting" || order.status !== "pending")) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't update shipping, billing, items and payment method when order is processed");
            }

            if (update.status || update.fulfillment_status || update.payment_status) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't update order statuses. This will happen automatically. Use metadata in order for additional statuses");
            }

            const { metadata, shipping_address, billing_address, no_notification, items, ...rest } = update;
            ...
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.UPDATED, {
                id: orderId,
                no_notification: order.no_notification
            });
            return result;
    ...
    async cancel(orderId: string): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            const order = await this.retrieve(orderId, {
                relations: ["fulfillments", "payments", "returns", "claims", "swaps", "items"]
            });

            if (order.refunds?.length > 0) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Order with refund(s) cannot be canceled");
            }
            ...
            const throwErrorIf = (
                ...
                if (arr?.filter(pred).length) {
                    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `All ${type} must be canceled before canceling an order`);
                }
            };
            ...
            throwErrorIf(order.returns, (r) => (r as Return).status !== "canceled", "returns");
            throwErrorIf(order.swaps, notCanceled, "swaps");
            throwErrorIf(order.claims, notCanceled, "claims");

            for (const item of order.items) {
                await this.inventoryService_.withTransaction(manager).adjustInventory(item.variant_id, item.quantity);
            }

            for (const p of order.payments) {
                await this.paymentProviderService_.withTransaction(manager).cancelPayment(p);
            }
            ...
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.CANCELED, {
                id: order.id,
                no_notification: order.no_notification
            });
            return result;
    ...
    async capturePayment(orderId: string): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot capture payment");
            }
            ...
            for (const p of order.payments) {
                if (p.captured_at === null) {
                    const result = await this.paymentProviderService_
                        ...
                        .capturePayment(p)
                        .catch((err) => {
                            this.eventBus_.withTransaction(manager).emit(OrderService.Events.PAYMENT_CAPTURE_FAILED, {
                                id: orderId,
                                payment_id: p.id,
                                error: err,
                                no_notification: order.no_notification
            ...
            order.payment_status = payments.every((p) => p.captured_at !== null) ? PaymentStatus.CAPTURED : PaymentStatus.REQUIRES_ACTION;

            const result = await orderRepo.save(order);

            if (order.payment_status === PaymentStatus.CAPTURED) {
                this.eventBus_.withTransaction(manager).emit(OrderService.Events.PAYMENT_CAPTURED, {
                    id: result.id,
                    no_notification: order.no_notification
                });
            }
    ...
    protected validateFulfillmentLineItem(item: LineItem, quantity: number): LineItem | null {
        ...
        if (quantity > item.quantity - item.fulfilled_quantity) {
            throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot fulfill more items than have been purchased");
        }
        return {
            ...item,
            quantity
    ...
    async createFulfillment(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            const order = await this.retrieve(orderId, {
                select: ["subtotal", "shipping_total", "discount_total", "tax_total", "gift_card_total", "total"],
                relations: [
                    "discounts",
                    "discounts.rule",
                    "region",
            ...
            if (order.status === OrderStatus.CANCELED) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot be fulfilled");
            }

            if (!order.shipping_methods?.length) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot fulfill an order that lacks shipping methods");
            }

            const fulfillments = await this.fulfillmentService_.withTransaction(manager).createFulfillment(order as unknown as CreateFulfillmentOrder, itemsToFulfill, {
                metadata,
            ...
            for (const item of order.items) {
                const fulfillmentItem = successfullyFulfilled.find((f) => item.id === f.item_id);

                if (fulfillmentItem) {
                    const fulfilledQuantity = (item.fulfilled_quantity || 0) + fulfillmentItem.quantity;
            ...
            for (const fulfillment of fulfillments) {
                await this.eventBus_.withTransaction(manager).emit(OrderService.Events.FULFILLMENT_CREATED, {
                    id: orderId,
    ...
    async cancelFulfillment(fulfillmentId: string): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            const canceled = await this.fulfillmentService_.withTransaction(manager).cancelFulfillment(fulfillmentId);

            if (!canceled.order_id) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, `Fufillment not related to an order`);
            }
            ...
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.FULFILLMENT_CANCELED, {
                id: order.id,
     ...
     */
    protected async getFulfillmentItems(order: Order, items: FulFillmentItemType[], transformer: (item: LineItem | undefined, quantity: number) => unknown): Promise<LineItem[]> {
        return (
            await Promise.all(
    ...
    async archive(orderId: string): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            ...
            if (order.status !== ("completed" || "refunded")) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Can't archive an unprocessed order");
            }
    ...
    async createRefund(
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot be refunded");
            }

            if (refundAmount > order.refundable_amount) {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Cannot refund more than the original order amount");
            }

            const refund = await this.paymentProviderService_.withTransaction(manager).refundPayment(order.payments, refundAmount, reason, note);
    ...
    protected async decorateTotals(order: Order, totalsFields: string[] = []): Promise<Order> {
        for (const totalField of totalsFields) {
            switch (totalField) {
                ...
                case "total": {
                    order.total = await this.totalsService_.withTransaction(this.manager_).getTotal(order);
                    break;
                }
                case "refunded_total": {
     ...
     * @return the result of the update operation
     */
    async registerReturnReceived(orderId: string, receivedReturn: Return, customRefundAmount?: number): Promise<Order> {
        return await this.atomicPhase_(async (manager) => {
            const order = await this.retrieve(orderId, {
            ...
            if (order.status === "canceled") {
                throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "A canceled order cannot be registered as received");
            }

            if (!receivedReturn || receivedReturn.order_id !== orderId) {
                throw new MedusaError(MedusaError.Types.NOT_FOUND, `Received return does not exist`);
            }
            ...
            if (refundAmount > order.refundable_amount) {
                ...
                const result = await orderRepo.save(order);
                this.eventBus_.withTransaction(manager).emit(OrderService.Events.RETURN_ACTION_REQUIRED, {
                    id: result.id,
            ...
            if (receivedReturn.refund_amount > 0) {
                const refund = await this.paymentProviderService_.withTransaction(manager).refundPayment(order.payments, receivedReturn.refund_amount, "return");
            ...
            const result = await orderRepo.save(order);
            await this.eventBus_.withTransaction(manager).emit(OrderService.Events.ITEMS_RETURNED, {
                id: order.id,
                return_id: receivedReturn.id,
                no_notification: receivedReturn.no_notification
            });
