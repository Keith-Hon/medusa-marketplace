...
class TotalsService extends TransactionBaseService<TotalsService> {
    ...
    async getTotal(cartOrOrder: Cart | Order, options: GetTotalsOptions = {}): Promise<number> {
        ...
        const giftCardTotal = options.exclude_gift_cards ? { total: 0 } : this.getGiftCardTotal(cartOrOrder);
     ...
     */
    async getShippingMethodTotals(shippingMethod: ShippingMethod, cartOrOrder: Cart | Order, opts: GetShippingMethodTotalsOptions = {}): Promise<ShippingMethodTotals> {
        ...
        if (opts.include_tax) {
            if (isOrder(cartOrOrder) && cartOrOrder.tax_rate !== null) {
                ...
                if (opts.use_tax_lines || isOrder(cartOrOrder)) {
                    if (typeof shippingMethod.tax_lines === "undefined") {
                        throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Tax Lines must be joined on shipping method to calculate taxes");
                    }

                    taxLines = shippingMethod.tax_lines;
                } else {
                    const orderLines = await this.taxProviderService_.withTransaction(this.manager_).getTaxLines(cartOrOrder.items, calculationContext);
            ...
            if (totals.tax_lines.length > 0) {
                totals.original_tax_total = await this.taxCalculationStrategy_.calculate([], totals.tax_lines, calculationContext);
    ...
    async getTaxTotal(cartOrOrder: Cart | Order, forceTaxes = false): Promise<number | null> {
        ...
        if (isOrder(cartOrOrder)) {
            ...
            if (!taxLinesJoined) {
                throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Order tax calculations must have tax lines joined on line items");
            }
            ...
            if (cartOrOrder.tax_rate === null) {
                ...
                const shippingTotal = this.getShippingTotal(cartOrOrder);
                const discountTotal = this.getDiscountTotal(cartOrOrder);
                return this.rounded((subtotal - discountTotal - giftCardTotal.total + shippingTotal) * (cartOrOrder.tax_rate / 100));
            }
        } else {
            taxLines = await this.taxProviderService_.withTransaction(this.manager_).getTaxLines(cartOrOrder.items, calculationContext);

            if (cartOrOrder.type === "swap") {
                const returnTaxLines = cartOrOrder.items.flatMap((i) => {
                    if (i.is_return) {
                        if (typeof i.tax_lines === "undefined") {
                            throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Return Line Items must join tax lines");
                        }
                        return i.tax_lines;
                    }
        ...
        const toReturn = await this.taxCalculationStrategy_.calculate(cartOrOrder.items, taxLines, calculationContext);
    ...
    getLineItemRefund(order: Order, lineItem: LineItem): number {
        ...
        const discountAmount = (allocationMap[lineItem.id]?.discount?.unit_amount || 0) * lineItem.quantity;
        ...
        if (typeof lineItem.tax_lines === "undefined") {
            throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Tax calculation did not receive tax_lines");
        }
    ...
    calculateDiscount_(lineItem: LineItem, variant: string, variantPrice: number, value: number, discountType: DiscountRuleType): LineDiscount {
        ...
        if (discountType === DiscountRuleType.PERCENTAGE) {
            ...
            return {
                lineItem,
                variant,
                amount: value >= variantPrice * lineItem.quantity ? variantPrice * lineItem.quantity : value * lineItem.quantity
            };
        }
    }
    ...
    getLineItemDiscountAdjustment(lineItem: LineItem, discount: Discount): number {
        const matchingDiscount = lineItem.adjustments.find((adjustment) => adjustment.discount_id === discount.id);
    ...
    getLineItemAdjustmentsTotal(cartOrOrder: Cart | Order): number {
        ...
        return cartOrOrder.items.reduce((total, item) => total + item.adjustments?.reduce((total, adjustment) => total + adjustment.amount, 0) || 0, 0);
    }
    ...
    getLineDiscounts(cartOrOrder: Cart | Order, discount: Discount): LineDiscountAmount[] {
        ...
        return merged.map((item) => {
            ...
            const discountAdjustments = adjustments.filter((adjustment) => adjustment.discount_id === discount.id);

            return {
                item,
                amount: item.allow_discounts ? discountAdjustments.reduce((total, adjustment) => total + adjustment.amount, 0) : 0
            };
        });
    }
     ...
     */
    async getLineItemTotals(lineItem: LineItem, cartOrOrder: Cart | Order, options: LineItemTotalsOptions = {}): Promise<LineItemTotals> {
        const calculationContext = this.getCalculationContext(cartOrOrder, {
            exclude_shipping: true
        });
        ...
        if (options.include_tax) {
            ...
            if (isOrder(cartOrOrder) && cartOrOrder.tax_rate !== null) {
                ...
                if (options.use_tax_lines || isOrder(cartOrOrder)) {
                    if (typeof lineItem.tax_lines === "undefined") {
                        throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "Tax Lines must be joined on items to calculate taxes");
                    }
        ...
        if (lineItemTotals.tax_lines.length > 0) {
            lineItemTotals.tax_total = await this.taxCalculationStrategy_.calculate([lineItem], lineItemTotals.tax_lines, calculationContext);
            lineItemTotals.total += lineItemTotals.tax_total;

            calculationContext.allocation_map = {}; // Don't account for discounts
            lineItemTotals.original_tax_total = await this.taxCalculationStrategy_.calculate([lineItem], lineItemTotals.tax_lines, calculationContext);
            lineItemTotals.original_total += lineItemTotals.original_tax_total;
        }
     ...
     */
    async getLineItemTotal(lineItem: LineItem, cartOrOrder: Cart | Order, options: GetLineItemTotalOptions = {}): Promise<number> {
