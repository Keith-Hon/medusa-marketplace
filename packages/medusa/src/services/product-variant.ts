...
class ProductVariantService extends BaseService {
    ...
    async retrieve(
        ...
        if (!variant) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `Variant with id: ${variantId} was not found`);
        }
     ...
     */
    async create(productOrProductId: string | Product, variant: CreateProductVariantInput): Promise<ProductVariant> {
        return this.atomicPhase_(async (manager: EntityManager) => {
            ...
            product.options.forEach((option) => {
                if (!variant.options.find((vo) => option.id === vo.option_id)) {
                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Variant options do not contain value for ${option.title}`);
                }
            });
            ...
            if (variantExists) {
                throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, `Variant with title ${variantExists.title} with provided options already exists`);
            }
     ...
     */
    async update(variantOrVariantId: string | Partial<ProductVariant>, update: UpdateProductVariantInput): Promise<ProductVariant> {
        return this.atomicPhase_(async (manager: EntityManager) => {
            const variantRepo = manager.getCustomRepository(this.productVariantRepository_);
            ...
            if (typeof variant === `string`) {
                ...
                if (typeof variant === "undefined") {
                    throw new MedusaError(MedusaError.Types.NOT_FOUND, `Variant with id ${variantOrVariantId} was not found`);
                } else {
    ...
    async getRegionPrice(variantId: string, context: GetRegionPriceContext): Promise<number> {
        return this.atomicPhase_(async (manager: EntityManager) => {
            ...
            const prices = await this.priceSelectionStrategy_.withTransaction(manager).calculateVariantPrice(variantId, {
     ...
     * @return {Promise} the result of the update operation.
     */
    async updateOptionValue(variantId: string, optionId: string, optionValue: string): Promise<ProductOptionValue> {
        return this.atomicPhase_(async (manager: EntityManager) => {
     ...
     * @return {Promise} the result of the update operation.
     */
    async addOptionValue(variantId: string, optionId: string, optionValue: string): Promise<ProductOptionValue> {
        return this.atomicPhase_(async (manager: EntityManager) => {
            const productOptionValueRepo = manager.getCustomRepository(this.productOptionValueRepository_);
    ...
    async deleteOptionValue(variantId: string, optionId: string): Promise<void> {
        return this.atomicPhase_(async (manager: EntityManager) => {
            const productOptionValueRepo: ProductOptionValueRepository = manager.getCustomRepository(this.productOptionValueRepository_);
    ...
    async list(
        ...
        if (q) {
            ...
            query.where = (qb: SelectQueryBuilder<ProductVariant>): void => {
                qb.where(where).andWhere([{ sku: ILike(`%${q}%`) }, { title: ILike(`%${q}%`) }, { product: { title: ILike(`%${q}%`) } }]);
            };
        }
     ...
     *   search param.
     */
    prepareListQuery_(selector: FilterableProductVariantProps, config: FindConfig<ProductVariant>): { query: FindWithRelationsOptions; relations: string[]; q?: string } {
        let q: string | undefined;
