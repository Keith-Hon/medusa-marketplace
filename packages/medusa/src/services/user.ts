...
class UserService extends TransactionBaseService<UserService> {
     ...
     * @return {Promise<User>} the user document.
     */
    async retrieveByApiToken(apiToken: string, relations: string[] = []): Promise<User> {
        const manager = this.manager_;
        const userRepo = manager.getCustomRepository(this.userRepository_);
        ...
        if (!user) {
            throw new MedusaError(MedusaError.Types.NOT_FOUND, `User with api token: ${apiToken} was not found`);
        }
    ...
    async create(user: CreateUserInput, password: string): Promise<User> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            ...
            await this.eventBus_.withTransaction(manager).emit(UserService.Events.CREATED, { id: newUser.id });
    ...
    async update(userId: string, update: UpdateUserInput): Promise<User> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            ...
            if (email) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "You are not allowed to update email");
            }

            if (password_hash) {
                throw new MedusaError(MedusaError.Types.INVALID_DATA, "Use dedicated methods, `setPassword`, `generateResetPasswordToken` for password operations");
            }
            ...
            await this.eventBus_.withTransaction(manager).emit(UserService.Events.UPDATED, { id: updatedUser.id });
    ...
    async setPassword_(userId: string, password: string): Promise<User> {
        return await this.atomicPhase_(async (manager: EntityManager) => {
            ...
            const hashedPassword = await this.hashPassword_(password);
            if (!hashedPassword) {
                throw new MedusaError(MedusaError.Types.DB_ERROR, `An error occured while hashing password`);
            }
    ...
    async generateResetPasswordToken(userId: string): Promise<string> {
        return await this.atomicPhase_(async (transactionManager) => {
            ...
            // Notify subscribers
            await this.eventBus_.withTransaction(transactionManager).emit(UserService.Events.PASSWORD_RESET, {
                email: user.email,
                token
            });
