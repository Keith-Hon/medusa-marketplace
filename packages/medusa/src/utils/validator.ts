...
export async function validator<T, V>(typedClass: ClassConstructor<T>, plain: V, config: ValidatorOptions = {}): Promise<T> {
    ...
    if (errors?.length) {
        throw new MedusaError(MedusaError.Types.INVALID_DATA, errorMessages.join(", "));
    }
