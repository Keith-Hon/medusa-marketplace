...
export function FeatureFlagColumn(featureFlag: string, columnOptions: ColumnOptions = {}): PropertyDecorator {
...
export function FeatureFlagDecorators(featureFlag: string, decorators: PropertyDecorator[]): PropertyDecorator {
    return function (target, propertyName) {
        setImmediate_((): any => {
            const featureFlagRouter = getFeatureFlagRouter();
...
export function FeatureFlagEntity(featureFlag: string, name?: string, options?: EntityOptions): ClassDecorator {
...
function getFeatureFlagRouter(): FlagRouter {
    const { configModule } = getConfigFile(path.resolve("."), `medusa-config`) as { configModule: ConfigModule };
