...
import { CartService, DraftOrderService, OrderService, PaymentProviderService } from "../../../../services";
import { defaultAdminOrdersFields as defaultOrderFields, defaultAdminOrdersRelations as defaultOrderRelations } from "../orders/index";
...
export default async (req, res) => {
    ...
    await entityManager.transaction(async (manager) => {
        ...
        const cart = await cartService.withTransaction(manager).retrieve(draftOrder.cart_id, {
            select: ["total"],
            relations: ["discounts", "discounts.rule", "shipping_methods", "region", "items"]
        });

        await paymentProviderService.withTransaction(manager).createSession("system", cart);

        await cartService.withTransaction(manager).setPaymentSession(cart.id, "system");
        ...
        await draftOrderService.withTransaction(manager).registerCartCompletion(draftOrder.id, result.id);
    });
