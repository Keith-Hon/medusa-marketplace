...
export default (app) => {
    ...
    route.delete("/:id", middlewares.wrap(require("./delete-draft-order").default));
    ...
    route.post("/:id/pay", middlewares.wrap(require("./register-payment").default));

    return app;
};

export const defaultAdminDraftOrdersRelations = ["order", "cart", "cart.items", "cart.items.adjustments"];
...
export const defaultAdminDraftOrdersCartFields: (keyof Cart)[] = ["subtotal", "tax_total", "shipping_total", "discount_total", "gift_card_total", "total"];
...
export const allowedAdminDraftOrdersFields = ["id", "status", "display_id", "cart_id", "canceled_at", "created_at", "updated_at", "metadata", "no_notification_order"];
