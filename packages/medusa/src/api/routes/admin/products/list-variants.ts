...
export default async (req: Request, res: Response) => {
    ...
    const { expand, fields, limit, offset } = await validator(AdminGetProductsVariantsParams, req.query);

    const queryConfig = getRetrieveConfig<ProductVariant>(
        defaultAdminGetProductsVariantsFields as (keyof ProductVariant)[],
        [],
        [...new Set([...defaultAdminGetProductsVariantsFields, ...(fields?.split(",") ?? [])])] as (keyof ProductVariant)[],
