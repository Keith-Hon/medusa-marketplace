...
export default (app, featureFlagRouter: FlagRouter) => {
    ...
    route.get("/tag-usage", middlewares.wrap(require("./list-tag-usage-count").default));

    route.get("/:id/variants", middlewares.normalizeQuery(), middlewares.wrap(require("./list-variants").default));
    route.post("/:id/variants", middlewares.wrap(require("./create-variant").default));

    route.post("/:id/variants/:variant_id", middlewares.wrap(require("./update-variant").default));

    route.post("/:id/options/:option_id", middlewares.wrap(require("./update-option").default));
    route.post("/:id/options", middlewares.wrap(require("./add-option").default));

    route.delete("/:id/variants/:variant_id", middlewares.wrap(require("./delete-variant").default));
    route.delete("/:id", middlewares.wrap(require("./delete-product").default));
    route.delete("/:id/options/:option_id", middlewares.wrap(require("./delete-option").default));

    route.post("/:id/metadata", middlewares.wrap(require("./set-metadata").default));
    route.get(
        "/:id",
...
export const defaultAdminProductRelations = ["variants", "variants.prices", "variants.options", "images", "options", "tags", "type", "collection"];
...
export const allowedAdminProductRelations = ["variants", "variants.prices", "images", "options", "tags", "type", "collection", "sales_channels"];
