...
describe("POST /admin/products/:id/variants/:variantId", () => {
    describe("successful removes variant", () => {
        ...
        beforeAll(async () => {
            subject = await request("DELETE", `/admin/products/${IdMap.getId("productWithOptions")}/variants/${IdMap.getId("variant1")}`, {
                adminSession: {
                    jwt: {
        ...
        it("calls service removeVariant", () => {
            ...
            expect(ProductVariantServiceMock.delete).toHaveBeenCalledWith(IdMap.getId("variant1"));
        });
