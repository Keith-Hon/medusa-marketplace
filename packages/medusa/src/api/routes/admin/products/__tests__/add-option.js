...
describe("POST /admin/products/:id/options", () => {
    describe("successful add option", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/products/${IdMap.getId("productWithOptions")}/options`, {
                payload: {
                    title: "Test option"
                },
                adminSession: {
        ...
        it("calls service addOption", () => {
            ...
            expect(ProductServiceMock.addOption).toHaveBeenCalledWith(IdMap.getId("productWithOptions"), "Test option");
        });
