...
describe("POST /admin/products", () => {
    describe("successful creation with variants", () => {
        ...
        it("assigns invokes productVariantService with ranked variants", () => {
            ...
            expect(ProductVariantServiceMock.create).toHaveBeenCalledWith(IdMap.getId("productWithOptions"), {
                title: "Test",
                variant_rank: 0,
                prices: [
                    {
    ...
    describe("successful creation of gift card product", () => {
        ...
        it("calls profile service", () => {
            expect(ShippingProfileServiceMock.retrieveGiftCardDefault).toHaveBeenCalledTimes(1);
            expect(ShippingProfileServiceMock.retrieveGiftCardDefault).toHaveBeenCalledWith();
        });
    });
    ...
    describe("invalid data returns error details", () => {
        ...
        it("returns error details", () => {
            ...
            expect(subject.body.message).toEqual(expect.stringContaining(`title must be a string`));
        });
    });
});
