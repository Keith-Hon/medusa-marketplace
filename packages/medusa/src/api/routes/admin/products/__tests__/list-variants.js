...
describe("GET /admin/products/:id/variants", () => {
    describe("successfully gets a product variants", () => {
        ...
        beforeAll(async () => {
            subject = await request("GET", `/admin/products/${IdMap.getId("product1")}/variants`, {
                adminSession: {
                    jwt: {
        ...
        it("should returns product decorated", () => {
            ...
            expect(subject.body.variants).toEqual(
                expect.arrayContaining([expect.objectContaining({ product_id: IdMap.getId("product1") }), expect.objectContaining({ product_id: IdMap.getId("product1") })])
            );
        });
        ...
        it("should call the get product from productService with the expected parameters including the config that has been given", async () => {
            await request("GET", `/admin/products/${IdMap.getId("product1")}/variants`, {
                adminSession: {
                    jwt: {
                        userId: IdMap.getId("admin_user")
                    }
