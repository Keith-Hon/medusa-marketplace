...
describe("POST /admin/products/:id", () => {
    describe("successfully updates a product", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/products/${IdMap.getId("product1")}`, {
                payload: {
                    title: "Product 1",
    ...
    describe("handles failed update operation", () => {
        it("throws if metadata is to be updated", async () => {
            try {
                ...
                expect(error.message).toEqual("Use setMetadata to update metadata fields");
            }
        });
        ...
        it("throws if variants is to be updated", async () => {
            try {
                ...
                expect(error.status).toEqual(400);
                expect(error.message).toEqual("Use addVariant, reorderVariants, removeVariant to update Product Variants");
            }
        });
    });
});
