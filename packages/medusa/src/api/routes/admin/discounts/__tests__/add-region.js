...
describe("POST /admin/discounts/:discount_id/regions/:region_id", () => {
    describe("successful addition", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/discounts/${IdMap.getId("total10")}/regions/${IdMap.getId("region-france")}`, {
                adminSession: {
                    jwt: {
        ...
        it("calls service retrieve", () => {
            ...
            expect(DiscountServiceMock.addRegion).toHaveBeenCalledWith(IdMap.getId("total10"), IdMap.getId("region-france"));
        });
    });
});
