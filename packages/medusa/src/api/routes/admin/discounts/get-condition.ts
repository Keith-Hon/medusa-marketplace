...
import { defaultAdminDiscountConditionFields, defaultAdminDiscountConditionRelations } from ".";
...
export default async (req, res) => {
    ...
    const validatedParams = await validator(AdminGetDiscountsDiscountConditionsConditionParams, req.query);
    ...
    const existsOnDiscount = discount.rule.conditions.some((c) => c.id === condition_id);

    if (!existsOnDiscount) {
        throw new MedusaError(MedusaError.Types.NOT_FOUND, `Condition with id ${condition_id} does not belong to Discount with id ${discount_id}`);
    }
    ...
    const discountCondition = await conditionService.retrieve(condition_id, config);
