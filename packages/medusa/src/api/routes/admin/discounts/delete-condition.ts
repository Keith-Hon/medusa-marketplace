...
export default async (req, res) => {
    ...
    const validatedParams = await validator(AdminDeleteDiscountsDiscountConditionsConditionParams, req.query);

    const conditionService: DiscountConditionService = req.scope.resolve("discountConditionService");

    const condition = await conditionService.retrieve(condition_id).catch(() => void 0);
    ...
    await manager.transaction(async (transactionManager) => {
        return await conditionService.withTransaction(transactionManager).delete(condition_id);
    });
