...
describe("GET /admin/sales-channels/:id", () => {
    describe("successfully get a sales channel", () => {
        ...
        beforeAll(async () => {
            subject = await request("GET", `/admin/sales-channels/${IdMap.getId("sales_channel_1")}`, {
                adminSession: {
                    jwt: {
        ...
        it("calls the retrieve method from the sales channel service", () => {
            ...
            expect(SalesChannelServiceMock.retrieve).toHaveBeenCalledWith(IdMap.getId("sales_channel_1"));
        });
