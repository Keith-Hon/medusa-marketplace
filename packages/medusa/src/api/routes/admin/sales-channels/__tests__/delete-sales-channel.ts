...
describe("DELETE /admin/sales-channels/:id", () => {
    describe("successfully delete a sales channel", () => {
        ...
        beforeAll(async () => {
            subject = await request("DELETE", `/admin/sales-channels/${IdMap.getId("sales_channel_1")}`, {
                adminSession: {
                    jwt: {
        ...
        it("calls the delete method from the sales channel service", () => {
            ...
            expect(SalesChannelServiceMock.delete).toHaveBeenCalledWith(IdMap.getId("sales_channel_1"));
        });
