...
export default (app) => {
     ...
     */
    route.delete("/:id/products/batch", middlewares.wrap(require("./remove-from-products").default));

    /**
     * Remove product types from tax rate
     */
    route.delete("/:id/product-types/batch", middlewares.wrap(require("./remove-from-product-types").default));

    /**
     * Remove shipping options from tax rate
     */
    route.delete("/:id/shipping-options/batch", middlewares.wrap(require("./remove-from-shipping-options").default));

    /**
     * Add products to tax rate
     */
    route.post("/:id/products/batch", middlewares.wrap(require("./add-to-products").default));

    /**
     * Add product types to tax rate
     */
    route.post("/:id/product-types/batch", middlewares.wrap(require("./add-to-product-types").default));

    /**
     * Add to shipping options
     */
    route.post("/:id/shipping-options/batch", middlewares.wrap(require("./add-to-shipping-options").default));
...
export const defaultAdminTaxRatesFields: (keyof TaxRate)[] = ["id", "rate", "code", "name", "region_id", "created_at", "updated_at"];
