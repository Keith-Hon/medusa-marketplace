...
export function pickByConfig<T>(obj: T | T[], config: FindConfig<T>): Partial<T> | Partial<T>[] {
...
export function getRetrieveConfig(fields?: (keyof TaxRate)[], expand?: string[]): FindConfig<TaxRate> {
    ...
    return {
        ...
        relations: expandFields.length ? expandFields : defaultAdminTaxRatesRelations
    };
}

export function getListConfig(fields?: (keyof TaxRate)[], expand?: string[], limit = 50, offset = 0, order?: { [k: symbol]: "DESC" | "ASC" }): FindConfig<TaxRate> {
    let includeFields: (keyof TaxRate)[] = [];
    if (typeof fields !== "undefined") {
        const fieldSet = new Set(fields);
