...
export default async (req, res) => {
    const value = await validator(AdminDeleteTaxRatesTaxRateShippingOptionsReq, req.body);

    const query = await validator(AdminDeleteTaxRatesTaxRateShippingOptionsParams, req.query);

    const rateService: TaxRateService = req.scope.resolve("taxRateService");

    const manager: EntityManager = req.scope.resolve("manager");
    await manager.transaction(async (transactionManager) => {
        return await rateService.withTransaction(transactionManager).removeFromShippingOption(req.params.id, value.shipping_options);
    });
