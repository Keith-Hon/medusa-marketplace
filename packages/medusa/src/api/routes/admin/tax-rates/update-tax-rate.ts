...
export default async (req, res) => {
    ...
    await manager.transaction(async (tx) => {
        ...
        await txRateService.update(req.params.id, omit(value, ["products", "product_types", "shipping_options"]));
        ...
        if (typeof value.product_types !== "undefined") {
            await txRateService.addToProductType(req.params.id, value.product_types, true);
        }

        if (typeof value.shipping_options !== "undefined") {
            await txRateService.addToShippingOption(req.params.id, value.shipping_options, true);
        }
    });
