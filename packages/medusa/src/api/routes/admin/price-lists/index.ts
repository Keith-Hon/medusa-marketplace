...
import { allowedAdminProductFields, defaultAdminProductFields, defaultAdminProductRelations } from "../products";
...
export default (app) => {
    ...
    route.get("/", transformQuery(AdminGetPriceListPaginationParams, { isList: true }), middlewares.wrap(require("./list-price-lists").default));
    ...
    route.get(
        ...
        transformQuery(AdminGetPriceListsPriceListProductsParams, {
            ...
            defaultRelations: defaultAdminProductRelations.filter((r) => r !== "variants.prices"),
            defaultLimit: 50,
            isList: true
        }),
    ...
    route.delete("/:id/products/:product_id/prices", middlewares.wrap(require("./delete-product-prices").default));
    route.delete("/:id/variants/:variant_id/prices", middlewares.wrap(require("./delete-variant-prices").default));

    route.post("/", transformBody(AdminPostPriceListsPriceListReq), middlewares.wrap(require("./create-price-list").default));
    ...
    route.delete("/:id/prices/batch", middlewares.wrap(require("./delete-prices-batch").default));

    route.post("/:id/prices/batch", middlewares.wrap(require("./add-prices-batch").default));

    return app;
};

export const defaultAdminPriceListFields = ["id", "name", "description", "type", "status", "starts_at", "ends_at", "created_at", "updated_at", "deleted_at"];
