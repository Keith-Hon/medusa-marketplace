...
describe("DELETE /price-lists/:id/prices/batch", () => {
    describe("successfully adds several new prices to a price list", () => {
        ...
        beforeAll(async () => {
            subject = await request("DELETE", `/admin/price-lists/pl_1234/prices/batch`, {
                payload: {
        ...
        it("calls PriceListService addPrices", () => {
            ...
            expect(PriceListServiceMock.deletePrices).toHaveBeenCalledWith("pl_1234", ["price_1234", "price_1235", "price_1236", "price_1237"]);
        });
    });
    ...
    describe("fails if no prices were provided", () => {
        ...
        it("returns descriptive error that price_ids is missing", () => {
            ...
            expect(subject.body.message).toEqual("each value in price_ids must be a string, price_ids should not be empty");
        });
    });
});
