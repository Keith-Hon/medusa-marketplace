...
describe("POST /admin/returns/:id/receive", () => {
    describe("successfully receives a return", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/returns/${IdMap.getId("test-return")}/receive`, {
                payload: {
                    items: [
                        {
        ...
        it("calls ReturnService receive", () => {
            ...
            expect(ReturnService.receive).toHaveBeenCalledWith(IdMap.getId("test-return"), [{ item_id: IdMap.getId("test"), quantity: 2 }], undefined, true);
        });
