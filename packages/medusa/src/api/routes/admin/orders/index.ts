...
import { Order } from "../../../..";
import { DeleteResponse, EmptyQueryParams, PaginatedResponse } from "../../../../types/common";
...
export default (app, featureFlagRouter: FlagRouter) => {
     ...
     */
    route.post("/:id/complete", middlewares.wrap(require("./complete-order").default));

    /**
     * Refund an amount to the customer's card.
     */
    route.post("/:id/refund", middlewares.wrap(require("./refund-payment").default));

    /**
     * Capture the authorized amount on the customer's card.
     */
    route.post("/:id/capture", middlewares.wrap(require("./capture-payment").default));

    /**
     * Create a fulfillment.
     */
    route.post("/:id/fulfillment", middlewares.wrap(require("./create-fulfillment").default));

    /**
     * Cancel a fulfillment related to an order.
     */
    route.post("/:id/fulfillments/:fulfillment_id/cancel", middlewares.wrap(require("./cancel-fulfillment").default));

    /**
     * Cancel a fulfillment related to a swap.
     */
    route.post("/:id/swaps/:swap_id/fulfillments/:fulfillment_id/cancel", middlewares.wrap(require("./cancel-fulfillment-swap").default));

    /**
     * Cancel a fulfillment related to a claim.
     */
    route.post("/:id/claims/:claim_id/fulfillments/:fulfillment_id/cancel", middlewares.wrap(require("./cancel-fulfillment-claim").default));

    /**
     * Create a shipment.
     */
    route.post("/:id/shipment", middlewares.wrap(require("./create-shipment").default));

    /**
     * Request a return.
     */
    route.post("/:id/return", middlewares.wrap(require("./request-return").default));
     ...
     */
    route.post("/:id/shipping-methods", middlewares.wrap(require("./add-shipping-method").default));

    /**
     * Archive an order.
     */
    route.post("/:id/archive", middlewares.wrap(require("./archive-order").default));
     ...
     * Cancels a swap.
     */
    route.post("/:id/swaps/:swap_id/cancel", middlewares.wrap(require("./cancel-swap").default));

    /**
     * Fulfills a swap.
     */
    route.post("/:id/swaps/:swap_id/fulfillments", middlewares.wrap(require("./fulfill-swap").default));

    /**
     * Marks a swap fulfillment as shipped.
     */
    route.post("/:id/swaps/:swap_id/shipments", middlewares.wrap(require("./create-swap-shipment").default));

    /**
     * Captures the payment associated with a swap
     */
    route.post("/:id/swaps/:swap_id/process-payment", middlewares.wrap(require("./process-swap-payment").default));
     ...
     * Cancels a claim
     */
    route.post("/:id/claims/:claim_id/cancel", middlewares.wrap(require("./cancel-claim").default));

    /**
     * Updates a claim
     */
    route.post("/:id/claims/:claim_id", middlewares.wrap(require("./update-claim").default));

    /**
     * Creates claim fulfillment
     */
    route.post("/:id/claims/:claim_id/fulfillments", middlewares.wrap(require("./fulfill-claim").default));

    /**
     * Creates claim shipment
     */
    route.post("/:id/claims/:claim_id/shipments", middlewares.wrap(require("./create-claim-shipment").default));
