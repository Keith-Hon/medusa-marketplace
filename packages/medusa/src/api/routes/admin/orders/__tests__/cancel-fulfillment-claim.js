...
describe("POST /admin/orders/:id/claims/:claim_id/fulfillments/:fulfillment_id/cancel", () => {
    describe("successfully cancels a fulfillment", () => {
        ...
        beforeAll(async () => {
            subject = await request(
                "POST",
                `/admin/orders/${IdMap.getId("test-order")}/claims/${IdMap.getId("test-claim")}/fulfillments/${IdMap.getId("claim-fulfillment")}/cancel`,
                {
                    adminSession: {
                        jwt: {
        ...
        it("calls claimService cancelFulfillment", () => {
            ...
            expect(ClaimServiceMock.cancelFulfillment).toHaveBeenCalledWith(IdMap.getId("claim-fulfillment"));
        });
    });
    ...
    describe("Trying to cancel a fulfillment unrelated to the claim fails", () => {
        ...
        beforeAll(async () => {
            subject = await request(
                "POST",
                `/admin/orders/${IdMap.getId("test-order")}/claims/${IdMap.getId("claim-fulfillment2")}/fulfillments/${IdMap.getId("claim-fulfillment")}/cancel`,
                {
                    adminSession: {
                        jwt: {
                            userId: IdMap.getId("admin_user")
    ...
    describe("Trying to cancel a fulfillment, where claim and order are unrelated", () => {
        ...
        beforeAll(async () => {
            subject = await request(
                "POST",
                `/admin/orders/${IdMap.getId("test-order2")}/claims/${IdMap.getId("test-claim")}/fulfillments/${IdMap.getId("claim-fulfillment")}/cancel`,
                {
                    adminSession: {
                        jwt: {
                            userId: IdMap.getId("admin_user")
