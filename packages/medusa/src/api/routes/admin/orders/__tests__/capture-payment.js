...
describe("POST /admin/orders/:id/capture", () => {
    describe("successfully captures payment for an order", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order")}/capture`, {
                adminSession: {
                    jwt: {
        ...
        it("calls OrderService capturePayment", () => {
            ...
            expect(OrderServiceMock.capturePayment).toHaveBeenCalledWith(IdMap.getId("test-order"));
        });
