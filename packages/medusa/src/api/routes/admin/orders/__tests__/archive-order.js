...
describe("POST /admin/orders/:id/archive", () => {
    describe("successfully archives an order", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("processed-order")}/archive`, {
                adminSession: {
                    jwt: {
        ...
        it("calls OrderService archive", () => {
            ...
            expect(OrderServiceMock.archive).toHaveBeenCalledWith(IdMap.getId("processed-order"));
        });
