...
describe("POST /admin/orders/:id/swaps/:swap_id/cancel", () => {
    describe("successfully cancels a claim", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order")}/swaps/${IdMap.getId("test-swap")}/cancel`, {
                adminSession: {
                    jwt: {
        ...
        it("calls SwapService cancel", () => {
            ...
            expect(SwapServiceMock.cancel).toHaveBeenCalledWith(IdMap.getId("test-swap"));
        });
    });
    ...
    describe("Trying to cancel a claim unrelated to the order fails", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order2")}/swaps/${IdMap.getId("test-swap")}/cancel`, {
                adminSession: {
                    jwt: {
                        userId: IdMap.getId("admin_user")
                    }
