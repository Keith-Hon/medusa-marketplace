...
describe("POST /admin/orders/:id/claims/:claim_id/cancel", () => {
    describe("successfully cancels a claim", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order")}/claims/${IdMap.getId("test-claim")}/cancel`, {
                adminSession: {
                    jwt: {
    ...
    describe("Trying to cancel a claim unrelated to the order fails", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order2")}/claims/${IdMap.getId("test-claim")}/cancel`, {
                adminSession: {
                    jwt: {
                        userId: IdMap.getId("admin_user")
                    }
