...
describe("POST /admin/orders/:id/return", () => {
    describe("successfully returns full order", () => {
        ...
        beforeAll(async () => {
            jest.clearAllMocks();
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order")}/return`, {
                payload: {
                    items: [
                        {
    ...
    describe("the api call overrides notification settings of order", () => {
        it("eventBus is called with the proper no notification feature", async () => {
            jest.clearAllMocks();
            const subject = await request("POST", `/admin/orders/${IdMap.getId("test-order")}/return`, {
                payload: {
                    items: [
                        {
                            item_id: IdMap.getId("existingLine"),
    ...
    describe("the api call inherits notification settings of order", () => {
        it("eventBus is called with the proper no notification feature", async () => {
            jest.clearAllMocks();
            await request("POST", `/admin/orders/${IdMap.getId("test-order")}/return`, {
                payload: {
                    items: [
                        {
                            item_id: IdMap.getId("existingLine"),
