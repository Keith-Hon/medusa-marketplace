...
describe("POST /admin/orders/:id/fulfillments/:fulfillment_id/cancel", () => {
    describe("successfully cancels a fulfillment", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order")}/fulfillments/${IdMap.getId("order-fulfillment")}/cancel`, {
                adminSession: {
                    jwt: {
    ...
    describe("Trying to cancel a fulfillment unrelated to the order fails", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/orders/${IdMap.getId("test-order2")}/fulfillments/${IdMap.getId("order-fulfillment")}/cancel`, {
                adminSession: {
                    jwt: {
                        userId: IdMap.getId("admin_user")
                    }
