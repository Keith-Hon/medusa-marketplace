...
import { EventBusService, OrderService, ReturnService } from "../../../../services";
        ...
        while (inProgress) {
            switch (idempotencyKey.recovery_point) {
                case "started": {
                    await manager.transaction(async (transactionManager) => {
                        const { key, error } = await idempotencyKeyService.withTransaction(transactionManager).workStage(idempotencyKey.idempotency_key, async (manager) => {
                            ...
                            const createdReturn = await returnService.withTransaction(manager).create(returnObj);

                            if (value.return_shipping) {
                                await returnService.withTransaction(manager).fulfill(createdReturn.id);
                            }

                            await eventBus.withTransaction(manager).emit("order.return_requested", {
                                id,
                ...
                case "return_requested": {
                    await manager.transaction(async (transactionManager) => {
                        const { key, error } = await idempotencyKeyService.withTransaction(transactionManager).workStage(idempotencyKey.idempotency_key, async (manager) => {
                            let order = await orderService.withTransaction(manager).retrieve(id, { relations: ["returns"] });
                            ...
                            if (value.receive_now) {
                                ...
                                if (!ret.length) {
                                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Return not found`);
                                }

                                ret = ret[0];

                                order = await returnService.withTransaction(manager).receive(ret.id, value.items, value.refund);
                            }
