...
describe("POST /admin/users/password-token", () => {
    describe("successfully generates reset password token", () => {
        ...
        it("calls UserService retrieve", () => {
            ...
            expect(UserServiceMock.retrieveByEmail).toHaveBeenCalledWith("vandijk@test.dk");
        });

        it("calls UserService generateResetPasswordToken", () => {
            expect(UserServiceMock.generateResetPasswordToken).toHaveBeenCalledTimes(1);
            expect(UserServiceMock.generateResetPasswordToken).toHaveBeenCalledWith(IdMap.getId("vandijk"));
        });
