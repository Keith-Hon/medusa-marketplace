...
export default async (req, res) => {
    ...
    try {
        ...
        let user: User;
        try {
            user = await userService.retrieveByEmail(validated.email || decoded?.email, {
        ...
        const verifiedToken = (await jwt.verify(validated.token, user.password_hash)) as payload;
        ...
        const userResult = await manager.transaction(async (transactionManager) => {
            return await userService.withTransaction(transactionManager).setPassword_(user.id, validated.password);
        });
