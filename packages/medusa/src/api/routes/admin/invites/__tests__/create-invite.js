...
describe("POST /invites", () => {
    describe("checks that role must not be empty", () => {
        ...
        it("throws when role is empty", () => {
            ...
            expect(subject.error.text).toEqual(`{"type":"invalid_data","message":"user must be an email"}`);
    ...
    describe("successfully creates an invite", () => {
        ...
        it("calls InviteService create", () => {
            ...
            expect(InviteServiceMock.create).toHaveBeenCalledWith("lebron@james.com", UserRole.ADMIN);
        });
    });
});
