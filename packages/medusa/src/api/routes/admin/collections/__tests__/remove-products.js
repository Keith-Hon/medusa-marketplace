...
describe("DELETE /admin/collections/:id/products/batch", () => {
    describe("successfully removes products from collection", () => {
        ...
        beforeAll(async () => {
            subject = await request("DELETE", `/admin/collections/${IdMap.getId("col")}/products/batch`, {
                payload: {
        ...
        it("product collection service remove products", () => {
            expect(ProductCollectionServiceMock.removeProducts).toHaveBeenCalledTimes(1);
            expect(ProductCollectionServiceMock.removeProducts).toHaveBeenCalledWith(IdMap.getId("col"), ["prod_1", "prod_2"]);
        });
    });
