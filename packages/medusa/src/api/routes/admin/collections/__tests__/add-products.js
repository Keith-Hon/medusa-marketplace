...
describe("POST /admin/collections/:id/products/batch", () => {
    describe("successfully adds products to collection", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/collections/${IdMap.getId("col")}/products/batch`, {
                payload: {
        ...
        it("product collection service update", () => {
            ...
            expect(ProductCollectionServiceMock.addProducts).toHaveBeenCalledWith(IdMap.getId("col"), ["prod_1", "prod_2"]);
        });
    });
    ...
    describe("error on non-existing collection", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/admin/collections/null/products/batch`, {
                payload: {
                    product_ids: ["prod_1", "prod_2"]
                },
                adminSession: {
        ...
        it("throws error", () => {
            expect(subject.body.message).toBe("Product collection with id: null was not found");
        });
    });
