...
describe("DELETE /admin/collections/:id", () => {
    describe("successful removes collection", () => {
        ...
        beforeAll(async () => {
            subject = await request("DELETE", `/admin/collections/${IdMap.getId("collection")}`, {
                adminSession: {
                    jwt: {
        ...
        it("calls product collection service delete", () => {
            ...
            expect(ProductCollectionServiceMock.delete).toHaveBeenCalledWith(IdMap.getId("collection"));
        });
