...
describe("GET /admin/categories/:id", () => {
    describe("get collection by id successfully", () => {
        let subject;
        beforeAll(async () => {
            subject = await request("GET", `/admin/collections/${IdMap.getId("col")}`, {
                adminSession: {
                    jwt: {
        ...
        it("calls retrieve from product collection service", () => {
            ...
            expect(ProductCollectionServiceMock.retrieve).toHaveBeenCalledWith(IdMap.getId("col"), { relations: defaultAdminCollectionsRelations });
        });
