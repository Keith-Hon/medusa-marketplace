            ...
            case "started": {
                await manager.transaction(async (transactionManager) => {
                    const { key, error } = await idempotencyKeyService.withTransaction(transactionManager).workStage(idempotencyKey.idempotency_key, async (manager) => {
                        const order = await orderService.withTransaction(manager).retrieve(swapDto.order_id, {
                        ...
                        const swap = await swapService.withTransaction(manager).create(order, swapDto.return_items, swapDto.additional_items, returnShipping, {
                ...
                await manager.transaction(async (transactionManager) => {
                    const { key, error } = await idempotencyKeyService
                        ...
                        .workStage(idempotencyKey.idempotency_key, async (transactionManager: EntityManager) => {
                            ...
                            const swap = await swapService.withTransaction(transactionManager).retrieve(swaps[0].id, {
