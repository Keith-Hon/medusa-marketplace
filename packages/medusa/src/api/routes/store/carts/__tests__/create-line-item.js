...
describe("POST /store/carts/:id", () => {
    describe("successfully creates a line item", () => {
        ...
        beforeAll(async () => {
            subject = await request("POST", `/store/carts/${IdMap.getId("emptyCart")}/line-items`, {
                payload: {
        ...
        it("calls LineItemService generate", () => {
            ...
            expect(LineItemServiceMock.generate).toHaveBeenCalledWith(IdMap.getId("testVariant"), IdMap.getId("testRegion"), 3, { metadata: undefined });
        });
    ...
    describe("handles unsuccessful line item generation", () => {
        ...
        it("calls LineItemService generate", () => {
            expect(LineItemServiceMock.generate).toHaveBeenCalledTimes(1);
            expect(LineItemServiceMock.generate).toHaveBeenCalledWith(IdMap.getId("fail"), IdMap.getId("testRegion"), 3, { metadata: undefined });
        });
