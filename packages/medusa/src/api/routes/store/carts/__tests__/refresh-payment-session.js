...
describe("POST /store/carts/:id/payment-session/update", () => {
    describe("successfully updates the payment session", () => {
        ...
        beforeAll(async () => {
            ...
            subject = await request("POST", `/store/carts/${cartId}/payment-sessions/stripe/refresh`, {});
        });
        ...
        it("calls CartService updatePaymentSession", () => {
            ...
            expect(CartServiceMock.refreshPaymentSession).toHaveBeenCalledWith(IdMap.getId("cartWithPaySessions"), "stripe");
        });
