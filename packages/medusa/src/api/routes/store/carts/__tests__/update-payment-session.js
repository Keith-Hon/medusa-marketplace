...
describe("POST /store/carts/:id/payment-session/update", () => {
    describe("successfully updates the payment session", () => {
        ...
        beforeAll(async () => {
            ...
            subject = await request("POST", `/store/carts/${cartId}/payment-sessions/default_provider`, {
                payload: {
                    data: {
                        data: "Something"
                    }
        ...
        it("calls CartService updatePaymentSession", () => {
            ...
            expect(CartServiceMock.setPaymentSession).toHaveBeenCalledWith(IdMap.getId("cartWithPaySessions"), "default_provider");

            expect(CartServiceMock.updatePaymentSession).toHaveBeenCalledTimes(1);
            expect(CartServiceMock.updatePaymentSession).toHaveBeenCalledWith(IdMap.getId("cartWithPaySessions"), {
                data: "Something"
            });
        });
