...
export default async (req, res) => {
    ...
    const validated = await validator(StorePostCartsCartLineItemsItemReq, req.body);
    ...
    await manager.transaction(async (m) => {
        ...
        if (validated.quantity === 0) {
            ...
            await cartService.withTransaction(m).updateLineItem(id, line_id, lineItemUpdate);
        }
