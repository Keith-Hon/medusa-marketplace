...
export const decorateLineItemsWithTotals = async (cart: Cart, req: Request, options: { force_taxes: boolean } = { force_taxes: false }): Promise<Cart> => {
        ...
        const items = await manager.transaction(async (transactionManager) => {
            return await Promise.all(
                cart.items.map(async (item: LineItem) => {
                    const itemTotals = await totalsService.withTransaction(transactionManager).getLineItemTotals(item, cart, {
