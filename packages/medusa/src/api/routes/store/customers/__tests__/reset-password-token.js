...
describe("POST /store/customers/password-token", () => {
    describe("successfully creates a customer", () => {
        ...
        it("calls CustomerService retrieve", () => {
            ...
            expect(CustomerServiceMock.retrieveByEmail).toHaveBeenCalledWith("lebron@james.com");
        });

        it("calls CustomerService retrieve", () => {
            expect(CustomerServiceMock.generateResetPasswordToken).toHaveBeenCalledTimes(1);
            expect(CustomerServiceMock.generateResetPasswordToken).toHaveBeenCalledWith(IdMap.getId("lebron"));
        });
