...
export function validateSalesChannelsExist(getSalesChannels: (req) => ProductSalesChannelReq[] | undefined): (req: Request, res: Response, next: NextFunction) => Promise<void> {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        const nonExistingSalesChannels = salesChannelIds.filter((scId) => existingChannels.findIndex((sc) => sc.id === scId) === -1);

        if (nonExistingSalesChannels.length) {
            req.errors = req.errors ?? [];
            req.errors.push(`Sales Channels ${nonExistingSalesChannels.join(", ")} do not exist`);
        }
