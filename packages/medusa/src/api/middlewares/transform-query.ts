...
export function transformQuery<T extends RequestQueryFields, TEntity extends BaseEntity>(
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const validated: T = await validator<T, Record<string, unknown>>(plainToClass, req.query, config);
            req.validatedQuery = validated;

            req.filterableFields = omit(validated, ["limit", "offset", "expand", "fields", "order"]);
