...
class SearchIndexingSubscriber {
    ...
    indexDocuments = async (): Promise<void> => {
        ...
        while (hasMore) {
            ...
            if (products.length > 0) {
                await this.searchService_.addDocuments(ProductService.IndexName, products, indexTypes.products);
    ...
    protected async retrieveNextProducts(lastSeenId: string, take: number): Promise<Product[]> {
        return await this.productService_.list(
            { id: { gt: lastSeenId } },
            {
                ...
                relations: ["variants", "tags", "type", "collection", "variants.prices", "images", "variants.options", "options"],
                take: take,
                order: { id: "ASC" }
            }
        );
