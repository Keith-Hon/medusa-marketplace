...
class BatchJobSubscriber {
    ...
    constructor({ eventBusService, batchJobService, strategyResolverService }: InjectedDependencies) {
        this.eventBusService_ = eventBusService;
        this.batchJobService_ = batchJobService;
        this.strategyResolver_ = strategyResolverService;

        this.eventBusService_.subscribe(BatchJobService.Events.CREATED, this.preProcessBatchJob).subscribe(BatchJobService.Events.CONFIRMED, this.processBatchJob);
    }

    preProcessBatchJob = async (data): Promise<void> => {
        const batchJob = await this.batchJobService_.retrieve(data.id);

        const batchJobStrategy = this.strategyResolver_.resolveBatchJobByType(batchJob.type);
