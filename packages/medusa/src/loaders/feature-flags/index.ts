...
export default (configModule: { featureFlags?: Record<string, string | boolean> } = {}, logger?: Logger, flagDirectory?: string): FlagRouter => {
    ...
    for (const flag of supportedFlags) {
        ...
        switch (true) {
            case typeof process.env[flagSettings.env_key] !== "undefined":
                if (logger) {
                    logger.info(`Using flag ${flagSettings.env_key} from environment with value ${process.env[flagSettings.env_key]}`);
                }
            ...
            case typeof projectConfigFlags[flagSettings.key] !== "undefined":
                if (logger) {
                    logger.info(`Using flag ${flagSettings.key} from project config with value ${projectConfigFlags[flagSettings.key]}`);
                }
