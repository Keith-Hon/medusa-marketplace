...
import { BaseService as LegacyBaseService, FileService, FulfillmentService, OauthService, PaymentService } from "medusa-interfaces";
...
async function registerMedusaApi(pluginDetails: PluginDetails, container: MedusaContainer): Promise<void> {
        ...
        switch (true) {
            case isTaxCalculationStrategy(module.prototype): {
                if (!("taxCalculationStrategy" in registeredServices)) {
                    container.register({
                        taxCalculationStrategy: asFunction((cradle) => new module(cradle, pluginDetails.options)).singleton()
                    });
            ...
            case isPriceSelectionStrategy(module.prototype): {
                if (!("priceSelectionStrategy" in registeredServices)) {
                    container.register({
                        priceSelectionStrategy: asFunction((cradle) => new module(cradle, pluginDetails.options)).singleton()
                    });
 ...
 */
function registerApi(pluginDetails: PluginDetails, app: Express, rootDirectory = "", container: MedusaContainer, activityId: string): Express {
    const logger = container.resolve<Logger>("logger");
...
export async function registerServices(pluginDetails: PluginDetails, container: MedusaContainer): Promise<void> {
    ...
    await Promise.all(
        files.map(async (fn) => {
            ...
            if (!(loaded.prototype instanceof LegacyBaseService) && !(loaded.prototype instanceof BaseService)) {
                const logger = container.resolve<Logger>("logger");
