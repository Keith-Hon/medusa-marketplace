...
export default ({ container, configModule, isTest }: LoaderOptions): void => {
    const useMock = typeof isTest !== "undefined" ? isTest : process.env.NODE_ENV === "test";

    const corePath = useMock ? "../strategies/__mocks__/[!__]*.js" : "../strategies/**/[!__]*.js";
    ...
    core.forEach((fn) => {
        ...
        if (isBatchJobStrategy(loaded.prototype)) {
            ...
            container.register({
                [name]: asFunction((cradle) => new loaded(cradle, configModule)).singleton(),
