...
const silentResolution = <T>(container: AwilixContainer, name: string, logger: Logger): T | never | undefined => {
    try {
        ...
        if (err.name !== "AwilixResolutionError") {
            ...
            logger.warn(`You don't have any ${identifier} provider plugins installed. You may want to add one to your project.`);
        }
        return;
    }
};
...
export default async ({ container }: { container: AwilixContainer }): Promise<void> => {
    ...
    await entityManager.transaction(async (manager: EntityManager) => {
        ...
        const payProviders = silentResolution<typeof BasePaymentService[]>(container, "paymentProviders", logger) || [];
        ...
        const notiProviders = silentResolution<typeof BaseNotificationService[]>(container, "notificationProviders", logger) || [];
        ...
        const fulfilProviders = silentResolution<typeof BaseFulfillmentService[]>(container, "fulfillmentProviders", logger) || [];
        ...
        const taxProviders = silentResolution<AbstractTaxService[]>(container, "taxProviders", logger) || [];
