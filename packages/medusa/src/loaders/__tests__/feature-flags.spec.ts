...
describe("feature flags", () => {
    ...
    it("should load the flag from project", async () => {
        writeFileSync(resolve(getFolderTestTargetDirectoryPath("flags"), "flag-1.js"), buildFeatureFlag("flag-1", true));

        const flags = await loadFeatureFlags({ featureFlags: { flag_1: false } }, undefined, getFolderTestTargetDirectoryPath("flags"));
    ...
    it("should load mix of flags", async () => {
        ...
        writeFileSync(resolve(getFolderTestTargetDirectoryPath("flags"), "flag-2.js"), buildFeatureFlag("flag-2", true));

        writeFileSync(resolve(getFolderTestTargetDirectoryPath("flags"), "flag-3.js"), buildFeatureFlag("flag-3", true));

        const flags = await loadFeatureFlags({ featureFlags: { flag_2: false } }, undefined, getFolderTestTargetDirectoryPath("flags"));
