...
import { In, Not, DeleteResult, SelectQueryBuilder, EntityRepository, FindManyOptions, FindOptionsUtils, Repository } from "typeorm";
...
const resolveableFields = ["product_count", "product_type_count", "shipping_option_count"];
...
export class TaxRateRepository extends Repository<TaxRate> {
    getFindQueryBuilder(findOptions: FindManyOptions<TaxRate>) {
        ...
        FindOptionsUtils.applyFindManyOptionsOrConditionsToQueryBuilder(qb, cleanOptions);
    ...
    applyResolutionsToQueryBuilder(qb: SelectQueryBuilder<TaxRate>, resolverFields: string[]): SelectQueryBuilder<TaxRate> {
        for (const k of resolverFields) {
            switch (k) {
                case "product_count":
                    qb.loadRelationCountAndMap(`${qb.alias}.product_count`, `${qb.alias}.products`);
                    break;
                case "product_type_count":
                    qb.loadRelationCountAndMap(`${qb.alias}.product_type_count`, `${qb.alias}.product_types`);
                    break;
                case "shipping_option_count":
                    qb.loadRelationCountAndMap(`${qb.alias}.shipping_option_count`, `${qb.alias}.shipping_options`);
                    break;
                default:
                    break;
            }
    ...
    async removeFromProduct(id: string, productIds: string[]): Promise<DeleteResult> {
    ...
    async addToProduct(id: string, productIds: string[], overrideExisting: boolean = false): Promise<ProductTaxRate[]> {
        const toInsert = productIds.map((pId) => ({ rate_id: id, product_id: pId }));
        const insertResult = await this.createQueryBuilder().insert().orIgnore(true).into(ProductTaxRate).values(toInsert).execute();
        ...
        return await this.manager.createQueryBuilder(ProductTaxRate, "ptr").select().where(insertResult.identifiers).getMany();
    }

    async removeFromProductType(id: string, productTypeIds: string[]): Promise<DeleteResult> {
        return await this.createQueryBuilder()
            .delete()
    ...
    async addToProductType(id: string, productTypeIds: string[], overrideExisting: boolean = false): Promise<ProductTypeTaxRate[]> {
        const toInsert = productTypeIds.map((pId) => ({
            rate_id: id,
            product_type_id: pId
        }));
        const insertResult = await this.createQueryBuilder().insert().orIgnore(true).into(ProductTypeTaxRate).values(toInsert).execute();
        ...
        return await this.manager.createQueryBuilder(ProductTypeTaxRate, "ptr").select().where(insertResult.identifiers).getMany();
    }

    async removeFromShippingOption(id: string, optionIds: string[]): Promise<DeleteResult> {
        return await this.createQueryBuilder()
            .delete()
    ...
    async addToShippingOption(id: string, optionIds: string[], overrideExisting: boolean = false): Promise<ShippingTaxRate[]> {
        const toInsert = optionIds.map((pId) => ({
            rate_id: id,
            shipping_option_id: pId
        }));
        const insertResult = await this.createQueryBuilder().insert().orIgnore(true).into(ShippingTaxRate).values(toInsert).execute();
        ...
        return await this.manager.createQueryBuilder(ShippingTaxRate, "str").select().where(insertResult.identifiers).getMany();
    }
    ...
    async listByProduct(productId: string, config: TaxRateListByConfig) {
        ...
        const results = await Promise.all([productRates.getMany(), typeRates.getMany()]);
    ...
    async listByShippingOption(optionId: string) {
        let rates = this.createQueryBuilder("txr").leftJoin(ShippingTaxRate, "ptr", "ptr.rate_id = txr.id").where("ptr.shipping_option_id = :optionId", { optionId });
