...
import { Brackets, EntityRepository, FindManyOptions, Repository } from "typeorm";
...
export class GiftCardRepository extends Repository<GiftCard> {
    public async findWithRelations(
        ...
        idsOrOptionsWithoutRelations: Omit<FindManyOptions<GiftCard>, "relations"> | string[] = {}
        ...
        return [Object.values(entitiesAndRelationsById).map((v) => merge({}, ...v)), count];
    }
    ...
    protected async queryGiftCards(
        ...
        const qb = this.createQueryBuilder("gift_card")
            ...
            .andWhere(
                new Brackets((qb) => {
                    return qb.where(`gift_card.code ILIKE :q`, { q: `%${q}%` }).orWhere(`display_id::varchar(255) ILIKE :dId`, { dId: `${q}` });
                })
            );
    ...
    public async listGiftCards(query: ExtendedFindConfig<GiftCard, QuerySelector<GiftCard>>, rels: (keyof GiftCard | string)[] = [], q?: string): Promise<GiftCard[]> {
        if (q) {
            const where = query.where;
            delete where.id;
    ...
    public async findOneWithRelations(relations: Array<keyof GiftCard> = [], optionsWithoutRelations: Omit<FindManyOptions<GiftCard>, "relations"> = {}): Promise<GiftCard> {
        // Limit 1
