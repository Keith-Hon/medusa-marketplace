import { DeleteResult, EntityRepository, EntityTarget, In, Not, Repository } from "typeorm";
...
export class DiscountConditionRepository extends Repository<DiscountCondition> {
    async findOneWithDiscount(conditionId: string, discountId: string): Promise<(DiscountCondition & { discount: Discount }) | undefined> {
        return (await this.createQueryBuilder("condition")
            .leftJoinAndMapOne("condition.discount", Discount, "discount", `condition.discount_rule_id = discount.rule_id and discount.id = :discId and condition.id = :dcId`, {
                discId: discountId,
                dcId: conditionId
            })
    ...
    getJoinTableResourceIdentifiers(type: string): {
        ...
        let joinTableForeignKey: DiscountConditionJoinTableForeignKey = DiscountConditionJoinTableForeignKey.PRODUCT_ID;
        let joinTableKey = "id";
        ...
        switch (type) {
            ...
            case DiscountConditionType.PRODUCT_TYPES: {
                resourceKey = "type_id";
                joinTableForeignKey = DiscountConditionJoinTableForeignKey.PRODUCT_TYPE_ID;
                joinTable = "product";
            ...
            case DiscountConditionType.PRODUCT_COLLECTIONS: {
                ...
                joinTableForeignKey = DiscountConditionJoinTableForeignKey.PRODUCT_COLLECTION_ID;
                joinTable = "product";
            ...
            case DiscountConditionType.PRODUCT_TAGS: {
                ...
                joinTableForeignKey = DiscountConditionJoinTableForeignKey.PRODUCT_TAG_ID;
            ...
            case DiscountConditionType.CUSTOMER_GROUPS: {
                ...
                joinTableForeignKey = DiscountConditionJoinTableForeignKey.CUSTOMER_GROUP_ID;
    ...
    async removeConditionResources(id: string, type: DiscountConditionType, resourceIds: string[]): Promise<DeleteResult | void> {
        const { conditionTable, joinTableForeignKey } = this.getJoinTableResourceIdentifiers(type);
    ...
    async addConditionResources(
        ...
        const insertResult = await this.createQueryBuilder().insert().orIgnore(true).into(conditionTable).values(toInsert).execute();
        ...
        return await this.manager.createQueryBuilder(conditionTable, "discon").select().where(insertResult.identifiers).getMany();
    }

    async queryConditionTable({ type, condId, resourceId }): Promise<number> {
        const { conditionTable, joinTable, joinTableForeignKey, resourceKey, joinTableKey } = this.getJoinTableResourceIdentifiers(type);

        return await this.manager
            .createQueryBuilder(conditionTable, "dc")
            .innerJoin(joinTable, "resource", `dc.${joinTableForeignKey} = resource.${resourceKey} and resource.${joinTableKey} = :resourceId `, {
                resourceId
            })
    ...
    async isValidForProduct(discountRuleId: string, productId: string): Promise<boolean> {
        ...
        for (const condition of discountConditions) {
            ...
            if (condition.operator === DiscountConditionOperator.IN && numConditions === 0) {
                return false;
            }

            if (condition.operator === DiscountConditionOperator.NOT_IN && numConditions > 0) {
                return false;
            }
        }
    ...
    async canApplyForCustomer(discountRuleId: string, customerId: string): Promise<boolean> {
        const discountConditions = await this.createQueryBuilder("discon")
            .select(["discon.id", "discon.type", "discon.operator"])
            .where("discon.discount_rule_id = :discountRuleId", {
                discountRuleId
