...
import { Brackets, EntityRepository, FindManyOptions, FindOperator, Repository } from "typeorm";
...
export class PriceListRepository extends Repository<PriceList> {
    ...
    public async findWithRelations(relations: string[] = [], idsOrOptionsWithoutRelations: Omit<FindManyOptions<PriceList>, "relations"> | string[] = {}): Promise<PriceList[]> {
        let entities;
        ...
        return map(entitiesAndRelationsById, (entityAndRelations) => this.merge(this.create(), ...entityAndRelations));
    }

    public async findOneWithRelations(relations: (keyof PriceList)[] = [], options: Omit<FindManyOptions<PriceList>, "relations"> = {}): Promise<PriceList | undefined> {
        options.take = 1;

        return (await this.findWithRelations(relations, options))?.pop();
    }

    async listAndCount(query: ExtendedFindConfig<FilterablePriceListProps>, groups: FindOperator<string[]>): Promise<[PriceList[], number]> {
        const qb = this.createQueryBuilder("price_list").where(query.where).skip(query.skip).take(query.take);
