...
export class CustomerRepository extends Repository<Customer> {
    async listAndCount(query: ExtendedFindConfig<Customer, Selector<Customer>>, q: string | undefined = undefined): Promise<[Customer[], number]> {
        const groups = query.where.groups as { value: string[] };
        delete query.where.groups;

        const qb = this.createQueryBuilder("customer").skip(query.skip).take(query.take);
