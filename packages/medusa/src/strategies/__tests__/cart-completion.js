    ...
    [
        "succeeds",
        {
            ...
            validate: function (value, { cartServiceMock, orderServiceMock }) {
                ...
                expect(cartServiceMock.authorizePayment).toHaveBeenCalledWith("test-cart", {
                    idempotency_key: "ikey"
                });

                expect(orderServiceMock.createFromCart).toHaveBeenCalledTimes(1);
                expect(orderServiceMock.createFromCart).toHaveBeenCalledWith("test-cart");
    ...
    [
        "creates swap",
        {
            ...
            validate: function (value, { swapServiceMock }) {
                ...
                expect(swapServiceMock.registerCartCompletion).toHaveBeenCalledWith("testswap");
            }
        }
    ]
];
...
describe("CartCompletionStrategy", () => {
    describe("complete", () => {
        test.each(toTest)("%s", async (title, { cart, idempotencyKey, validate }) => {
