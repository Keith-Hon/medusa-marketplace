...
import { ProductExportBatchJob, ProductExportBatchJobContext, ProductExportColumnSchemaDescriptor, ProductExportPriceData, productExportSchemaDescriptors } from "./index";
...
export default class ProductExportStrategy extends AbstractBatchJobStrategy<ProductExportStrategy, InjectedDependencies> {
    ...
    protected readonly defaultRelations_ = [...defaultAdminProductRelations, "variants.prices.region"];
    /*
     *
     ...
     *
     */
    protected readonly columnDescriptors: Map<string, ProductExportColumnSchemaDescriptor> = new Map(productExportSchemaDescriptors);
    ...
    async prepareBatchJobForProcessing(
        ...
        const { limit, offset, order, fields, expand, filterable_fields, ...context } = (batchJob?.context ?? {}) as ProductExportBatchJobContext;
    ...
    async preProcessBatchJob(batchJobId: string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager) => {
            const batchJob = (await this.batchJobService_.withTransaction(transactionManager).retrieve(batchJobId)) as ProductExportBatchJob;
            ...
            const [productList, count] = await this.productService_.withTransaction(transactionManager).listAndCount(filterable_fields, {
                ...(list_config ?? {}),
            ...
            while (offset < productCount) {
                if (!products?.length) {
                    products = await this.productService_.withTransaction(transactionManager).list(filterable_fields, {
                        ...list_config,
                        skip: offset,
            ...
            await this.batchJobService_.withTransaction(transactionManager).update(batchJob, {
                context: {
                    shape: {
                        ...
                        prices: [...pricesData].map((stringifyData) => JSON.parse(stringifyData))
                    }
                },
                result: {
                    stat_descriptors: [
    ...
    async processJob(batchJobId: string): Promise<void> {
        ...
        return await this.atomicPhase_(
            async (transactionManager) => {
                let batchJob = (await this.batchJobService_.withTransaction(transactionManager).retrieve(batchJobId)) as ProductExportBatchJob;

                const { writeStream, fileKey, promise } = await this.fileService_.withTransaction(transactionManager).getUploadStreamDescriptor({
                ...
                while (offset < productCount) {
                    ...
                    batchJob = (await this.batchJobService_.withTransaction(transactionManager).update(batchJobId, {
                        result: {
                            file_size: approximateFileSize,
                            count: productCount,
    ...
    public async buildHeader(batchJob: ProductExportBatchJob): Promise<string> {
        const { prices = [], dynamicImageColumnCount, dynamicOptionColumnCount, dynamicSalesChannelsColumnCount } = batchJob?.context?.shape ?? {};
    ...
    private appendSalesChannelsDescriptors(maxScCount: number): void {
        for (let i = 0; i < maxScCount; ++i) {
            ...
            this.columnDescriptors.set(`Sales channel ${i + 1} Description`, {
                accessor: (product: Product) => product?.sales_channels[i]?.description ?? "",
                entityName: "product"
            });
        }
    }
    ...
    private appendOptionsDescriptors(maxOptionsCount: number): void {
        for (let i = 0; i < maxOptionsCount; ++i) {
            this.columnDescriptors
                .set(`Option ${i + 1} Name`, {
                    accessor: (productOption: Product) => productOption?.options[i]?.title ?? "",
                    entityName: "product"
                })
    ...
    private appendMoneyAmountDescriptors(pricesData: ProductExportPriceData[]): void {
        for (const priceData of pricesData) {
            if (priceData.currency_code) {
                this.columnDescriptors.set(`Price ${priceData.currency_code?.toUpperCase()}`, {
                    accessor: (variant: ProductVariant) => {
                        const price = variant.prices.find((variantPrice) => {
                            return variantPrice.currency_code && priceData.currency_code && variantPrice.currency_code.toLowerCase() === priceData.currency_code.toLowerCase();
                        });
            ...
            if (priceData.region) {
                this.columnDescriptors.set(`Price ${priceData.region.name} ${priceData.region?.currency_code ? "[" + priceData.region?.currency_code.toUpperCase() + "]" : ""}`, {
                    accessor: (variant: ProductVariant) => {
                        const price = variant.prices.find((variantPrice) => {
                            return (
                                variantPrice.region &&
                                priceData.region &&
                                variantPrice.region?.name?.toLowerCase() === priceData.region?.name?.toLowerCase() &&
                                variantPrice.region?.id?.toLowerCase() === priceData.region?.id?.toLowerCase()
                            );
                        });
                        return price?.amount?.toString() ?? "";
                    },
    ...
    private async onProcessCanceled(batchJobId: string, fileKey: string): Promise<void> {
