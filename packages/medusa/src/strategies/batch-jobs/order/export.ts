...
import { OrderDescriptor, OrderExportBatchJob, OrderExportBatchJobContext, orderExportPropertiesDescriptors } from ".";
...
class OrderExportStrategy extends AbstractBatchJobStrategy<OrderExportStrategy> {
    ...
    protected readonly orderExportPropertiesDescriptors = [...orderExportPropertiesDescriptors];
    ...
    async prepareBatchJobForProcessing(
        ...
        const { limit, offset, order, fields, expand, filterable_fields, ...context } = batchJob.context as OrderExportBatchJobContext;
    ...
    async preProcessBatchJob(batchJobId: string): Promise<void> {
        return await this.atomicPhase_(async (transactionManager) => {
            const batchJob = (await this.batchJobService_.withTransaction(transactionManager).retrieve(batchJobId)) as OrderExportBatchJob;
            ...
            if (!count) {
                const [, orderCount] = await this.orderService_.withTransaction(transactionManager).listAndCount(filterable_fields, {
                    ...(list_config ?? {}),
    ...
    async processJob(batchJobId: string): Promise<void> {
        ...
        return await this.atomicPhase_(
            async (transactionManager) => {
                let batchJob = (await this.batchJobService_.withTransaction(transactionManager).retrieve(batchJobId)) as OrderExportBatchJob;
                ...
                const { list_config = {}, filterable_fields = {} } = batchJob.context;
                const [, count] = await this.orderService_.listAndCount(filterable_fields, {
                    ...list_config,
                    order: { created_at: "DESC" },
                    skip: offset,
                    take: Math.min(batchJob.context.batch_size ?? Infinity, limit)
                ...
                const lineDescriptor = this.getLineDescriptor(list_config.select as string[], list_config.relations as string[]);
                ...
                while (offset < orderCount) {
                    orders = await this.orderService_.withTransaction(transactionManager).list(filterable_fields, {
                        ...list_config,
                        skip: offset,
                    ...
                    if (batchJob.status === BatchJobStatus.CANCELED) {
                        ...
                        await this.fileService_.withTransaction(transactionManager).delete({ key: fileKey });
    ...
    public async buildTemplate(): Promise<string> {
        return this.buildHeader(this.getLineDescriptor(this.defaultFields_, this.defaultRelations_));
    }

    private buildHeader(lineDescriptor: OrderDescriptor[] = this.orderExportPropertiesDescriptors): string {
        return [...lineDescriptor.map(({ title }) => title)].join(this.DELIMITER) + this.NEWLINE;
    }

    private buildCSVLine(order: Order, lineDescriptor: OrderDescriptor[]): string {
        return [...lineDescriptor.map(({ accessor }) => accessor(order))].join(this.DELIMITER) + this.NEWLINE;
    }

    private getLineDescriptor(fields: string[], relations: string[]): OrderDescriptor[] {
        return this.orderExportPropertiesDescriptors.filter(({ fieldName }) => fields.indexOf(fieldName) !== -1 || relations.indexOf(fieldName) !== -1);
    }
    ...
    private addSalesChannelColumns(): void {
        this.orderExportPropertiesDescriptors.push({
            ...
            accessor: (order: Order): string => [order.sales_channel.name, order.sales_channel.description].join(";")
        });
    }
}
