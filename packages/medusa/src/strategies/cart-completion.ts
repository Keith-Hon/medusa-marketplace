...
class CartCompletionStrategy implements ICartCompletionStrategy {
    ...
    constructor({ idempotencyKeyService, cartService, orderService, swapService }) {
    ...
    async complete(id: string, ikey: IdempotencyKey, context: RequestContext): Promise<CartCompletionResponse> {
        ...
        while (inProgress) {
            switch (idempotencyKey.recovery_point) {
                case "started": {
                    const { key, error } = await idempotencyKeyService.workStage(idempotencyKey.idempotency_key, async (manager: EntityManager) => {
                ...
                case "tax_lines_created": {
                    const { key, error } = await idempotencyKeyService.workStage(idempotencyKey.idempotency_key, async (manager: EntityManager) => {
                        const cart = await cartService.withTransaction(manager).authorizePayment(id, {
                            ...context,
                            idempotency_key: idempotencyKey.idempotency_key
                        });

                        if (cart.payment_session) {
                            if (cart.payment_session.status === "requires_more" || cart.payment_session.status === "pending") {
                                return {
                                    response_code: 200,
                                    response_body: {
                                        data: cart,
                        ...
                        switch (cart.type) {
                            case "swap": {
                                try {
                                    ...
                                    let swap = await swapService.withTransaction(manager).registerCartCompletion(swapId as string);

                                    swap = await swapService.withTransaction(manager).retrieve(swap.id, { relations: ["shipping_address"] });
                            ...
                            default: {
                                ...
                                if (!cart.payment && cart.total > 0) {
                                    throw new MedusaError(MedusaError.Types.INVALID_DATA, `Cart payment not authorized`);
                                }

                                let order: Order;
                                try {
                                    order = await orderService.withTransaction(manager).createFromCart(cart.id);
                                } catch (error) {
                                    if (error && error.message === "Order from cart already exists") {
                                        order = await orderService.withTransaction(manager).retrieveByCartId(id, {
                                    ...
                                    } else if (error && error.code === MedusaError.Codes.INSUFFICIENT_INVENTORY) {
                                        return {
                                            response_code: 409,
                                            response_body: {
                                                message: error.message,
                                ...
                                order = await orderService.withTransaction(manager).retrieve(order.id, {
                                    select: ["subtotal", "tax_total", "shipping_total", "discount_total", "total"],
                                    relations: ["shipping_address", "items", "payments"]
                                });
                ...
                default:
                    idempotencyKey = await idempotencyKeyService.update(idempotencyKey.idempotency_key, {
