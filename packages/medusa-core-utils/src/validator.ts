...
Joi['orderFilter'] = () => {
    return Joi.object().keys({
        id: Joi.string(),
        q: Joi.string(),
        status: Joi.array().items(Joi.string().valid("pending", "completed", "archived", "canceled", "requires_action")).single(),
        fulfillment_status: Joi.array()
            ...
            .single(),
        payment_status: Joi.array().items(Joi.string().valid("captured", "awaiting", "not_paid", "refunded", "partially_refunded", "canceled", "requires_action")).single(),
...
Joi['productFilter'] = () => {
    return Joi.object().keys({
        ...
        status: Joi.array().items(Joi.string().valid("proposed", "draft", "published", "rejected")).single(),
