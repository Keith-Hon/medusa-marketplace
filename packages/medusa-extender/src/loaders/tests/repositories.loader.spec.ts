...
describe("Repositories loader", () => {
    ...
    describe("overriddenRepositoriesLoader", () => {
        it(" should override MedusaOrderRepository with OrderRepository", async () => {
            ...
            const { OrderRepository: MedusaOrderRepositoryReImport } = (await import("@medusajs/medusa/dist/repositories/order")) as { OrderRepository };
            ...
            expect(new MedusaOrderRepositoryReImport().testProperty).toBe("I am the property from UserRepository that extend MedusaOrderRepository");
        });
    });
