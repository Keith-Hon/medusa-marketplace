 ...
 * @param container
 */
export async function repositoriesLoader(repositories: GetInjectableOptions<"repository">, container: MedusaContainer): Promise<void> {
...
function registerRepository(container: MedusaContainer, repositoryOptions: GetInjectableOption<"repository">): void {
    ...
    const resolutionKey = repositoryOptions.resolutionKey ?? `${lowerCaseFirst(repository.name)}${!repository.name.toLowerCase().includes("repository") ? "Repository" : ""}`;
    container.register({
