 ...
 * @param container
 */
export async function entitiesLoader(entities: GetInjectableOptions<"entity">, container: MedusaContainer): Promise<void> {
...
export function registerEntity(container: MedusaContainer, entityOptions: GetInjectableOption<"entity">): void {
    ...
    const resolutionKey = entityOptions.resolutionKey ?? `${lowerCaseFirst(entity.name)}${!entity.name.toLowerCase().includes("entity") ? "Entity" : ""}`;
    container.register({
