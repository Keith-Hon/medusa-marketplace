 ...
 * @param routers
 */
export async function adminApiLoader(app: Router, middlewares: GetInjectableOptions<"middleware">, routers: GetInjectableOptions<"router">): Promise<void> {
    const adminMiddlewares = middlewares
        .map((middleware) => ({
            ...middleware,
            routes: middleware.routes.filter((route) => route.path.startsWith("/admin")).map((route) => ({ ...route, path: route.path.replace("/admin", "") }))
        }))
    ...
    const adminRouters = routers
        .map((router) => ({
            ...router,
            routes: router.routes.filter((route) => route.path.startsWith("/admin")).map((route) => ({ ...route, path: route.path.replace("/admin", "") }))
        }))
