 ...
 * @param path
 */
export function generateComponent(name: string, { module, middleware, service, router, validator, entity, repository, migration, path }: GenerateCommandOptions): void {
    ...
    if (migration) {
        ...
        createComponentIfNecessary(componentDescriptor, getMigrationTemplate(componentDescriptor.componentName, timestamp));
    }
...
export function updateModuleImports(fullDestinationPath: string): void {
    ...
    const updateModuleImportsContent = (_fullDestinationPath: string) => {
        ...
        for (const file of files) {
            ...
            const isComponentInSubDirectory = _fullDestinationPath.split(slashRegexp).slice(-1).pop() !== resolvedModulePath.split(slashRegexp).slice(-2).shift();
            ...
            const updatedModuleContent = moduleContent
                ...
                .replace(/(import\s+.*\s+from\s+.*(?!;))/, (str: string, matches: string) => {
                    const subDirectoryRelativePath = isComponentInSubDirectory ? _fullDestinationPath.split(slashRegexp).slice(-1).pop() : null;
                    return `${matches ? `${matches}\n` : ""}import { ${componentClassName} } from './${subDirectoryRelativePath ? subDirectoryRelativePath + "/" : ""}${
