 ...
 */
export async function loadPackages(logger: Logger, packages: { name: string; version: string; isDevDependency?: boolean }[]): Promise<void> {
...
export async function getPackageManagerCommand(): Promise<string | never> {
    ...
    const installCmd = packageManager === "npm" ? "npm i" : packageManager === "yarn" ? "yarn" : packageManager === "pnpm" ? "pnpm add" : null;
    if (!installCmd) {
