...
export class Logger {
    ...
    public push(description: string, ...variables: string[]): this {
        ...
        this.logsQueue.push([`${blue(`[${this._app}]      -`)} ${date}   ${yellow(`[${this._context}]`)} ${blue(description)}`, ...variables]);
        return this;
    }
    ...
    private buildLog(color: Chalk, description: string, ...variables: string[]): this {
        const date = new Date().toLocaleString("en-US", { hour12: true });
        console.log(`${blue(`[${this._app}]      -`)} ${date}   ${yellow(`[${this._context}]`)} ${color(description)}`, ...variables);
        return this;
    }
}
