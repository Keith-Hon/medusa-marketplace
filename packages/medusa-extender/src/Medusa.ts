...
export class Medusa {
    ...
    public async load(modules: Type[]): Promise<MedusaContainer> {
        ...
        await customApiLoader(this.#express, moduleComponentsOptions.get("middleware") ?? [], moduleComponentsOptions.get("router") ?? []);
        await adminApiLoader(this.#express, moduleComponentsOptions.get("middleware") ?? [], moduleComponentsOptions.get("router") ?? []);
        await storeApiLoader(this.#express, moduleComponentsOptions.get("middleware") ?? [], moduleComponentsOptions.get("router") ?? []);
        await databaseLoader(moduleComponentsOptions.get("entity") ?? [], moduleComponentsOptions.get("repository") ?? [], moduleComponentsOptions.get("migration") ?? []);
