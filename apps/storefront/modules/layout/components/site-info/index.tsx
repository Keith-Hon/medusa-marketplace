...
const Collapsible: React.FC<{ label: string; children?: React.ReactNode }> = ({ label, children }: { label: string; children?: React.ReactNode }) => {
    return (
        <div className="pb-4">
            <Disclosure>
                {({ open }) => {
...
const AnimatedButton = ({ open }: { open: boolean }) => {
    return (
        <div className="w-[15px] h-[15px] relative cursor-pointer">
            ...
            <div className={clsx("bg-gray-900 absolute left-[7px] inset-y-0 w-px transition-all duration-300 ease-out", { "rotate-90": open })} />
        </div>
    );
};
