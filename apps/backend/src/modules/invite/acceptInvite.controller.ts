...
export default async (req, res) => {
    ...
    await manager.transaction(async (m) => {
        ...
        const user = await inviteService.withTransaction(m).accept(validated.token, validated.user);
        ...
        if (store_id) {
            ...
            throw new MedusaError(MedusaError.Types.INVALID_DATA, `No store id assocaited with the invite of id: ${decoded.invite_id}`);
        }
    });
};
